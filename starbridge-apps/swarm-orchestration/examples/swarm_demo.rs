...
use starbridge_swarm_orchestration::{
    ...
    build_drain_action, build_open_board_action, coordinator_child_program_vk, dispatch_within_budget,
    field_from_bytes, swarm_factory_descriptor,
};
...
fn main() {
    ...
    println!("        board cell {} · birth receipt {}", short(board.as_bytes()), short(&r.receipt_hash()));
    ...
    println!("        receipt {} · worker-A's meter = {} · async wake deposited", short(&d.receipt_hash()), slot(&exec, board, SPENT_A_SLOT));

    // worker-A drains the wake in its OWN separate turn.
    let drain = exec
        .submit_action(&cclerk, build_drain_action(&cclerk, worker_a, 2, field_from_bytes(b"index-the-docs")))
        .expect("worker-A drain commits");
    println!("        worker-A DRAINS the wake in its OWN turn · receipt {}", short(&drain.receipt_hash()));
    println!(
    ...
    let (sa, sb, bud) = (slot(&exec, board, SPENT_A_SLOT), slot(&exec, board, SPENT_B_SLOT), slot(&exec, board, BUDGET_SLOT));
    println!("[3] CONSERVE dispatch 'summarize' -> worker-B  (cost {cost_b}, epoch 2->3)");
    println!("        spent_a({sa}) + spent_b({sb}) = {} <= budget({bud}) — the swarm spent at most its mandate\n", sa + sb);

    // ── 4. REFUSE the breach: an over-budget dispatch is rejected by the executor ──
    println!("[4] REFUSE a budget breach: dispatch 'runaway' -> worker-B  (cost 400 ⇒ {}+400 > {bud})", sa);
    let breach = build_dispatch_action(&cclerk, board, Worker::A, worker_a, cost_a, 400, 4, "runaway");
    ...
    let (sa2, sb2) = (slot(&exec, board, SPENT_A_SLOT), slot(&exec, board, SPENT_B_SLOT));
    println!("        fail-closed: meters unchanged (spent_a={sa2}, spent_b={sb2}) — the swarm cannot exceed B\n");
    ...
    println!("[5] REFUSE an over-grant: worker-A reaches the treasury (a cell the swarm holds NO cap to)");
    let over_reach = cclerk.make_action(
        worker_a,
        "exfiltrate",
        vec![Effect::Transfer { from: treasury, to: worker_a, amount: 1_000 }],
    );
    ...
    let treasury_bal = exec.with_ledger_mut(|l| l.get(&treasury).map(|c| c.state.balance()).unwrap_or(0));
    println!(
        "        fail-closed: treasury untouched (balance {treasury_bal}) — a worker cannot exceed its mandate\n"
    );
    println!("=== every action provably authorized, recorded, budgeted, and coordinated — without trusting the loops ===");
