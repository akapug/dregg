...
async fn the_three_job_roles_see_different_cap_only_surfaces() {
    ...
    assert_eq!(visible(&router, "signature").await, serde_json::json!(["view_job"]));
    assert_eq!(visible(&router, "either").await, serde_json::json!(["view_job"]));
    assert_eq!(visible(&router, "root").await, serde_json::json!(["view_job"]));
}
...
async fn the_app_ships_a_web_component_surface_and_a_manifest() {
    ...
    assert_eq!(m["discoverable"], serde_json::json!(["compute", "marketplace"]));
