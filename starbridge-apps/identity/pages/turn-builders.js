...
// Slot indices + issuer-lifecycle topics come from the AUTOGENERATED constants
// module (rendered from src/lib.rs), so the JS cannot drift from the slot layout
// / event vocabulary the executor enforces.
import {
...
const TOPIC_ISSUED    = TOPICS.ISSUED;
const TOPIC_REVOKED   = TOPICS.REVOKED;
const TOPIC_PRESENTED = TOPICS.PRESENTED;
// JS-only display events emitted by the in-browser verifier (no Rust
// counterpart — credential verification is a read path, not a turn).
