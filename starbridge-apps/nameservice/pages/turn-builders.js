...
// Slot indices + topic names + factory-vk come from the AUTOGENERATED
// constants module (rendered from src/lib.rs by the `constants_generator`
// example). This is the anti-drift bridge: the JS can no longer disagree with
// the Rust slot layout / event vocabulary the executor enforces.
import {
  NAME_HASH_SLOT,
  OWNER_HASH_SLOT,
  EXPIRY_SLOT,
  REVOKED_SLOT,
  RESOLVE_TARGET_SLOT,
  REVOKED_TOMBSTONE_PREFIX as REVOKED_TOMBSTONE_PREFIX_STR,
  TOPICS,
} from './constants.generated.js';

const TOPIC_REGISTERED   = TOPICS.REGISTERED;
const TOPIC_RENEWED      = TOPICS.RENEWED;
const TOPIC_TRANSFERRED  = TOPICS.TRANSFERRED;
const TOPIC_REVOKED      = TOPICS.REVOKED;
const TOPIC_TARGET_SET   = TOPICS.TARGET_SET;

const REVOKED_TOMBSTONE_PREFIX = new TextEncoder().encode(REVOKED_TOMBSTONE_PREFIX_STR);
