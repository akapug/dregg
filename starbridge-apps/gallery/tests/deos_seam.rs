...
use starbridge_gallery::{
    CURATOR_SLOT, FEATURED_SLOT, PHASE_CURATED, PHASE_REVEAL, PHASE_SUBMISSION, PHASE_SLOT,
...
fn seeding_installs_the_gallery_program_and_submission_phase() {
    ...
    assert_eq!(state.fields[PHASE_SLOT], field_from_u64(PHASE_SUBMISSION), "PHASE seeded to SUBMISSION");
    assert_ne!(state.fields[CURATOR_SLOT], [0u8; 32], "the curator is bound");
}
...
fn an_artist_submits_a_sealed_piece_through_the_gated_fire_a_real_verified_turn() {
    ...
    let state = executor.cell_state(cclerk.cell_id()).unwrap();
    assert_eq!(state.fields[submit_slot(1)], sub2.seal(), "the second seal landed on the next free slot");
    assert_eq!(state.fields[submit_slot(0)], sub.seal(), "the first seal is untouched (frozen)");
}
...
fn the_lit_button_set_tracks_the_phase_the_htmx_tooth() {
    ...
    assert!(lit_submission.contains(&"submit".to_string()), "SUBMISSION: submit lights");
    assert!(lit_submission.contains(&"close_submissions".to_string()), "SUBMISSION: close lights");
    assert!(!lit_submission.contains(&"reveal".to_string()), "SUBMISSION: reveal dark");
    assert!(!lit_submission.contains(&"curate".to_string()), "SUBMISSION: curate dark");
    ...
    assert!(!lit_reveal.contains(&"submit".to_string()), "REVEAL: submit darkens (the htmx tooth)");
    assert!(!lit_reveal.contains(&"close_submissions".to_string()), "REVEAL: close darkens");
    assert!(lit_reveal.contains(&"reveal".to_string()), "REVEAL: reveal lights (the htmx tooth)");
    assert!(lit_reveal.contains(&"curate".to_string()), "REVEAL: curate lights");
}
...
fn a_visitor_cannot_fire_curate_the_cap_tooth_bites_in_band() {
    ...
    assert_eq!(s.fields[PHASE_SLOT], field_from_u64(PHASE_REVEAL), "still REVEAL — curate never fired");
}
...
fn the_executor_re_enforces_swapping_a_committed_submission_is_refused_writeonce() {
    ...
    assert!(refused.is_err(), "swapping a committed sealed submission must be refused by the executor");
...
fn the_executor_re_enforces_a_phase_rewind_is_refused_strictmonotonic() {
    ...
    let r = fire_curate(&app, &AuthRequired::None, field_from_u64(11), featured.seal(), &cclerk, &executor)
        .expect("the honest curate commits");
    assert_ne!(r.turn_hash, [0u8; 32], "a real verified curate turn");
    let after = executor.cell_state(cell).unwrap();
    assert_eq!(after.fields[PHASE_SLOT], field_from_u64(PHASE_CURATED), "the call CURATED");
    assert_eq!(after.fields[FEATURED_SLOT], field_from_u64(11), "the featured artist is announced");
}
...
fn the_honest_lifecycle_runs_through_the_gated_fires() {
    ...
    fire_submit(&app, &AuthRequired::Either, sub_a.seal(), &cclerk, &executor).expect("A submits");
    fire_submit(&app, &AuthRequired::Either, sub_b.seal(), &cclerk, &executor).expect("B submits");
    ...
    fire_reveal(&app, &AuthRequired::Either, field_from_u64(10), 30, &cclerk, &executor)
        .expect("A reveals");
    fire_reveal(&app, &AuthRequired::Either, field_from_u64(11), 50, &cclerk, &executor)
        .expect("B reveals");

    // CURATE (the CURATOR, None): feature B's piece.
    let r = fire_curate(&app, &AuthRequired::None, field_from_u64(11), sub_b.seal(), &cclerk, &executor)
        .expect("curate commits");
    assert_ne!(r.turn_hash, [0u8; 32], "a real verified curate turn");

    let s = executor.cell_state(cclerk.cell_id()).unwrap();
    assert_eq!(s.fields[PHASE_SLOT], field_from_u64(PHASE_CURATED), "CURATED");
...
fn register_deos_mounts_the_seeded_surface_into_the_context() {
    ...
    let s = executor.cell_state(cclerk.cell_id()).unwrap();
    assert_eq!(s.fields[submit_slot(0)], sub.seal(), "the sealed submission committed through the mounted surface");
}
