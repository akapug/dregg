...
use starbridge_gallery::{
    CURATOR_SLOT, FEATURED_SLOT, GALLERY_FACTORY_VK, PHASE_CURATED, PHASE_REVEAL, PHASE_SUBMISSION,
    PHASE_SLOT, SUBMIT_BASE, Submission, close_submissions_effects, curate_effects,
...
fn birth_gallery_cell(exec: &EmbeddedExecutor, cclerk: &AppCipherclerk, token_tag: &[u8]) -> CellId {
    ...
    exec.submit_turn(&birth).expect("gallery-cell birth commits");
...
fn seed_phase_submission(exec: &EmbeddedExecutor, cclerk: &AppCipherclerk, gallery: CellId) {
    ...
    exec.submit_action(cclerk, set_phase).expect("seed PHASE = SUBMISSION");
}
...
fn factory_born_gallery_runs_the_whole_call() {
    ...
    let has_program = exec.with_ledger_mut(|ledger| {
        ledger.get(&gallery).map(|c| !c.program.is_none()).unwrap_or(false)
    });
    ...
    exec.submit_action(
        &cclerk,
        cclerk.make_action(gallery, "submit", submit_effects(gallery, submit_slot(0), &sub_a.seal())),
    )
    .expect("first sealed submission must commit");
    exec.submit_action(
        &cclerk,
        cclerk.make_action(gallery, "submit", submit_effects(gallery, submit_slot(1), &sub_b.seal())),
    )
    ...
    assert_eq!(phase, field_from_u64(PHASE_SUBMISSION), "still in SUBMISSION");

    // Close submissions (SUBMISSION → REVEAL).
    exec.submit_action(&cclerk, cclerk.make_action(gallery, "close_submissions", close_submissions_effects(gallery)))
    ...
    exec.submit_action(&cclerk, cclerk.make_action(gallery, "curate", curate_effects(gallery, featured_id, &sub_b.seal())))
    ...
    assert_eq!(phase, field_from_u64(PHASE_CURATED), "the call must end CURATED");
...
fn factory_born_gallery_refuses_swapping_a_committed_submission() {
    ...
    exec.submit_action(
        &cclerk,
        cclerk.make_action(gallery, "submit", submit_effects(gallery, submit_slot(0), &sub.seal())),
    )
    ...
    let err = exec
        ...
        .expect_err("swapping a committed sealed submission must be refused — the anti-tamper tooth");
    ...
    let s0 = exec.with_ledger_mut(|ledger| ledger.get(&gallery).unwrap().state.fields[submit_slot(0)]);
    assert_eq!(s0, sub.seal(), "the refused swap committed nothing — the original seal stands");
}
...
fn factory_born_gallery_refuses_phase_rewind() {
    ...
    let phase = exec.with_ledger_mut(|ledger| ledger.get(&gallery).unwrap().state.fields[PHASE_SLOT]);
    ...
    exec.submit_action(&cclerk, cclerk.make_action(gallery, "curate", curate_effects(gallery, field_from_u64(11), &featured.seal())))
        .expect("the honest forward curate commits (REVEAL -> CURATED)");
    let phase = exec.with_ledger_mut(|ledger| ledger.get(&gallery).unwrap().state.fields[PHASE_SLOT]);
    assert_eq!(phase, field_from_u64(PHASE_CURATED), "the forward advance commits — the call CURATED");
