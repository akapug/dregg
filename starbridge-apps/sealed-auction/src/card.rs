...
//!     native `bind` closure makes), so the displayed phase advances when a fired turn
//!     commits;
//!   - one `button` per lifecycle method (`commit_bid` / `close_commit` / `reveal_bid`
//!     / `resolve`), each carrying its `onClick = { turn, arg }` — the EXACT cap-gated
...
///
/// A `vstack` of a header, a live `bind` on the lifecycle [`PHASE_SLOT`], and the four
/// lifecycle-advancing buttons. Renderer-independent DATA: hand it to any `deos-view`
/// renderer (native / web / discord) to paint the SAME card. The button `turn` names
...
pub fn auction_card_value() -> Value {
    json!({
        ...
        "children": [
            ...
            button("Commit Bid",   METHOD_COMMIT_BID,   0),
            button("Close Commit", METHOD_CLOSE_COMMIT, 0),
            button("Reveal Bid",   METHOD_REVEAL_BID,   0),
...
mod tests {
    ...
    fn the_card_is_a_vstack_with_a_header_a_phase_bind_and_four_buttons() {
        ...
        // header, bind, commit_bid, close_commit, reveal_bid, resolve
        assert_eq!(children.len(), 6);
        assert_eq!(children[0]["kind"], "text");
        assert_eq!(children[0]["props"]["text"], "Sealed Auction");
    }
