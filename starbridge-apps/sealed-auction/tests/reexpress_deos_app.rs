...
//!
//! The AUCTION's surface on the observer ⊂ bidder ⊂ auctioneer rights ladder
//! (`Signature ⊂ Either ⊂ None`):
//!   - `view_auction` — cap-only (an OBSERVER reads the board);
//!   - `commit_bid` / `close_commit` / `reveal_bid` / `resolve` — GATED (cap∧state):
//!     the cap-gate AND a live-state PHASE precondition, with the FULL auction program
//!     re-enforced by the executor on the fire (the seam — see `tests/deos_seam.rs`).
//!     The commit board lives ON-LEDGER: a commit writes a `WriteOnce` slot, so
//!     OVERWRITING a committed bid is an executor refusal (anti-front-running), not a
//!     `BTreeMap` membership check.
//!
...
use starbridge_sealed_auction::{auction_app, seed_auction};
...
// =============================================================================
// SMALLER: the whole surface is one builder (the SHIPPED `src::auction_app`).
// =============================================================================
...
fn the_whole_app_is_one_composed_registration() {
    ...
    // lifecycle ops (commit_bid / close_commit / reveal_bid / resolve) are GATED (cap∧state).
    assert_eq!(app.name(), "sealed-auction");
    assert_eq!(app.cells().len(), 1);
    let auction = &app.cells()[0];
    assert_eq!(
        auction.surface().all_names(),
        vec!["view_auction".to_string()],
        "the cap-only surface: just the board read (the lifecycle ops are gated)"
    );
    // The gated surface carries the four state-mutating, cap∧state lifecycle operations.
    let mut gated: Vec<String> = auction
        .gated_surface()
        .affordances
        .iter()
        ...
        .collect();
    gated.sort();
    // Lexical order: "resolve" < "reveal_bid" (the 's' < 'v' at index 2).
    assert_eq!(
        gated,
        vec![
            "close_commit".to_string(),
            "commit_bid".to_string(),
            "resolve".to_string(),
            "reveal_bid".to_string(),
        ]
    );
...
#[tokio::test]
async fn the_three_auction_roles_see_different_cap_only_surfaces() {
    use axum::body::Body;
...
#[tokio::test]
async fn an_observer_can_view_the_auction_a_real_turn() {
    use axum::body::Body;
    use axum::http::{Request, StatusCode};
    use tower::ServiceExt;
    ...
    async fn fire(router: &axum::Router, name: &str, tier: &str) -> StatusCode {
        router
            .clone()
            .oneshot(
                Request::post(format!("/auction/fire/{name}"))
                    .header(dregg_app_framework::HELD_RIGHTS_HEADER, tier)
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    // the floor), so any held tier clears the `view_auction` cap gate (not 403). The observer
    // read is a real EmitEvent turn through the executor.
    assert_ne!(
        fire(&router, "view_auction", "signature").await,
...
// =============================================================================
// MORE CAPABLE (3): web-of-cells — the AUCTION cell is a distributed sturdyref.
// =============================================================================

#[tokio::test]
async fn the_auction_is_published_into_the_web_of_cells() {
    let (cclerk, executor) = agent();
    // The SHIPPED surface, re-built with a captp server attached (the web-of-cells minter).
    // `auction_app` publishes the auction cell at the observer tier.
    let captp = CapTpServer::new(FederationId([0x5a; 32]));
    let base = auction_app(&cclerk, &executor);
    let app = DeosApp::builder("sealed-auction", cclerk.clone(), executor.clone())
        .web_of_cells(captp)
        .cell(base.cells()[0].clone())
        .build();

    // The AUCTION cell is exported as a real `dregg://` sturdyref — a bidder/auditor on
...
// =============================================================================
// MORE CAPABLE (4): rehydratable frustum-snapshot of the auction, per-viewer.
// =============================================================================

#[test]
fn an_auction_snapshot_rehydrates_per_viewer_respecting_the_lattice() {
    let (cclerk, executor) = agent();
    let app = auction_app(&cclerk, &executor);
    let auction = &app.cells()[0];

    // Snapshot the auction; it witnessed a commit turn, sources gone (a cold snapshot handed
    ...
    // An OBSERVER (Signature) rehydrating reacquires only `view_auction` (the cap-only
    // surface at its tier) — the snapshot respects the lattice.
    let observer = auction.rehydrate(&snap, AuthRequired::Signature).unwrap();
    assert_eq!(observer.visible_names(), vec!["view_auction".to_string()]);
    ...
    let blocked = auction.rehydrate(&snap, AuthRequired::Custom { vk_hash: [7u8; 32] });
...
async fn the_app_ships_a_web_component_surface_and_a_manifest() {
    ...
    assert!(js.contains("fireEndpoint: \"/auction/fire/view_auction\","));
