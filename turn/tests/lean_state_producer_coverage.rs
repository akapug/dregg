...
#[test]
fn seal_falls_back_factory_dissolved() {
    if skip_no_lean() {
        return;
    }
    // FACTORY-DISSOLVED (F3): the seal/swiss/sturdyref verb family (CreateSealPair/Seal/Unseal/
    // ExportSturdyRef/EnlivenRef/DropRef/ValidateHandoff) died with the kernel sealedBoxes/swiss
    // side-tables — a STORED CAPABILITY is a caps-in-slots factory value
    // (Dregg2/Apps/CapSlotFactory.lean), and every load+exercise is gated on R7 retrieval-epoch
    // freshness (stored_cap_only_fresh_if_epoch_unrevoked + no_forge_from_storage; the runtime
    // gate is apply.rs's stored_epoch re-check). The verified kernel no longer parses the seal
    // wire actions, so a turn carrying one refuses LOUDLY at the wire (malformed-wire sentinel)
    // and falls back to the Rust executor during the transitional window — never a silent state
    // install. The family exits every producer list.
    let (pre, a_id, b_id) = two_open_cells();
    ...
    let turn = single_effect_turn(
        ...
        a_id,
        0,
        Effect::Seal { pair_id: [0x55u8; 32], capability: cap },
    );
    // drive the LEAN wire directly (the Rust executor independently gates on the held sealer
    // cap, which is beside the point here — the pin is the VERIFIED KERNEL's wire refusal):
    let host = ShadowHostCtx::diag();
    match execute_via_lean(&turn, &pre, &host) {
        Ok(_) => panic!(
            "Seal unexpectedly crossed the wire — factory dissolution means the verified kernel \
             must not parse it"
        ),
        Err(e) => {
            // the refusal is LOUD at the marshallability gate (no wire arm — Lean-ineligible ⇒
            // the producer falls back to the Rust executor for the transitional window) or, for
            // a hand-crafted wire form, at the verified kernel's parser (the F3 arms are gone).
            let why = e.to_string();
            assert!(
                why.contains("not fully marshallable")
                    || why.contains("malformed-wire")
                    || why.contains("decode failed"),
                "Seal must refuse loudly (factory-dissolved), got: {why}"
            );
        }
    }
    assert!(!lean_shadow::producer_mappable_effects().contains(&"Seal"));
    assert!(!lean_shadow::producer_root_agreeing_effects().contains(&"Seal"));
    assert!(!lean_shadow::producer_root_gap_effects().contains(&"Seal"));
}

#[test]
fn export_sturdy_ref_falls_back_factory_dissolved() {
    if skip_no_lean() {
        return;
    }
    // FACTORY-DISSOLVED (F3), the sturdyref representative: a sturdy ref is a stored-cap slot
    // (the swiss-number unguessability is the crypto portal, orthogonal to the R7 freshness
    // gate). The verified kernel refuses the wire form loudly.
    let (pre, a_id, b_id) = two_open_cells();
    let turn = single_effect_turn(
        a_id,
        a_id,
    ...
    match execute_via_lean(&turn, &pre, &host) {
        Ok(_) => panic!(
            "ExportSturdyRef unexpectedly crossed the wire — factory dissolution means the \
             verified kernel must not parse it"
        ),
        Err(e) => {
            // the refusal is LOUD at the marshallability gate (no wire arm — Lean-ineligible ⇒
            ...
            assert!(
                why.contains("not fully marshallable")
                    || why.contains("malformed-wire")
                    || why.contains("decode failed"),
                "ExportSturdyRef must refuse loudly (factory-dissolved), got: {why}"
            );
        }
    }
    assert!(!lean_shadow::producer_mappable_effects().contains(&"ExportSturdyRef"));
    assert!(!lean_shadow::producer_root_agreeing_effects().contains(&"ExportSturdyRef"));
    assert!(!lean_shadow::producer_root_gap_effects().contains(&"ExportSturdyRef"));
}

#[test]
fn drop_ref_falls_back_factory_dissolved() {
    if skip_no_lean() {
        return;
    }
    // FACTORY-DISSOLVED (F3), the GC representative: dropping a stored ref is a cell-program slot
    // write (the slot owner forgets the value) — no kernel verb. The verified kernel refuses the
    // wire form loudly.
    let (pre, a_id, _b_id) = two_open_cells();
    let turn = single_effect_turn(a_id, a_id, 0, Effect::DropRef { ref_id: [0x07u8; 32] });
    let host = ShadowHostCtx::diag();
    match execute_via_lean(&turn, &pre, &host) {
        Ok(_) => panic!(
            "DropRef unexpectedly crossed the wire — factory dissolution means the verified \
             kernel must not parse it"
        ),
        Err(e) => {
            // the refusal is LOUD at the marshallability gate (no wire arm — Lean-ineligible ⇒
            ...
            assert!(
                why.contains("not fully marshallable")
                    || why.contains("malformed-wire")
                    || why.contains("decode failed"),
                "DropRef must refuse loudly (factory-dissolved), got: {why}"
            );
        }
    }
    assert!(!lean_shadow::producer_mappable_effects().contains(&"DropRef"));
    assert!(!lean_shadow::producer_root_agreeing_effects().contains(&"DropRef"));
    assert!(!lean_shadow::producer_root_gap_effects().contains(&"DropRef"));
}
