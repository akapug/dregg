...
## Done-log
- ✅ **CONFORMANCE DONE + verified** (`9bb0c1c4e`): 13-file both-wired spine meets ArkLib guidelines — 13/13 Apache headers (Authors: Ember Arlynx) + module docstrings (self-contained, internal refs stripped) + ## References + citation keys; `def Inv`→`IsEncoding` (shadowed Mathlib Inv); ArkLib lint-style.py CLEAN (was 13 ERR_COP + 13 ERR_MOD + 77 ERR_LIN); char-aware line-length clean (the awk-141 was unicode byte-counting; real = 1 line in the DROPPED KzgQDlogVacuity). 6 BibTeX (BB04/Sho97/Mau05/FKL18/Sch80/Zip79) staged in the PR-target references.bib. Fresh green (8930 jobs) + raw #print axioms clean on all 6 headline theorems. → PR-branch-rebuild (conformed + GgmShoupEmbed + BibTeX + both-wired README) + issue body. Also: ADOPT-ARKLIB-VCVIO-IDEAS.md (94f92b621) — don't-vendor (toolchain v4.31 vs our v4.30 + loom2 unmerged-PR); port deltas onto RomOracle; we're ahead on FRI-arithmetic/hygiene/QROM (natural upstream).
- ⚑⚑ **BOTH MODELS WIRED** — Shoup Tier-2 DONE (`f8fe5ef98`, GgmShoupEmbed.lean): `embedShoup` → ArkLib's REAL Groups.tSdhAdversary + `shoup_tSdh_ggm_sound` (byte-identical RHS to the Maurer capstone), sorry-free/axiom-clean (fresh pinned-v4.31.0, raw #print axioms cited). Faithfulness crux `groupEqPattern_eq` PROVEN (real equality matrix = symbolic eqPattern off the bad event, via injectivity — not assumed). Sufficient-test PASSED (8 checks incl embedShoup_noncollapsing + real <1). So BOTH standard GGM models now bound ArkLib's actual tSdhExperiment. → post-Shoup sweep: two-pager both-wired (=blueprint) + conformance (ArkLib guidelines) + issue package.
