        ...
        match cli.cmd {
            ...
            Cmd::Invites => {
                ...
                for r in invites {
                    println!(
                        "  {} ({}) · {} members",
                        r.display_name, r.room_id, r.joined_members
                    );
                }
            }
            ...
            Cmd::Encryption => {
                ...
                println!(
                    "device id:      {}",
                    client.device_id().unwrap_or_else(|| "(none)".into())
                );
                println!(
                    "key backup:     {}",
                    if client.backup_enabled().await {
                        "enabled"
                    } else {
                        "DISABLED — run recovery setup"
                    }
                );
            }
            Cmd::Rooms => {
