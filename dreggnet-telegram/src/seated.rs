//! # `SeatedTug` — the seat-claiming adapter that makes multiway-tug PLAYABLE BY TELEGRAM USERS.
//!
...
//! (the bot's `TelegramCipherclerk` pubkey) — never one of those strings — so a Telegram user could
...
//! so the hidden-hand fog reaches the right chat: your cards, their commitment.
//!
