...
//! - **Sealing**: Fresh ephemeral X25519 keypair. DH(ephemeral_secret, sealer_public) -> shared secret -> ChaCha20-Poly1305.
//! - **Unsealing**: DH(unsealer_secret, ephemeral_public) -> same shared secret -> decrypt.
...
impl SealPair {
    ...
    pub fn seal(&self, cap: &CapabilityRef) -> SealedBox {
        ...
        let enc_key = Self::derive_encryption_key(shared.as_bytes());
    ...
    fn derive_encryption_key(shared_secret: &[u8; 32]) -> [u8; 32] {
        let mut hasher = blake3::Hasher::new_derive_key("pyana-seal encryption v2");
