...
| **G4** | ~~umem / WIDE effect-family weld~~ **CLOSED** (`da0c47dd6`/`443661298`): the welded form is the deployed DEFAULT — the producer arms it in both constructors, the executor drops the bare wide member whenever a welded twin exists (fail-closed); bare survives only for the by-design carve-outs + multi-cohort chain legs | closed | done — the registry-default flip fired |
| **G5** | Capacity satisfaction (17/18/19) — **SOUND-AS-SHAPED at HEAD**: row-locality FIXED (`fec691cf5`, first-row force + caveat-uniformity), selector forcing DONE + strengthened (`f68868022`/`cfb66c37f`, deployed-bound columns discharge the hcommitLimb hypothesis), 18/19 inequality gadgets BUILT w/ teeth (`6ba95f033` — DUE_BITS range + wrap-close; overflow-safe product-compare + IS-NONZERO positivity). Escrow(17) staged descriptor emitted + real-STARK 6/6 (`gentian_carrier_floor_prove`) | would-be-yes if declared; dead-by-default | remaining = the registry flag-day: 18/19 descriptor-emit + producer aux-fill + prove exercise (mechanical; escrow emit-ready) |
