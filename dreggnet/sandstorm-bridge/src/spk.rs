...
/// `xz_decompress` is an OOM bomb (red-team #H-2): a few-KB stream of zeros expands
...
mod tests {
    ...
    /// **Red-team #H-2 PoC (xz bomb refused).** The defense bounds the **decompressed
    /// output size** during the xz stream, *before* the signature check — so a package
    ...
    /// **Red-team #H-2 PoC (capnp over-count bomb refused).** A crafted `Archive`
