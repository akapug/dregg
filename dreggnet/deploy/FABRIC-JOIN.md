...
The message below was posted to the builders.dev `general` chat (the spec an
operator asked for). It is kept here so it is durable and re-shareable. The reusable
...
Onboarding a new operator's homelab into the dreggnet fabric:
It's early and ARPA-style: if it falls over, we learn. The more *independent*
operators run nodes, the realer the consensus gets — that's the whole point.
Repo: https://github.com/emberian/dregg
...
- **Consensus node** — a dregg federation node (the `dregg-node` image / a native
  ...
  who controls a few. An independent 5-node operator mesh is exactly the shape of
  quorum we're after. A node is *modest* on resources (a couple cores, a few GB RAM, a little
  disk for state) — it gossips and votes, it doesn't do the heavy lifting.

- **Compute backend** — the polyana exec+prover agent (the `node-agent`
  pattern). It serves `:8021/fulfill` on the overlay and runs durable, metered
  polyana workloads (wasm/python tiers) + STARK turn proving. This one *scales*
...
- Compute backend: run the agent as a systemd service (the `node-agent.service`
...
backends (the polyana exec/prover :8021 agent), and subsidized early-era
compute/hosting for agents. If an operator points a few homelab boxes at the overlay
and tells us which roles they want them in, we'll wire dispatch to them and go from there.

Honest current state: the fabric is the AWS **edge** (stable public IP, TLS,
headscale control, DERP relay) + **node-a** (the home compute node, the
primary compute) coming online. We need ≥5 nodes for consensus that means
something — independent operator homelabs are how we get there. ٩( ᐛ )و
...
- The node-a deployment (the concrete operate pattern): `deploy/COMPUTE-BACKEND.md`
- The agent itself + systemd unit: `deploy/node-agent/` (`node-agent.service`)

## Fabric facts (as of 2026-06-28)

- **headscale control:** `https://headscale.dreggnet.example.com` (LIVE, real
  Let's Encrypt cert; `/health` → `{"status":"pass"}`).
- **edge:** AWS `<EDGE_HOST>` — Caddy (public TLS) + gateway + control + postgres
