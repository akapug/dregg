//! `persvati-agent` — the DreggNet compute-backend bridge agent.
//!
...
//! This binary is what answers that POST on persvati — it runs the lease as a
...
async fn main() -> std::io::Result<()> {
    let bind = std::env::var("PERSVATI_AGENT_BIND")
        .unwrap_or_else(|_| format!("0.0.0.0:{DEFAULT_AGENT_PORT}"));
    let listener = TcpListener::bind(&bind).await?;
    eprintln!("persvati-agent: bridge agent listening on http://{bind}");
    eprintln!("  GET  /health");
    ...
    loop {
        ...
        let (stream, peer) = match listener.accept().await {
            Ok(pair) => pair,
            Err(e) => {
                eprintln!("persvati-agent: accept error (continuing to serve): {e}");
        ...
        tokio::spawn(async move {
            if let Err(e) = serve_connection(stream, &counter).await {
                eprintln!("persvati-agent: connection from {peer} errored: {e}");
            }
        });
    }
}
