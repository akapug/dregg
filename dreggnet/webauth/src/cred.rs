...
//! than depended-on so the DreggNet workspace builds **offline** and pulls **no
//! AGPL dregg git** into its default closure (the same licensing discipline the
//! `dreggnet-bridge` crate keeps for its dregg-verify lane). The construction is
...
impl Credential {
    ...
    /// credential issued by `root`, and the presenter holds its bearer tail key"
    /// independent of any per-surface context.
    ///
    /// The **login gate** rides this: at login the target surface (and thus its
    /// required capability) is not yet known, so login proves genuine issuance +
    /// possession here, and the per-surface caveat meet is then enforced on every
    /// `/auth` by [`Credential::verify`]. Fail-closed: any chain/PoP defect is a
    /// [`Refusal`].
    pub fn verify_chain(&self, root: &PublicKey) -> Result<(), Refusal> {
        let last = self.blocks.last().expect("a credential has a root block");
    ...
    /// [`Credential::attenuate`] would sign the next block under, and the key a
    /// holder proves possession of by signing a login challenge
    /// ([`Credential::sign_challenge`] / [`verify_pop`]). Equal to the last
    /// block's `next_pub` (checked on decode), so it commits the whole chain.
    pub fn proof_public(&self) -> [u8; 32] {
        self.proof.verifying_key().to_bytes()
    }

    /// Sign an opaque login challenge with the bearer tail key — the client side
    /// of the proof-of-possession handshake. The verifier checks it with
    /// [`verify_pop`] against [`Credential::proof_public`].
    pub fn sign_challenge(&self, msg: &[u8]) -> [u8; 64] {
        self.proof.sign(msg).to_bytes()
    }

    /// Is this credential expired at wall-clock `now`? True iff any first-party
    /// `NotAfter { at }` (or a `Within` upper bound) it carries is already past.
    /// Used at `/auth` to distinguish an EXPIRED-but-genuine session (→ 401,
    /// re-login) from a genuine session that merely lacks the surface's cap
    /// (→ 403).
...
/// `pubkey` is the presented credential's [`Credential::proof_public`], `msg` is
/// the domain-tagged login challenge. Fail-closed: a malformed key/sig is `false`.
