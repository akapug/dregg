//! **Role → permission → cap.** The bridge that makes the cap-attenuation
//! lattice legible as orgs + roles.
//!
//! The org is its own dregg cell: it has its own minting authority (a
//! [`RootKey`](dreggnet_webauth::cred::RootKey), held by [`crate::OrgAuthority`]).
...
//! semantics of the caveat chain, the attenuated cap admits a request iff the
//! request satisfies *both* the owner grant (any perm, this org) *and* the role
//! caveat (a perm in the role) — i.e. exactly the role's permissions over exactly
//! this org. And because the webauth `attenuate` has no removal API and the
//! signature chain is unforgeable, appending a caveat can only ever *narrow*:
//! a viewer who tries to append a `write` caveat to self-promote only makes the
//! meet `read AND write` = unsatisfiable, never `write`. That no-amplify property
//! IS the security of roles — proven over the same `attenuate` webauth proves.

use dreggnet_webauth::cred::{Caveat, Context, Credential, Pred, PublicKey, Refusal, RootKey};

use crate::role::{ORG_KEY, PERM_KEY, Permission, Role};

/// Derive the org's stable id from its minting authority's public key. The org
/// is a cell; its id is `org:<first 16 hex of the org root pubkey>` — the same
/// shape as a webauth subject (`dregg:<16 hex>`), but for an org-account.
...
/// is one of `perms`. The single building block of both the owner grant and
/// every role attenuation.
...
/// Attenuate an owner grant down to a role's permissions (and optionally a
/// tighter expiry). The appended `AnyOf(role perms)` caveat can only *narrow*
/// reach — this is where "the role IS an attenuation of the owner's authority"
/// is literal.
...
/// adds/accepts them: their portable, offline-verifiable authority over the
/// org's resources, narrowed to their role.
...
/// The verification context for an action requiring `perm` over `org_id` at
/// clock `now`. Binds the two teeth: `org` (scoping) and `perm` (role-gating).
...
/// Authorize a role-cap to perform `perm` over `org_id` at clock `now`,
/// verifying it against the org's root public key — fully offline, fail-closed.
/// `Ok(())` iff the cap was minted by THIS org's authority AND its role grants
/// `perm`. A viewer-cap for `ResourceWrite`, or any org's cap against another
/// org's id/key, is a [`Refusal`].
pub fn authorize(
    cap: &Credential,
