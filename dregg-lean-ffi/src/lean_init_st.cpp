...
/* dregg_ffi_init_st — the single-threaded, libuv-thread-free init.
 *
 * Returns 0 on success, 1 if a module initializer reported an IO error. Drop-in
 * equivalent to `dregg_ffi_init` EXCEPT it does not start the libuv event-loop
 * thread (and thus runs the whole runtime on the caller's thread alone). Idempotency
 * is the CALLER's responsibility (the Rust side guards it behind a OnceLock), exactly
 * as for `dregg_ffi_init`. */
extern "C" int dregg_ffi_init_st(void) {
    /* The libuv-free prefix of lean_initialize_runtime_module(), in order. */
