...
pub struct PgMirrorConfig {
    ...
    /// to. The PRIMARY target is pg17 (`docs/PG-DREGG.md` §5).
    pub url: String,
}
...
impl NodeMirror {
    ...
    /// - with `pg-mirror-live`: a live pg17 [`pg_live::PgSink`] (a follow-up
    ///   wires the async connect; for now the live feature path still routes
    ///   through this same construction).
    ...
    pub fn from_env(head: [u8; 32], next_ordinal: u64) -> Option<NodeMirror> {
        let _cfg = PgMirrorConfig::from_env()?;
        let sink: Box<dyn MirrorSink> = Box::new(LoggingSink::default());
...
pub mod pg_live {
    //! The live pg17 sink: writes each [`MirrorBatch`] through the kernel writer
    ...
    impl PgSink {
        /// Connect to the configured postgres (pg17). The connection task is
        ...
        async fn write_batch(&self, batch: &MirrorBatch) -> Result<(), String> {
            ...
            for c in &batch.cells {
                // pg17 MERGE upsert (pg-dregg's dregg.merge_cell), one atomic stmt.
                self.client
                    .execute(
                        "SELECT dregg.merge_cell($1,$2,$3,$4,$5,$6::jsonb,$7,$8,$9)",
                        &[
                            &c.cell_id.as_slice(),
                            &c.mode,
                            &c.balance,
    ...
    impl MirrorSink for PgSink {
        fn emit(&mut self, batch: &MirrorBatch) -> Result<(), String> {
            ...
            tokio::task::block_in_place(|| {
                self.rt.clone().block_on(self.write_batch(batch))
            })
        }
    }
}
