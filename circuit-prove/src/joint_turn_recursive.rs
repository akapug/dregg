//! In-circuit BINDING NODES for the deployed recursion tree.
//!
//! Each `prove_*_binding_node*` below aggregates TWO already-wrapped batch-STARK leaves —
//! an effect-vm leg leaf that RE-EXPOSES claimed PI lanes, and a sub-proof leaf that exposes
//! its GENUINE in-circuit-computed counterpart — and `connect`s the two claims lane-by-lane
//! inside the combine, re-exposing the chain SEGMENT so the node folds into
//! [`crate::ivc_turn_chain::aggregate_tree`] like any per-turn segment leaf. A leg whose
//! claim no verifying sub-proof backs has no satisfying combine: the aggregate does not
//! prove, and a light client never receives a verifying artifact.
//!
//! Deployed wiring: [`crate::ivc_turn_chain`]'s `prove_chain_core_rotated` mints these nodes
//! for custom (`prove_custom_binding_node_state_segmented`, the deployed default), app-root
//! weld, claim, bridge, and sovereign turns; the deployed teeth live in
//! `circuit-prove/tests/{custom,bridge,sovereign,...}_binding_deployed_tooth.rs`.
//!
//! HISTORY (E9, 2026-07-18): this module previously also hosted the joint-turn recursive
//! fold (`prove_joint_turn_recursive*` / `verify_joint_turn_recursive` over a
//! `JointTurnAggregationAir` binding leaf). That layer was DELETED as dead scaffolding: the
//! binding leaf was a hand-written Rust AIR whose `bundle_digest` was a FREE WITNESS in-AIR
...
/// Direct-IR2 app-root binding node with the additional exact program-VK8 tooth.
/// The leg exposes `segment || commitment8 || field_K || program_vk8`; the direct
/// leaf exposes `commitment8 || old8 || new8 || R || canonical_vk8`. All eight VK
/// lanes are connected in-circuit before the segment is re-exposed.
...
// ============================================================================
//
// (The 3-cell joint-fold integration test `joint_turn_recursive_rotated.rs` was deleted with
// the joint-turn fold — E9, see the module header. The fold-wire teeth below and the
// `*_binding_deployed_tooth.rs` integration tests are the live coverage of this module.)
