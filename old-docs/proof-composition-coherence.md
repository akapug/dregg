...
The TurnExecutor already enforces all pyana rules (conservation, capability checks, effect application). Proving it inside SP1 means one circuit handles arbitrary effects -- SetField, GrantCapability, Transfer, Revoke, anything future. Specialized AIRs (Option A) require a new circuit per effect type and a composition layer. Kimchi native (Option C) gives small proofs but requires reimplementing every rule as gates, doubling the audit surface.

SP1 proving the executor is the canonical inner proof. It says: "I ran the real pyana state machine on these inputs and got these outputs." General-purpose, auditable, one implementation.

## 2. The VK Bridging Problem

A destination chain verifying an SP1 Groth16 proof needs to know the verification key corresponds to "the pyana executor program." This is solved by:

1. **Program commitment**: SP1 produces a program VK (hash of the ELF). This is a fixed constant for a given pyana release.
2. **Registry contract**: An on-chain registry maps `program_vk_hash -> "pyana-executor-v{N}"`. Governed by pyana's multisig or DAO. Updated on protocol upgrades.
3. **The verifier checks**: `proof.vk_hash == registry.current_pyana_vk()`.

The destination doesn't need to understand pyana's rules. It trusts that SP1 soundly executed a program whose VK is registered. The VK commitment IS the trust anchor. This is the same model as Succinct's SP1 verifier gateway.
...
Key insight: the wrapping is done ONCE per destination family, not per effect type. Adding a new effect to pyana requires zero circuit changes to the bridge -- only the SP1 guest program (which is just Rust) gets updated.
...
The architecture is: **one program, one proof, many wrappers.** The program is the TurnExecutor. The proof is SP1's STARK. The wrappers are mechanical translations to destination-specific formats. Adding a new destination chain means writing one new wrapper, not reimplementing pyana's logic.
