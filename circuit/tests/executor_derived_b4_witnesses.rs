...
fn check_honest_proves_and_forged_rejects(
    ...
    assert_eq!(
        honest.len(),
        expected_width,
        "{label}: honest witness width"
    );
    assert_eq!(
        forged.len(),
        expected_width,
        "{label}: forged witness width"
    );

    // The honest executor-derived witness MUST prove + verify.
    let proof = prove_and_verify_descriptor(&desc, honest).unwrap_or_else(|e| {
        panic!("{label}: honest executor-derived witness must prove+verify: {e}")
    });
...
fn b4_executor_derived_queue_allocate() {
    check_honest_proves_and_forged_rejects(
        QUEUEALLOCATE_DESCRIPTOR_JSON,
        72,
        4,
        &QUEUEALLOCATE_HONEST,
        &QUEUEALLOCATE_FORGED,
        68,
        69,
        "queueAllocateA",
    );
...
fn b4_executor_derived_queue_atomic_tx() {
    check_honest_proves_and_forged_rejects(
        QUEUEATOMICTX_DESCRIPTOR_JSON,
        76,
        6,
        &QUEUEATOMICTX_HONEST,
        &QUEUEATOMICTX_FORGED,
        68,
        69,
        "queueAtomicTxA",
    );
...
fn b4_executor_derived_pipelined_send() {
    check_honest_proves_and_forged_rejects(
        PIPELINEDSEND_DESCRIPTOR_JSON,
        74,
        5,
        &PIPELINEDSEND_HONEST,
        &PIPELINEDSEND_FORGED,
        68,
        69,
        "pipelinedSendA",
    );
