...
impl StarkAir for Poseidon2Air {
    ...
    fn eval_constraints(
        ...
        let in_len = WIDTH.min(local.len());
        input_state[..in_len].copy_from_slice(&local[..in_len]);
        let mut claimed_output = [BabyBear::ZERO; WIDTH];
        let out_len = WIDTH.min(local.len().saturating_sub(WIDTH));
        claimed_output[..out_len].copy_from_slice(&local[WIDTH..WIDTH + out_len]);
