...
pub fn verify_root_transition(witness: &FoldWitness) -> Option<BabyBear> {
    ...
    Some(compute_root_transition_hash(witness.old_root, witness.new_root, &fact_hashes))
}

pub struct FoldAir { pub witness: FoldWitness }

impl FoldAir {
    pub fn new(witness: FoldWitness) -> Self { Self { witness } }
}

impl Air for FoldAir {
    fn trace_width(&self) -> usize { FOLD_AIR_WIDTH }
    fn num_public_inputs(&self) -> usize { 5 }

    fn constraints(&self) -> Vec<Constraint> {
        vec![
            Constraint { name: "row_type_binary".into(), eval: Box::new(|row, _, _| { let rt = row[col::ROW_TYPE]; rt * (rt - BabyBear::ONE) }) },
            Constraint { name: "membership_root_matches_old_root".into(), eval: Box::new(|row, _, _| { let is_removal = BabyBear::ONE - row[col::ROW_TYPE]; is_removal * (row[col::MEMBERSHIP_ROOT] - row[col::OLD_ROOT]) }) },
            Constraint { name: "hash_valid_binary".into(), eval: Box::new(|row, _, _| { let hv = row[col::HASH_VALID]; hv * (hv - BabyBear::ONE) }) },
            Constraint { name: "removal_hash_required".into(), eval: Box::new(|row, _, _| { let is_removal = BabyBear::ONE - row[col::ROW_TYPE]; is_removal * (BabyBear::ONE - row[col::HASH_VALID]) }) },
            Constraint { name: "fact_hash_correct".into(), eval: Box::new(|row, _, _| { let is_removal = BabyBear::ONE - row[col::ROW_TYPE]; let expected = hash_fact(row[col::FACT_PRED], &[row[col::FACT_TERM_START], row[col::FACT_TERM_START+1], row[col::FACT_TERM_START+2]]); is_removal * (row[col::FACT_HASH] - expected) }) },
            Constraint { name: "old_root_consistent".into(), eval: Box::new(|row, _, pi| row[col::OLD_ROOT] - pi[0]) },
            Constraint { name: "new_root_consistent".into(), eval: Box::new(|row, _, pi| row[col::NEW_ROOT] - pi[1]) },
            Constraint { name: "removal_count_increment".into(), eval: Box::new(|row, next_row, _| { if let Some(next) = next_row { let is_removal = BabyBear::ONE - row[col::ROW_TYPE]; let is_next_removal = BabyBear::ONE - next[col::ROW_TYPE]; is_removal * is_next_removal * (next[col::REMOVAL_COUNT] - row[col::REMOVAL_COUNT] - BabyBear::ONE) } else { BabyBear::ZERO } }) },
            Constraint { name: "delta_nonempty".into(), eval: Box::new(|row, _, _| { let is_summary = row[col::ROW_TYPE]; let total = row[col::REMOVAL_COUNT] + row[col::CHECK_COUNT]; if is_summary == BabyBear::ONE && total == BabyBear::ZERO { BabyBear::ONE } else { BabyBear::ZERO } }) },
            Constraint { name: "root_transition_binding".into(), eval: Box::new(|row, _, pi| { let is_summary = row[col::ROW_TYPE]; is_summary * (row[col::MEMBERSHIP_ROOT] - pi[4]) }) },
        ]
    }

    fn first_row_constraints(&self) -> Vec<Constraint> {
        vec![Constraint { name: "first_removal_count".into(), eval: Box::new(|row, _, _| { let is_removal = BabyBear::ONE - row[col::ROW_TYPE]; is_removal * (row[col::REMOVAL_COUNT] - BabyBear::ONE) }) }]
    }

    fn last_row_constraints(&self) -> Vec<Constraint> {
        vec![
            Constraint { name: "last_row_is_summary".into(), eval: Box::new(|row, _, _| row[col::ROW_TYPE] - BabyBear::ONE) },
            Constraint { name: "total_removals_match".into(), eval: Box::new(|row, _, pi| row[col::REMOVAL_COUNT] - pi[2]) },
            Constraint { name: "total_checks_match".into(), eval: Box::new(|row, _, pi| row[col::CHECK_COUNT] - pi[3]) },
        ]
    }
    ...
    fn generate_trace(&self) -> (Vec<Vec<BabyBear>>, Vec<BabyBear>) {
        ...
        let public_inputs = vec![w.old_root, w.new_root, BabyBear::new(w.removed_facts.len() as u32), BabyBear::new(w.num_added_checks as u32), expected_rt];
        (trace, public_inputs)
    }
}
...
pub fn build_shared_tree(leaves: &[BabyBear], depth: usize) -> (BabyBear, Vec<MerkleWitness>) {
    ...
    for &leaf in leaves.iter().take(max_leaves) { bottom.push(leaf); }
    while bottom.len() < max_leaves { bottom.push(BabyBear::ZERO); }
    levels.push(bottom);
    for _ in 0..depth {
        let prev = levels.last().unwrap();
        let mut next = Vec::with_capacity(prev.len() / fan_out);
        for chunk in prev.chunks(fan_out) { next.push(hash_4_to_1(&[chunk[0], chunk[1], chunk[2], chunk[3]])); }
        levels.push(next);
    }
    ...
    for (leaf_idx, &leaf_hash) in leaves.iter().enumerate() {
        ...
        for level in 0..depth {
            ...
            for j in 0..fan_out { if j as u8 != position { siblings.push(levels[level][group_start + j]); } }
            proof_levels.push(MerkleLevelWitness { position, siblings: [siblings[0], siblings[1], siblings[2]] });
            idx /= fan_out;
        }
        proofs.push(MerkleWitness { leaf_hash, levels: proof_levels, expected_root: root });
    }
    (root, proofs)
}
...
pub fn build_membership_proof(leaf_hash: BabyBear, depth: usize) -> MerkleWitness {
    ...
    MerkleWitness { leaf_hash, levels, expected_root: current }
}

pub fn create_test_fold(num_removals: usize, num_checks: usize) -> FoldWitness {
    let new_root = BabyBear::new(222222);
    if num_removals == 0 {
        return FoldWitness { old_root: BabyBear::new(111111), new_root, removed_facts: vec![], num_added_checks: num_checks };
    }
    let facts_data: Vec<(BabyBear, [BabyBear; 3])> = (0..num_removals).map(|i| {
        (BabyBear::new((i * 100 + 10) as u32), [BabyBear::new((i * 100 + 20) as u32), BabyBear::new((i * 100 + 30) as u32), BabyBear::ZERO])
    }).collect();
    let fact_hashes: Vec<BabyBear> = facts_data.iter().map(|(pred, terms)| hash_fact(*pred, terms)).collect();
    let (old_root, proofs) = build_shared_tree(&fact_hashes, 4);
    let removed_facts: Vec<RemovedFact> = facts_data.into_iter().zip(proofs.into_iter()).map(|((predicate, terms), proof)| {
        RemovedFact { predicate, terms, membership_proof: Some(proof) }
    }).collect();
    FoldWitness { old_root, new_root, removed_facts, num_added_checks: num_checks }
}
...
mod tests {
    ...
    fn fold_air_valid_single_removal() {
        ...
        let result = MockProver::verify(&air);
        assert!(result.is_valid(), "Fold AIR should verify: {:?}", result.violations());
    }
    ...
    fn fold_air_valid_checks_only() {
        let witness = FoldWitness { old_root: BabyBear::new(100), new_root: BabyBear::new(200), removed_facts: vec![], num_added_checks: 3 };
        let air = FoldAir::new(witness);
        let result = MockProver::verify(&air);
        assert!(result.is_valid(), "Fold AIR (checks only) should verify: {:?}", result.violations());
    }

    #[test]
    fn fold_air_empty_delta_fails() {
        let witness = FoldWitness { old_root: BabyBear::new(100), new_root: BabyBear::new(200), removed_facts: vec![], num_added_checks: 0 };
        let air = FoldAir::new(witness);
    ...
    fn fold_air_no_membership_proof_fails() {
        let witness = FoldWitness {
            old_root: BabyBear::new(111111), new_root: BabyBear::new(222222),
            removed_facts: vec![RemovedFact { predicate: BabyBear::new(10), terms: [BabyBear::new(20), BabyBear::new(30), BabyBear::ZERO], membership_proof: None }],
            num_added_checks: 0,
        };
        assert!(!MockProver::verify(&FoldAir::new(witness)).is_valid(), "Missing membership proof should fail");
    }
    ...
    fn fold_air_wrong_membership_root_fails() {
        ...
        let witness = FoldWitness {
            old_root: BabyBear::new(999999), new_root: BabyBear::new(222222),
            removed_facts: vec![RemovedFact { predicate, terms, membership_proof: Some(proof) }],
            num_added_checks: 0,
        };
        assert!(!MockProver::verify(&FoldAir::new(witness)).is_valid(), "Wrong root should fail");
    }
    ...
    fn fold_air_forged_membership_proof_fails() {
        ...
        let witness = FoldWitness {
            old_root: proof.expected_root, new_root: BabyBear::new(222222),
            removed_facts: vec![RemovedFact { predicate, terms, membership_proof: Some(proof) }],
            num_added_checks: 0,
        };
        assert!(!MockProver::verify(&FoldAir::new(witness)).is_valid(), "Forged proof should fail");
    }
}
