...
//!
//! **VENDORED, byte-identical, from the committed `dregg_app_framework::dreggverse_map`**
//! (`app-framework/src/dreggverse_map.rs`). It is a thin, pure navigation over the
//! REAL `starbridge_web_surface` witness-graph ([`Backlinks`]) and per-viewer
//! [`Membrane`] — both already direct dependencies of this crate — so vendoring it
//! here lets the cockpit render the genuine [`DreggverseMap::project_for`] WITHOUT
//! dragging `dregg-app-framework`'s heavy `tokio`/`axum`/`captp`/`storage` tree (and
//! the plonky3-recursion `[patch]` skew) into this standalone, headless,
//! `cargo test`-able workspace. The `app-framework` source named the cockpit panel as
//! its "follow-on (NOT built here)"; THIS lane builds it ([`crate::links_here`] is the
//! gpui-free model the cockpit's WHAT-LINKS-HERE tab renders, projecting THIS map
//! through the focused agent's membrane).
//!
...
//! graph, [`Membrane`] / [`SurfaceCapability`] for the projection (the SAME
//! `is_attenuation` lattice the web-of-cells browser and `rehydrate` use). No parallel
//! backlink index, no parallel cap model, no toy graph. The witness-graph IS the
//! docuverse; this is its navigation.
//!
//! ## Cockpit panel — BUILT (this lane)
//!
//! - **the WHAT-LINKS-HERE panel** ([`crate::links_here`] + the cockpit's
//!   `links_here_panel`) — for the cell currently in focus, it renders
//!   [`DreggverseMap::project_for`] through the focused agent's [`Membrane`]: the
