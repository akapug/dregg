...
/// **THE PULSE** — the 250 ms background beat that consumes the World's dynamics
/// stream so the desktop repaints when the World moves WITHOUT the desktop's own
/// hand (a bot reactor, an attached agent, a live node). Extracted from this file so
/// the reactivity work owns a file rather than a line-range; see its module doc for
/// the ⚠ on which of its four concerns a wake edge can and cannot replace.
pub mod pulse;
pub mod rewind;
pub mod spotter;
