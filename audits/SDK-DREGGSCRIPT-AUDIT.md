# SDK-Dreggscript Audit — Bottom-Up Gap Walk
...
the bottom-up walk that `dreggscript/README.md` §"Design discipline"
asks for: **imagine the runtime API dreggscript would compile to,
write each primitive as an ugly Rust method-chain, identify the SDK
gap.**

The discipline (`dreggscript/README.md`):
...
Reference shape from `dreggscript/README.md` §Q1:
...
method-chain that dreggscript would compile against. None of that
exists as a type. The rest of this document walks the chain primitive
by primitive and names the gaps.

## Method-chain walk

For each primitive, this section gives (a) the chain as dreggscript
...
let mut cell = dregg_cell::Cell::new_hosted(
    cclerk.public_key().0,
    /* token_id */ [0u8; 32],
);
...
- *Missing.* No SDK type named `Cell` representing "this cipherclerk's
  logical cell, with state and behaviour." `dregg_cell::Cell` is the
...
unresolved design question for dreggscript. Two viable paths:

1. **Compile closure to circuit.** `dregg-dsl-runtime::CellProgram`
   already targets a DSL; dreggscript's `with_behavior(closure)` could
...
2. **Run closure as host-side actor.** A new `dregg_runtime::Actor`
...
Path 1 is the one consistent with the rest of dregg's philosophy
...
*any* dreggscript work; it cannot be punted.
...
let live_ref: dregg_captp::LiveRef = cclerk
    .accept_capability("dregg://auctioneer/...")?;

// Build a PipelinedAction (this is `cell.send`'s payload shape today):
let action = dregg_captp::pipeline::PipelinedAction {
    method: dregg_captp::symbol("bid"),
    args: postcard::to_allocvec(&BidArgs { amount: 100 })?,
    authorization: dregg_captp::action::Authorization::Signature(..),
};
...
- *Missing.* No SDK helper "given a Rust message type that derives
  ...
  message and whose `method` is the variant name." Dreggscript needs
  this — every `cell.send(cap, msg)` call would emit it.
- *Missing.* No SDK type called `MsgKind` (this is dreggscript's
  ...
  identifier. **Closest reference:** `dregg_captp::symbol("bid")` is
...
- *Async vs sync.* `runtime.execute_turn` is **synchronous** (the
  ...
  execution. Dreggscript will need *both* — local
...
let restrictions = dregg_token::Attenuation {
...
let mut cap_set = dregg_cell::CapabilitySet::new();
...
- *Partial / fragmented* (SDK-REVIEW C-3, still open). Two parallel
  cap surfaces:
  - `dregg_sdk::HeldToken` / `dregg_token::Attenuation` (macaroon-bearer)
  - `dregg_cell::CapabilityRef` / `dregg_cell::AttenuatedCap` (cell-resident)
- *Missing.* No unified `Capability` type. Dreggscript can't have one
  signature for `cell.attenuate_cap(cap, narrower)` until the two
  notions converge — or until dreggscript explicitly compiles to two
...
// (2) dregg_cell::Cell::spawn_child for ledger-level child cells:
let parent_cell: dregg_cell::Cell = /* … */;
...
transparency. Dreggscript's job is to surface the factory pattern
...
side* of CapTP (`dregg_captp::CapTpServer::on_pipeline_message` in
...
- *Missing entirely.* The runtime has no concept of "a cell is the
  ...
  uses axum + `dregg_app_framework::*` — not the SDK).
...
These are the same decisions as gap 1 (`with_behavior`). Dreggscript
...
**Current closest (ugly):**
```rust
// CapInbox lives in dregg_storage, NOT in dregg_sdk:
let mut inbox = dregg_storage::CapInbox::new(
    QuotaId(1),
    /* capacity */ 100,
    /* min_deposit */ 50,
);
...
- *Missing from SDK.* `CapInbox` is in `dregg_storage` and not
  re-exported through `dregg_sdk`. Apps that want an inbox must
  `use dregg_storage::CapInbox` directly — same "apps step around
...
- *Missing.* `read_next` is synchronous; dreggscript's `inbox.read_next().await`
...
- *Inconsistency.* "Inbox" vs "queue" vs "CapTP promise inbox" are
  ...
  own, none unified. Dreggscript will need to pick one.
...
- *Missing.* `FactoryDescriptor` is in `dregg_cell`, not surfaced as a
...
- *Missing.* `FieldConstraint::WriteOnce` is not in the current enum —
  ...
  exists in `dregg_cell::program::StateConstraint`, but it's a
...
- *Remaining.* The dreggscript shape still wants:
...
## Prioritized gap list (for the next dreggscript work)
...
| 2 | `CapInbox` not re-exported from SDK | `pub use dregg_storage::CapInbox` in `sdk/src/lib.rs` | Trivial |
...
| # | Gap | Addition | Notes |
|---|---|---|---|
| 6 | No `FactoryDescriptorBuilder` in the SDK | `pub struct FactoryDescriptorBuilder { … }` with `.with_slot_caveat(..)`, `.with_initial_state(..)`, `.with_creation_budget(..)`, `.deploy(&cclerk) -> [u8; 32]` | Wraps `dregg_cell::FactoryDescriptor` |
...
| # | Gap | Notes |
|---|---|---|
| 11 | `with_behavior(closure)` — closure vs deployed circuit | Decides whether dreggscript's behaviour is provable (compile to circuit) or merely executable (host actor). The single biggest open question. |
...
| 14 | Async path for local execution | `AgentRuntime::execute_turn` is sync. Dreggscript's `.await` semantics need a thin async wrapper (mechanical) plus a coherent "when does this resolve" story (which depends on the answer to #11/#12). |
...
language commitments. After that work lands, dreggscript would
...
dreggscript. They cannot be deferred indefinitely: without an answer
...
are not yet composed into the layer dreggscript wants to sit on.**
...
Dreggscript is *not* blocked on language design — it is blocked on
