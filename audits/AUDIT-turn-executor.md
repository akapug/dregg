...
**Receipt-chain binding:** `TurnReceipt::receipt_hash()` (turn.rs:303-368) does include all the relevant fields, with version tag `dregg-receipt-v2`. But the receipt is never bound to the prior receipt at executor write time (P0-3); chain integrity is a verifier-side property only.
