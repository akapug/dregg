//! # `SeatedTug` — the seat-claiming adapter that makes multiway-tug PLAYABLE BY WECHAT USERS.
//!
...
//! (the bot's `WeChatCipherclerk` pubkey) — never one of those strings — so a WeChat user could
//! open and watch a tug round but every play would be refused as "actor holds no seat".
//!
...
//! so the hidden-hand fog reaches the right conversation: your cards, their commitment.
//!
//! This is a byte-for-byte peer of `dreggnet_web::seated` — the SAME adapter the web catalog wraps
//! `tug` in, so Telegram plays the identical seated tug the browser does.
//!
...
/// The multiway-tug offering with **frontend-claimable seats**.
