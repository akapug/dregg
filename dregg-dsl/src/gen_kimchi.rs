...
pub fn generate_kimchi(ir: &ConstraintIr) -> TokenStream {
    ...
    quote! {
        pub fn #fn_name() -> dregg_dsl_runtime::KimchiCircuitDescriptor {
            dregg_dsl_runtime::KimchiCircuitDescriptor {
        ...
        match stmt {
            Statement::Require(req) => match &req.kind {
                RequirementKind::LessEqual { .. } | RequirementKind::GreaterEqual { .. } => {
                    ...
                    gates.push(quote! {
                        dregg_dsl_runtime::KimchiGate {
                            typ: dregg_dsl_runtime::KimchiGateType::Generic,
                ...
                RequirementKind::Membership { .. } => {
                    ...
                    for _ in 0..depth {
                        gates.push(quote! {
                            dregg_dsl_runtime::KimchiGate {
                                typ: dregg_dsl_runtime::KimchiGateType::Poseidon,
                                coeffs: vec![],
                                wires: 12,
                            }
                        });
