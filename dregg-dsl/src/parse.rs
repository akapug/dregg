...
/// Parse a `#[dregg_caveat]` function — pure requirements, no mutations.
pub fn parse_caveat(func: &ItemFn) -> Result<ConstraintIr, syn::Error> {
    parse_constraint(func, false, None)
}

/// Parse a `#[dregg_effect]` function — may contain mutations.
pub fn parse_effect(
    func: &ItemFn,
...
fn parse_constraint(
    ...
    if statements.is_empty() {
        return Err(syn::Error::new(
            func.block.span(),
            "dregg constraint function must contain at least one require!() or mutation statement",
        ));
    }
    ...
    for arg in &func.sig.inputs {
        match arg {
            FnArg::Typed(pat_type) => {
                let ident = match pat_type.pat.as_ref() {
                    ...
                    _ => {
                        return Err(syn::Error::new(
                            pat_type.pat.span(),
                            "dregg constraint parameters must be simple identifiers",
                        ));
                    }
                };
            ...
            FnArg::Receiver(_) => {
                return Err(syn::Error::new(
                    arg.span(),
                    "dregg constraint functions cannot have self parameters",
                ));
            }
        }
    }
        ...
        Type::Reference(tr) => {
            if tr.mutability.is_some() {
                if !allow_mut {
                    return Err(syn::Error::new(
                        tr.span(),
                        "&mut parameters are only allowed in #[dregg_effect] functions",
                    ));
                }
...
fn parse_inner_type(ty: &Type) -> Result<ParamType, syn::Error> {
    match ty {
        ...
        _ => Err(syn::Error::new(
            ty.span(),
            "unsupported type in dregg constraint (supports u64, [u8; 32], Set<T>, &mut u64)",
        )),
    }
}
        ...
        match stmt {
            Stmt::Macro(sm) => {
                if is_require_macro(&sm.mac) {
                    ...
                    return Err(syn::Error::new(
                        sm.mac.path.span(),
                        "only require!() macros are supported in dregg constraint bodies",
                    ));
                }
            }
            Stmt::Expr(expr, _semi) => {
                if let Some(parsed) = try_parse_statement_expr(expr, is_effect)? {
                    ...
                    return Err(syn::Error::new(
                        expr.span(),
                        "unsupported expression in dregg constraint body",
                    ));
                }
            }
            Stmt::Local(_) => {
            ...
            _ => {
                return Err(syn::Error::new(
                    Span::call_site(),
                    "unsupported statement in dregg constraint body",
                ));
            }
        }
    }
