...
pub fn generate_rust_evaluator(ir: &ConstraintIr) -> TokenStream {
    ...
    quote! {
        pub fn #fn_name(#(#params),*) -> Result<(), dregg_dsl_runtime::ConstraintError> {
            #body
            Ok(())
        }
    }
    ...
    match stmt {
        Statement::Require(req) => match &req.kind {
            RequirementKind::LessEqual { left, right } => {
                quote! {
                    if !(#left <= #right) {
                        return Err(dregg_dsl_runtime::ConstraintError::CaveatViolation(#caveat_name));
                    }
                }
            }
            ...
            RequirementKind::BitRange { value, bits } => {
                ...
                quote! {
                    {
                        let __dregg_v: u64 = #value;
                        let __dregg_bound: u128 = 1u128 << #bits_lit;
                        if !((__dregg_v as u128) < __dregg_bound) {
                            return Err(dregg_dsl_runtime::ConstraintError::CaveatViolation(#caveat_name));
                        }
                    }
                }
            ...
            RequirementKind::MerkleAtPosition { .. } => {
                ...
                quote! {
                    // TODO(dregg-dsl/gen_rust.rs): implement runtime Merkle
            ...
            RequirementKind::Poseidon2Hash { .. } => {
                ...
                quote! {
                    // TODO(dregg-dsl/gen_rust.rs): implement runtime
...
/// Generate an effect descriptor function for `#[dregg_effect]` annotated functions.
pub fn generate_effect_descriptor(ir: &ConstraintIr) -> TokenStream {
    ...
    quote! {
        pub fn #fn_name() -> dregg_dsl_runtime::EffectDescriptor {
            dregg_dsl_runtime::EffectDescriptor {
                name: #effect_name,
