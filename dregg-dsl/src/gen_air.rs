...
pub fn generate_air_descriptor(ir: &ConstraintIr) -> TokenStream {
    ...
    quote! {
        pub fn #fn_name() -> dregg_dsl_runtime::AirConstraintSet {
            dregg_dsl_runtime::AirConstraintSet {
        ...
        match stmt {
                ...
                let c = match &req.kind {
                    RequirementKind::LessEqual { left, right } => {
                        ...
                        *aux_width += 2;
                        ...
                        quote! {
                            dregg_dsl_runtime::Constraint::RangeCheck {
                    ...
                    RequirementKind::Equal { left, right } => {
                        ...
                        let right_str = quote!(#right).to_string();
                        quote! {
                            dregg_dsl_runtime::Constraint::Equality {
                    ...
                    RequirementKind::NotEqual { left, right } => {
                        ...
                        *aux_width += 1;
                        ...
                        let right_str = quote!(#right).to_string();
                        quote! {
                            dregg_dsl_runtime::Constraint::NonEquality {
                                desc: concat!(#left_str, " != ", #right_str),
                                inverse_col: #inv_col,
                            }
                        }
                    ...
                    RequirementKind::Membership { set, element } => {
                        ...
                        *aux_width += depth * 2 + 1; // position bits + sibling hashes + root check
                        ...
                        quote! {
                            dregg_dsl_runtime::Constraint::MerkleMembership {
            ...
            Statement::Mutate(mutation) => {
                ...
                *aux_width += 2;
                ...
                out.push(quote! {
                    dregg_dsl_runtime::Constraint::Transition {
