...
`SDK-REVIEW.md` (the as-is SDK surface), `PYANA-FLAWS-FROM-APPS.md`
...
| `site/src/starbridge.html` (88 LoC) | Page chrome. Topbar (runtime picker, URI input, Go/Snapshot, cursor scrubber). Three-pane body: object tree, inspector, raw-JSON. Mounts `<pyana-app>` and bootstraps `_includes/studio/starbridge.js`. |
| `site/src/_includes/studio/starbridge.js` (411 LoC) | Page-specific orchestration. Loads `runtimes.js` registry, instantiates the active runtime, drives the cursor, mounts `<pyana-${kind}>` inspectors via URI dispatch. Exposes `window.__starbridge` for tests. |
| `site/src/_includes/studio/runtime-in-memory.js` | Owns the wasm handle from `/pkg/pyana_wasm.js`, exposes the `Runtime` contract. |
| `site/src/_includes/studio/{runtimes.js, context.js, inspectors.js, uri.js, ...}` | Inspector registry, URI parser, runtime kinds enumeration, `<pyana-app>` context provider. |
...
deferred). The inspectors registry exists; only `<pyana-cell>` is
wired end-to-end (per STUDIO.md §9, Phase 0).

### 1.2 The wasm in-browser node

`wasm/` builds a `pkg/pyana_wasm.js` ESM module that the site loads
...
- A real-but-in-memory pyana node, sitting in browser linear memory,
...
| App | Path today | What it does | Current primitives used | Pyana-native composition | Userspace-gap blockers |
|---|---|---|---|---|---|
| **governed-namespace** | `apps/governed-namespace/` | DAO-governed routing table + capability-secure file storage; propose/vote/amend routes via constitutional threshold; mount caps at paths; discover by tags; resolve to sturdy refs. | DFA route classifier, content-addressed storage, `pyana_storage::namespace_mount`, threshold voting (in-app); HTTP/JSON. *Some* real cell/turn use. | (a) DAO = sovereign cell whose state holds `(route_table_root, voter_root, proposal_root)`. (b) Each route is a `Capability` mounted on the namespace cell with `Caveat::PathPrefix` attenuation. (c) Proposals are sub-cells produced by a *governance factory* (`FactoryDescriptor` constrains: outcome can only be Accept/Reject; threshold = N). (d) Vote turns: `Effect::SetField(proposal.votes[i] = 1)` gated by a `Caveat::VoterRoll` capability. (e) Amendment: atomic turn `Effect::SetField(route_table_root = new_root)` requiring threshold-witness across N caveat-discharges. (f) Files = nameless content-addressed blob (`Effect::BindBlob`, G23) on the namespace cell. | Tier 1 #1 (transition constraint for monotone proposal-id counter); Tier 1 #3 (`AuthenticatedRequest<C>`); Tier 1 #4 (federation clock for proposal expiry); Tier 2 #7 (`pyana-credentials` for voter eligibility); Tier 3 #15 (`Effect::BindBlob`). |
...
Each sketch covers: pyana primitives composed, where Factories help,
what Starbridge UI surfaces are needed, demo vs. real version.

### 3.1 `nameservice` (recommended **first** build)

**Why first.** Already the cleanest pyana-native shape on paper (the
...
- `<pyana-name-registry uri="pyana://cell/{registry_id}">` — list all
  registered names, click-through to each name cell.
- `<pyana-name uri="pyana://cell/{name_cell_id}">` — show
...
- Rent uses the in-browser `PyanaRuntime` clock (manual `advance +1`).
...
into `pyana-credentials` (Tier 2 #7), then writing the Starbridge
...
- `<pyana-issuer uri="...">` — schema, issued count, revocation root.
- `<pyana-credential uri="...">` — attributes (selective disclosure
  preview), present-via-proof flow.
- `<pyana-presentation-builder>` — wizard: pick credential → pick
...
atomic route changes; file blobs via real `pyana-storage::BlobStore`.
...
- `<pyana-artwork uri="...">` — image (from blob), provenance chain
  walk, current owner.
- `<pyana-auction uri="...">` — phase indicator, time remaining,
  commitment count, reveal count, top-of-book commitment.
- `<pyana-bid-builder>` — commit / reveal flows.
- Live updates: subscribe to `pyana://cell/{auction_id}` for state
  changes (Tier 3 #11).
...
- `<pyana-bounty-board uri="...">` — list of open bounties, filter by
  qualification tier.
- `<pyana-bounty uri="...">` — reward, deadline, claim count
  (anonymous), delivery status.
- `<pyana-bounty-claim-builder>` — anonymous claim wizard
...
3. **Intent matcher selects the lowest-cost matching seller.**
   ...
   `pyana_intent::matcher::match_intent` runs the buyer's predicate
...
- `<pyana-job-board uri="...">` — list of open jobs, filter by
  predicates.
- `<pyana-job-builder>` — wizard: pick predicates (GPU count, RAM,
  uptime, deadline) → preview matching seller pool → post intent.
- `<pyana-seller-response>` — temporal-predicate proof visualizer
  (show the trace columns, the window endpoints, the threshold).
- `<pyana-delivery-proof>` — DSL circuit visualizer for the
  delivery proof.
- `<pyana-dispute>` — optimistic settlement state machine view.
...
storage-layer-ness comes from `pyana-storage::inbox::CapInbox` for
...
- Content delivery = `pyana-storage::inbox::CapInbox::receive_at` from creator → subscriber.
...
- `<pyana-subscription uri="...">` — tier, next debit height, cap
  status.
- `<pyana-creator-dashboard>` — active subscribers (anonymous
  commitments only), total recurring revenue.
- `<pyana-delegate-debit>` — UI to install the debit envelope.
...
- `apps/orderbook/src/blinded_bids.rs` — `FairDistributionEndpoint`
  pattern; this is approximately what `pyana-storage::BlindedQueue`
...
│   │   ├── proposal.js
│   │   ├── ...
│   │   └── index.js             # ← registers all via window.pyana.register
...
│       ├── ...
├── nameservice/
│   ├── Cargo.toml               # ← uses app-framework for HTTP (if needed) + pyana-sdk
│   ├── src/
...
- `pyana-turn` (for `Effect` enum, `TurnBuilder`).
...
`window.pyana.createFromFactory` can resolve `factory_vk` strings
...
target dir, all of pyana-sdk's compile artifacts cached.
...
The host (`pyana-node`, a back-end aggregator binary, or the wasm
...
This is what makes "starbridge-apps are pyana-native from day 0"
...
`<pyana-app>` context (per `site/STUDIO.md` §6) configured for that
...
the general-purpose debugger / explorer for *any* pyana URI. Each
...
2. **`identity`** (Week 2-4, parallel with #1's tail). Crypto
   ...
   `pyana-credentials` (Tier 2 #7). **Blocked on:** nothing
   gating — can proceed independently. *Output:* the
   `pyana-credentials` crate that the rest of the apps need.
...
1. **Workspace shape: A or B?** Single root workspace (Option A in
   ...
   isolates starbridge-apps' deps from the pyana core. Default: A
...
2. **In-browser-only vs. server-side hybrid.** Some apps (gallery
   ...
   **Question:** is the goal that *every* starbridge-app run with no
   per-app server (just `pyana-node` + browser), or do some get to
...
7. **Subscription as a starbridge-app.** I've included it (§3.8),
   ...
   *recurring-payment primitive*. Alternative: extract the
   recurring-debit pattern into `pyana-storage` as a building
...
9. **`pyana-credentials` crate scope.** Lifting `bridge::present`
   is Tier 2 #7. The natural home is a new crate `pyana-credentials`
...
12. **Naming.** "starbridge-apps" is fine as a directory name.
    The brand name for what users see — "Pyana Studio apps"? "Pyana
...
| 7 (Tier 2) | `pyana-credentials` crate + `Presented<P>` extractor | identity (provides it), bounty-board, privacy-voting, governed-namespace, gallery (gated auctions) |
