...
// =============================================================================
// Hybrid post-quantum session key-exchange (HNDL mitigation)
// =============================================================================

pub mod hybrid {
    //! A **hybrid** (classical + post-quantum) key-exchange for the CapTP
    //! transport, so a recorded session is not *harvest-now-decrypt-later*
    //! (HNDL) vulnerable: an adversary who records the handshake today and
    //! acquires a quantum computer later still cannot recover the session
    //! key.
    //!
    //! The classical relay path ([`super::RelayNetlayer`]) seals each frame
    //! with X25519 ECDH → HKDF-SHA256 → ChaCha20-Poly1305. That protects
    //! against a *classical* adversary but not a future quantum one, because
    //! X25519 (a discrete-log problem) falls to Shor's algorithm. This module
    //! adds the missing half: a session KEM whose derived key depends on
    //! **both** an X25519 secret **and** an ML-KEM-768 (FIPS 203) secret.
    //!
    //! ## The combiner (the load-bearing correctness point)
    //!
    //! We follow the published **X-Wing** / TLS **X25519MLKEM768** hybrid-KEM
    //! construction: derive one classical secret `ss_x25519` and one
    //! post-quantum secret `ss_mlkem`, then feed **both, concatenated, plus
    //! the full public transcript** through a single KDF:
    //!
    //! ```text
    ...
    //! ```
    //!
    //! This is a **concatenation KDF, never XOR**: with XOR an adversary who
    //! learns one secret could cancel it and forge agreement on the other;
    //! with a collision-resistant KDF over the *concatenation* the output
    //! depends jointly and inextricably on both. Consequently breaking X25519
    //! alone (quantum) does not recover the key — `ss_mlkem` still protects
    //! it — and breaking ML-KEM alone does not either — `ss_x25519` protects
    //! it. That two-sided dependence is exactly what the `hybrid_dependence_*`
    //! tests below pin.
    //!
    //! The `transcript` binds the derived key to the exact public handshake
    //! material (both X25519 public keys, the ML-KEM encapsulation key, and
    //! the ML-KEM ciphertext), so an active attacker cannot substitute
    //! ephemeral material without changing the key — the same transcript
    //! binding X-Wing performs over `ct_X ‖ pk_X`.
    //!
    //! ## Shape
    //!
    //! One round trip. The **responder** publishes a [`HybridOffer`]
    //! (its X25519 ephemeral public key + its ML-KEM encapsulation key) and
    //! keeps the matching [`HybridResponder`] secrets. The **initiator**
    //! consumes the offer with [`initiate`], producing a
    //! [`HybridInitiatorMessage`] (its X25519 ephemeral public key + the
    //! ML-KEM ciphertext) and *its* copy of the session key. The responder
    //! feeds that message to [`HybridResponder::finish`] to derive the *same*
    //! session key. Confidentiality only: this is a KEM, there is no enroll /
    //! pin / signature here (peer authentication rides the existing
    //! identity/handoff layer).
    ...
    /// HKDF domain-separation / version tag for the hybrid combiner. Bump on
    /// any change to the transcript layout or combiner.
    const HYBRID_DOMAIN: &[u8] = b"dregg-captp-hybrid-kem-x25519-mlkem768-v1";

    /// Errors from the hybrid handshake (all are malformed-wire faults; the
    /// KEM itself does not fail on well-formed input).
    #[derive(Clone, Debug, PartialEq, Eq)]
    pub enum HybridError {
        /// The ML-KEM encapsulation key in an offer was the wrong length /
        /// malformed.
        BadEncapKey,
        /// The ML-KEM ciphertext in an initiator message was the wrong
        /// length / malformed.
        BadCiphertext,
    ...
    /// An OS-backed CSPRNG adapter exposing the `rand_core` 0.6
    /// `CryptoRngCore` that `ml-kem` / `kem` require, sourced from the same
    /// `getrandom` the rest of this crate uses. Every call reads fresh OS
    /// entropy (no reseed state to compromise).
    struct OsCsprng;
    ...
    /// Responder-side secret state kept between publishing a [`HybridOffer`]
    /// and calling [`finish`](Self::finish). Holds the X25519 secret, the
    /// ML-KEM decapsulation key, and a copy of the public offer material
    /// needed to reconstruct the transcript.
    ...
    /// Build the concatenation-KDF transcript: the exact public handshake
    /// bytes, in a fixed order both sides agree on.
    ...
    /// (concatenation, never XOR) with the transcript as HKDF `info`. See the
    /// module docs.
    ...
    /// Responder step 1: mint a fresh hybrid offer and keep the matching
    /// secret state. The returned [`HybridOffer`] is sent to the initiator;
    /// the [`HybridResponder`] is retained for [`finish`](HybridResponder::finish).
    ...
    /// Initiator step: consume a responder's [`HybridOffer`], deriving the
    /// session key and the [`HybridInitiatorMessage`] to send back.
    ///
    /// Fails only if the offer's ML-KEM encapsulation key is malformed.
    pub fn initiate(
        offer: &HybridOffer,
    ) -> Result<(HybridInitiatorMessage, [u8; 32]), HybridError> {
        let mut rng = OsCsprng;
        ...
        let ek_encoded = Encoded::<Ek>::try_from(offer.mlkem_ek.as_slice())
            .map_err(|_| HybridError::BadEncapKey)?;
    ...
    impl HybridResponder {
        /// Responder step 2: consume the initiator's message and derive the
        /// session key — identical to the initiator's when the handshake is
        /// faithful.
        ///
...
mod tests {
    ...
    // -------------------------------------------------------------------
    // Hybrid post-quantum session key-exchange
    // -------------------------------------------------------------------

    use super::hybrid;

    /// KEM correctness: both parties derive the SAME session key from the
    /// hybrid handshake (X25519 + ML-KEM-768 round trip).
    #[test]
    fn hybrid_roundtrip_same_key() {
        let (offer, responder) = hybrid::responder_offer();
        ...
        let (msg, initiator_key) = hybrid::initiate(&offer).expect("initiate");
    ...
    /// Zeroing or replacing the ML-KEM secret alone changes the key (so the
    /// key genuinely rides the PQ half, not just X25519) — and zeroing or
    /// replacing the X25519 secret alone changes it too. This pins the
    /// concatenation-KDF combiner: neither half can be cancelled.
    #[test]
    fn hybrid_dependence_on_both_secrets() {
        ...
        let key = hybrid::combine(&ss_x, &ss_m, t);

        // Zero the ML-KEM secret alone -> different key.
        assert_ne!(
            key,
            hybrid::combine(&ss_x, &[0u8; 32], t),
            "zeroing the ML-KEM secret must change the key"
        );
        // Replace the ML-KEM secret alone -> different key.
        assert_ne!(
            key,
            hybrid::combine(&ss_x, &[0x33u8; 32], t),
            "replacing the ML-KEM secret must change the key"
        );
        // Zero the X25519 secret alone -> different key.
        assert_ne!(
            key,
            hybrid::combine(&[0u8; 32], &ss_m, t),
            "zeroing the X25519 secret must change the key"
        );
        // Replace the X25519 secret alone -> different key.
        assert_ne!(
            key,
            hybrid::combine(&[0x44u8; 32], &ss_m, t),
            "replacing the X25519 secret must change the key"
        );
        // The transcript is bound too (active-attacker material substitution).
        assert_ne!(
            key,
            hybrid::combine(&ss_x, &ss_m, b"other-transcript"),
            "the transcript must bind into the key"
        );
    }

    /// End-to-end: a ciphertext tampered in flight makes the responder derive
    /// a DIFFERENT key than the initiator — the ML-KEM half genuinely
    /// participates in the agreed session key (not a decorative addition).
    #[test]
    fn hybrid_tampered_ciphertext_diverges() {
        let (offer, responder) = hybrid::responder_offer();
        let (mut msg, initiator_key) = hybrid::initiate(&offer).expect("initiate");

        // Flip a byte deep inside the ML-KEM ciphertext (ML-KEM's implicit
        // rejection yields a well-formed but DIFFERENT shared secret).
    ...
    fn hybrid_rejects_malformed_material() {
        let (mut offer, _responder) = hybrid::responder_offer();
        offer.mlkem_ek.truncate(10);
        assert_eq!(
            hybrid::initiate(&offer).unwrap_err(),
            hybrid::HybridError::BadEncapKey
        );

        let (offer, responder) = hybrid::responder_offer();
        let (mut msg, _k) = hybrid::initiate(&offer).unwrap();
        msg.mlkem_ct.truncate(10);
        assert_eq!(
            responder.finish(&msg).unwrap_err(),
            hybrid::HybridError::BadCiphertext
        );
    }
}
