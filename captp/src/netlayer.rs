...
/// The HYBRID (X25519 + ML-KEM-768) session key-exchange for the CapTP
/// transport — the X-Wing / TLS `X25519MLKEM768` concatenation-KDF combiner —
/// now lives in the shared `dregg-pq` leaf so captp no longer embeds ml-kem
/// itself. Re-exported here under the original `hybrid` path so every call site
/// (and the tests below) are unchanged.
pub use dregg_pq::hybrid_kem as hybrid;
