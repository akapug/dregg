    ...
    pub(crate) fn new(rendering_context: Rc<dyn RenderingContext>, paint: &Paint) -> Self {
        ...
        let (mut webrender_renderer, webrender_api_sender) = webrender::create_webrender_instance(
            ...
            webrender::WebRenderOptions {
                ...
                // dregg servo-render SWGL FORK (the ONLY change vs published
                // servo-paint 0.1.0): WebRender's default `clear_caches_with_quads:
                // true` makes its picture-cache clear path issue a depth-`GL_ALWAYS`
                // quad (`renderer/mod.rs:2902` → `Device::enable_depth(Always)`).
                // SWGL's `DepthFunc` (`gl.cc:1384`) only accepts `GL_LESS`/`GL_LEQUAL`
                // and `assert(false)`s on `GL_ALWAYS` → SIGABRT before any page paints.
                // Real servo drives SWGL through a `SwCompositor` that never takes this
                // quad-clear path; servo-paint 0.1.0 has no SWGL-aware option, so we set
                // the flag WebRender's own software path uses: clear caches with a
                // scissored `glClear` (the depth-LEQUAL path), not a depth-ALWAYS quad.
                // This is the minimal SWGL adaptation of the published `WebRenderOptions`.
