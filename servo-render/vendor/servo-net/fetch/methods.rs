...
async fn scheme_fetch(
    ...
    match scheme {
        ...
        // ── servo-render FORK: the http(s) interception point ──────────────────
        // If an embedder registered a `ProtocolHandler` for http/https in the
        // `ProtocolRegistry` (now permitted — `FORBIDDEN_SCHEMES` no longer blocks
        // them, see protocols/mod.rs), route the load THROUGH it: this is where a
        // cap-gated embedder handler owns the http(s) byte socket instead of the
        // built-in hyper path. If NO embedder handler is registered (the default
        // servo embed), fall back to the built-in `http_fetch` so servo's behaviour
        // is unchanged. The `borrow` of `request` here is the SAME `&mut Request`
        // the `_` arm below hands to `handler.load`.
        "http" | "https" if context.protocols.get(scheme).is_some() => {
            let handler = context
                .protocols
                .get(scheme)
                .expect("checked is_some in the guard");
            handler.load(request, done_chan, context).await
        },
