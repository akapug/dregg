...
//
// ── servo-render FORK ── `http`/`https` REMOVED from the forbidden set so an
// embedder may register a `ProtocolHandler` for them (the cap-gated http(s) byte
// socket). `chrome`/`about` stay forbidden (servo-internal pseudo-schemes that
// `scheme_fetch` handles specially). `scheme_fetch` (fetch/methods.rs) consults
// the registry for http/https FIRST and only falls back to the built-in
// `http_fetch` (servo's internal hyper) when NO embedder handler is registered —
// so servo's default behaviour is unchanged for an embedder that registers none.
static FORBIDDEN_SCHEMES: [&str; 2] = ["chrome", "about"];
