...
const ATLAS = {
  ...
  sections: [
    ...
    },
    {
      ...
      what: "Each turn carries a STARK proving it was a valid kernel transition. Turns fold into a constant-size recursive whole-chain aggregate. A light client holds no secrets, re-runs no cell, and — checking only a succinct root — learns the whole history is genuine. The apex theorem and the five guarantees are axiom-clean; the crypto floor has been reduced — StarkSound, deployed-tree faithfulness, AND FriLdtDeployedBound (as stated) are now discharged in-tree, and the deployed recursion fold carries a ~112.6-bit PROVEN soundness bound (wrap_perFold_soundness_capacity; the FRI capacity conjecture is refuted). The only genuine crypto floor left is Poseidon2 collision-resistance, plus the L>1 Johnson/correlated-agreement lemmas (proved by counting at L≤186 interior / L≤292 boundary; GS-ideal 128 blocked).",
      components: [
        ...
        { name: "The crypto floor — reduced to Poseidon2 CR + the L>1 counting residual", status: "DESIGN", what: "StarkSound is DISCHARGED (starkSound_of_verifyAlgo, a THEOREM over verifyAlgo: AlgoStarkSound + DeployedRefines), the BBHR18/BCIKS20 proximity algebra is PROVED on HashCR (friProximityK8_discharge), DeployedFaithful is PROVEN in-tree (deployedFaithfulEff_canonical), and FriLdtDeployedBound as-stated is DISCHARGED (FriLdtJohnson.lean, the trivial 7/8-counting branch). The dim-2 recursion fold has a ~112.6-bit PROVEN per-fold bound (wrap_perFold_soundness_capacity: |Good| ≤ C(64,2)=2016 over |F| ≈ 2^123.6) — that is the arity-2 figure; the DEPLOYED arity-8 wrap reads 109 (FriArityTransfer), and both are claims about 96.9%-far words, not the Johnson radius FRI operates at (FriJohnsonRadiusGap: M=1 is FALSE in [448,496)). Per-fold and query are SEPARATE columns, never multiplied. On the query side the FRI capacity conjecture is REFUTED (Crites–Stewart eprint 2025/2046, by reduction; Kambiré arXiv 2604.09724, a prime-field counterexample), so the 130 column is a drift baseline only; the Johnson QUERY column reads 73 but is the m→∞ idealisation that drops BCIKS20's commit-phase term ε_C, which reads 71 at the deployed wrap and BINDS — the ethSTARK eq.(20) composite is ~70. ε_C contains no num_queries and no query_pow_bits, so the ext-degree-4 ceiling is ~77.98 at any q (at commit_pow=0, where every shipped config sits — plonky3's separate commit-phase PoW knob grinds those terms and is unpriced); the lever on that ceiling is the extension degree, log₂ p = 30.91 bits/degree, and plonky3 panics on degree 6. What remains: Poseidon2SpongeCR (standard collision-resistance) and the L>1 generalization of the two Johnson lemmas (RSListBound, FriProximityGapChallenges) — proved by ordered-pair counting at L≤186 interior (dIn=52) / L≤292 boundary (dIn=56); the GS-ideal L≤128 is BLOCKED (multiset fibre-concentration).",
    ...
    // ───────────────────────────── DEOS ─────────────────────────────
    {
      ...
      components: [
        ...
        { name: "deos-hermes — confined agent", status: "STAGED", what: "An agent loop as a cap-gated surface cell: every ACP tool-call is intercepted → cap-gated, metered, RECEIPTED turn through the proven ToolGateway. End-to-end loop runs over a mock peer; live subprocess install is the seam.",
          files: [f("deos-hermes/src/bridge.rs", 18), f("deos-hermes/src/acp_client.rs", 33), f("docs/SUPERSEDED/HERMES-INTEGRATION.md")] },
