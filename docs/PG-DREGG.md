...
**Pinning.** `pgrx = "0.17"` (supports `pg13`–`pg17` via the `pg13`…`pg17`
features). The **primary target is PostgreSQL 17** (the current release; see
Part III for the pg17 features the mirror leverages); pg13–pg16 remain
selectable for older deployments. `dregg-auth` is a path dependency
...
cargo pgrx init --pg17 $(brew --prefix postgresql@17)/bin/pg_config  # primary target
cargo pgrx test pg17        # runs the #[pg_test]s against the managed pg17
cargo pgrx run  pg17        # opens psql with the extension loaded
# (pg13–pg16 remain available: cargo pgrx test pg16, etc.)
```
...
# Part III — PostgreSQL 17 leverage

`pg-dregg` targets **PostgreSQL 17** as its primary backend (the current
release). The extension builds and its full test suite runs against a brew
`postgresql@17` via `cargo pgrx test pg17`; pg13–pg16 remain selectable for
older deployments. This part surveys the pg15/16/17 features that matter to the
dregg mirror, says which are **applied** (shipped + test-proven) versus
**designed** (a clear milestone), and ends with the federation-via-replication
path that pg17 unlocks.

## 14. The feature survey — what pg15/16/17 bring the mirror
...
- **SQL/JSON `JSON_TABLE` (pg17)** — the embedded jsonb state, projected into a
  ...
  *Why it matters:* the mirror's value is "your node IS your postgres" — the
  ...
  (`src/lib.rs`, run under `cargo pgrx test pg17`) + `ddl_is_emittable_…` /
