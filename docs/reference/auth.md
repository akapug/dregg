...
`Permissions` carries one `AuthRequired` per action (`send`, `receive`, `set_state`, `set_permissions`, `set_verification_key`, `increment_nonce`, `delegate`, `access`) (`cell/src/permissions.rs:84`). Presets (`:106`–`:163`): `default_user` (signature for all actions except `receive` and `access`, both `None`), `sovereign_default` (same but `set_verification_key: Proof`, self-upgrading), `zkapp` (proof for all, `receive: None`), `frozen` (`Impossible` for all). `Permissions::check(action, auth)` dispatches via `for_action` to `AuthRequired::is_satisfied_by`; `AuthKind` (the *provided* kind) is `Signature | Proof` only — `Custom`/`Impossible` are never satisfied through this method (`Custom` goes through the executor's per-variant check) (`:24`–`:44`, `:165`–`:183`).
