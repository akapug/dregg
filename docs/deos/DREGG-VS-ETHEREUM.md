...
| **ERC-4337** account abstraction | UserOps, bundlers, EntryPoint, paymasters | **A (partial)** | Caps *are* account abstraction — auth is data, delegation/session-keys are attenuated caps. `invoke()` is the command front-door, no privileged EntryPoint (`app-framework/src/invoke.rs:60`). **Lesson half:** the *paymaster UX pattern* (sponsored fees) is worth importing — see §3. |
...
- App layer: `app-framework/src/invoke.rs:60` (`invoke`; `invoke_with_descriptor` @85); `app-framework/src/reactor.rs:209`
