...
| **The package manager (`PackageManager`, the APK installer)** | install/verify/track APKs; a UID gets the manifest's permissions **ambiently at runtime** | **the cell factory / hatchery** — "installing an app" = minting a cell from a factory descriptor (a provable, cap-gated birth) whose `allowed_cap_templates` are EXACTLY the manifest's declared permissions (a permission not declared yields no cap — no ambient escalation); a foreign APK = minting an **android-cell** whose cap-set scopes its I/O | **prototyped:** `android-cell/src/appfactory.rs` (`AndroidManifest::to_factory_descriptor` — the manifest→`FactoryDescriptor` map over `cell/src/factory.rs`'s real `FactoryDescriptor`/`CapTemplate`, content-addressed + authority-sensitive); `HATCHERY-ABSTRACTION-MINT.md` |
