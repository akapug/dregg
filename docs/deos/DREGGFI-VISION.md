...
1. **Execution soundness** — the multilateral clearing conserves per-asset + is atomic. *`Ring.lean` PROVED.* Plus the **limit-respecting fairness half** (`clearing_respects_limits`, `Market/Fairness.lean`): no participant is worse off than its declaration on either side, with refusing teeth — *the first `metatheory/Market/` rung, PROVED.* Still **unbuilt** (the genuinely-open fairness piece): **uniform-price optimality** (all legs of a two-sided batch clear at ONE price) + **envy-free / Shapley–Scarf TTC-core stability** — the named next rungs (conservation and even individual-rationality ≠ uniform-price optimality).
...
1. **Now (unblocked, composing on proven pieces):** the DrEX rung-1 **limit-respecting fairness theorem** is LANDED (`clearing_respects_limits`, `Market/Fairness.lean`, composing with `Ring.lean`); the next fairness rung — **uniform-price optimality + envy-free/TTC-core stability** — is the open build; plus the cross-chain governance demo; OCIP's attested lane + money-paths.
...
- **Fairness is PARTLY proven; the optimal-clearing half is not.** dregg now proves conservation + atomicity **+ limit-respecting individual rationality both sides** (`clearing_respects_limits`, `Market/Fairness.lean`: nobody debited above its offer nor credited below its minimum, with refusing teeth). What is NOT yet proven is **uniform-price optimal clearing** (one clearing price for a two-sided batch) and **envy-free / Shapley–Scarf TTC-core stability**. So "fair" is a *verified* claim for the individual-rationality sense and an *architecture* claim for the uniform-price/envy-free sense — say which.
