# DreggScript — design and exploration

A directory for design notes, explorations, and prototypes of the
behavior/protocol language for dregg app authoring. **Not yet a language;
not yet code.** This directory captures the design conversation that
precedes the language.

## The two-language model

dregg has two distinct surface languages serving different layers:

1. **`dregg-dsl`** (exists today) — *caveat predicate language*.
...
2. **`dreggscript`** (this directory's subject) — *behavior / protocol
   ...
   calls, cell program declarations (which themselves may emit dregg-dsl
   predicates), CapTP wire protocols.

The two languages **compose**: dreggscript invokes dregg-dsl when it needs
...
`dregg-sdk` or a new `dregg-cell` crate?
...
- **CakeML** (`~/dev/CakeML`) — verified ML implementation, HOL-derived
  ...
  **`translator/`** (proof-producing HOL→CakeML), **`cv_translator/`**.
  ...
  HOL theorems can reach into CakeML-compiled dreggscript through
...
handoffs cleanly — and dreggscript gets its own implementation,
likely on Lean 4.
...
- Effect handlers (Eff / Koka / OCaml 5) — "I declare what effects
  ...
  naturally onto the dregg Effect enum.
...
`dregg`'s design lineage already names many points:
...
- **Fly.io macaroons** — attenuable bearer tokens (dregg-dsl's origin)
- **seL4 / L4.verified** — formal capability discipline
- **Greg Egan's Polis** — software citizens
- **svenvs** — verified safety envelopes

References worth studying for dreggscript specifically:
- **Hash / Unison** — content-addressed code, distributed composition
- **discord-bot's DiscordCapability** — the "capability + adapter"
  userspace pattern already in dregg
- **OCapN protocol** — formal capability transport that dregg's CapTP
  draws on
- **biscuit-rs** — caveat-language ancestry, may inform dregg-dsl
  evolution
...
- `STAGE-7-PLUS-DESIGN.md` — the proof-system trajectory dreggscript
  compiles into
- `STAGE-7-GAMMA-AGGREGATION-DESIGN.md` — cross-cell aggregation
  semantics dreggscript will eventually surface
- `WITNESSED-RECEIPT-CHAIN-DESIGN.md` — the replay semantics
  dreggscript's runtime must produce
- `DSL-TO-EFFECT-VM-FEASIBILITY-STUDY.md` — why dregg-dsl stays at
