# PureCake and CakeML as dreggscript compile targets — exploration
...
## Integration story: dreggscript → PureCake

### What it would look like

dreggscript → desugar to PureLang surface (Haskell-subset). Cell
...
dreggscript Q3 maps reasonably onto a lazy `IO` event loop.

Concretely: write a backend `gen_purelang` alongside the existing
dregg-dsl `gen_*` backends. It would emit `.hs` files that the
...
not true of any other backend in the dregg stack.

### Verdict

**Interesting but speculative.** The laziness story is a feature for
some dreggscript shapes (lazy infinite event streams; demand-driven
...
build a credible dreggscript→PureLang emitter; another 6–12 months
to make svenvs talk to PureCake/CakeML semantics in a way that
actually composes. **Year-scale.**

## Integration story: dreggscript → CakeML

### What it would look like

dreggscript → desugar to CakeML surface. Cells become `structure`s
...
CakeML is the bedrock. svenvs already speaks HOL4. Any dreggscript
...
match Rust's mental model and dregg's existing turn/receipt
...
**Cost**: 3–9 months for a credible dreggscript→CakeML emitter
that handles the cell behavior subset. 6–12 additional months to
build a svenvs ↔ CakeML semantic bridge (i.e., state dregg cell
...
| Mental fit for dregg     | speculative            | natural                   |

## Verdict

**CakeML is the credible verified backend.** It is the more
actively maintained, more thoroughly verified, more
mentally-aligned-with-dregg option. The strict semantics match
Rust and match dregg's turn-by-turn discipline. The `translator/`
...
which has been demonstrated for systems work at dregg's scale.
...
**Neither is right *yet*** for production dregg work. The honest
...
**1-page proof-of-life**: hand-write a single dregg cell behavior
...
substrate carry the message shapes dregg actually needs, or does
it collapse on the first three-party handoff?

If CakeML's FFI cannot represent CapTP introductions cleanly,
the answer is **neither, and dreggscript gets its own
