...
| 4 | **incrementNonce** (`EffectVmEmitIncrementNonce`) | **A** | `incNonceDescriptor_classA` (= `incNonceDescriptor_full_sound:233` + `incNonceDescriptor_commit_binds_state` + `unify_incNonce_exec` → `execFullA … (.incrementNonceA …)`) | **CLASS-A PROMOTED (per-cell, the transfer bar).** The effect IS the nonce tick — an in-commitment state-block column, bound + anti-ghosted (13 cols). The A− residual ("executor-orphaned") is CLOSED: `unify_incNonce_exec` welds the descriptor's bound block to `execFullA`'s `IncrementNonceSpec` post-state (the whole conserved-economic `cellProjN` block agrees — balance frozen, frame frozen). The ONE named residual (NOT a soundness gap): the nonce CARRIER divergence — the descriptor's on-trace `state.NONCE` ticks the runtime per-cell sequence counter, while the executor writes the cell-record `nonce` field to `n` (`nonce_write_is_out_of_row`); both monotone nonce advances on distinct carriers, the conserved-economic block fully agreed + anti-ghosted. |
...
| # | Effect | Class | Note |
|---|--------|:-----:|------|
| 51 | **setField** (`EffectVmEmitSetField`) | **A** | `setFieldDescriptor_classA` (= `setFieldDescriptor_full_sound` + `setFieldDescriptor_commit_binds_state` + `unify_setField_exec` → `execFullA … (.setFieldA …)`) | **CLASS-A PROMOTED (per-cell, the transfer bar).** A field write IS an in-commitment column move: the EffectVM row's `fields[0..7]` columns are among the 13 the deployed `state_commit` absorbs, so the written `fields[slot]` is bound + anti-ghosted EXACTLY like transfer's `bal_lo`. The new dedicated `setFieldVmDescriptor slot` FORCES `fields[slot]_after = param.VALUE` (the written value) + FREEZES every other state-block column (the other 7 fields + bal/nonce/cap/reserved); `setFieldDescriptor_full_sound` gives `CellSetFieldSpec` (the per-field write + whole-frame freeze), `setFieldDescriptor_commit_binds_state` anti-ghosts ALL 13 absorbed cols (tampering the written slot OR any frozen column ⇒ UNSAT), and `unify_setField_exec` welds to `execFullA … (.setFieldA actor cell (slotName slot) v)` (= `SetFieldSpec`, the cellstatefield executor⟺spec corner): the written slot reads `v`, the conserved balance frozen (`slotName_ne_balance` ⇒ `setField_balOf`). Named honest boundary (the executor/record-layer legs the per-row block doesn't carry, the same shape transfer's nonce/log has): the caveat+authority+membership+liveness GUARD (`setField_guard_is_offrow`, the executor's domain restriction the record-layer `SetFieldCommit` corner welds) and the one-row receipt LOG append (`setField_log_is_offrow`). |
