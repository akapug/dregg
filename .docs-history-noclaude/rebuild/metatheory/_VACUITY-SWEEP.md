...
What the sweep *did* find is one structural-weakness — a named "binding" invariant whose
tag argument was decorative (the real binding predicate orphaned) — now **RESOLVED** (Finding 1
below): the binding predicates `cwmInCompartment`/`sgmInBucket` genuinely USE their tag
(`cwmAnchor = comp` / `sgmAnchor = bucket`), are proven non-vacuous both poles, and carry the
literal tag into the production crown. Also: the **WART** Ring seed ember already named, and a
handful of decorative-totality notes.

## Severity-sorted table

| # | Severity | Location | Claim advertised | Why weaker than advertised | Load-bearing? |
|---|----------|----------|------------------|----------------------------|---------------|
| 1 | **RESOLVED** | `Apps/CompartmentWorkflowMandate.lean`, `Apps/StorageGatewayMandate.lean` | `cwmInCompartment k comp` / `sgmInBucket k bucket` — "the mandate is bound to compartment `comp` / bucket `bucket`" | ~~Body ignored the `comp`/`bucket` argument; the real binding was orphaned.~~ FIXED: each predicate now folds in the literal anchor conjunct (`cwmAnchor k = comp` / `sgmAnchorIs k bucket = true`), so a wrong/drifted tag is REJECTED. Non-vacuous both poles (`#guard`: right tag accepted, wrong tag + drift rejected). The crown `agent_mandate_safety_forever` carries the genuine binding under the stated anchor-safe residual (`DualSchedAnchorSafe`); the orphaned `*Strong` predicates are now the carriers it routes through. Mutation-confirmed load-bearing (reverting to the tag-ignoring body reds the carry + axiom-hygiene). | YES — in the COMPOSED PRODUCTION CROWN `agent_mandate_safety_forever`, now genuinely tag-pinning |
...
### Finding 1 (RESOLVED) — the "in-compartment" / "in-bucket" tag is decorative; the binding predicate is orphaned

> **RESOLUTION (resolved).** The weak `cwmInCompartment`/`sgmInBucket` predicates now GENUINELY bind
> their tag — each folds in the literal anchor conjunct:
> ```
> def cwmInCompartment (k) (comp)   := mandateCell ∈ k.accounts ∧ cwmMandateProgramOK k ∧ cwmAnchor k = comp
> def sgmInBucket      (k) (bucket) := mandateCell ∈ k.accounts ∧ sgmMandateProgramOK k ∧ sgmAnchorIs k bucket = true
> ```
> so the predicate is value-dependent on the tag: a wrong/drifted anchor is REJECTED. Non-vacuity is
> witnessed at both poles on concrete data (`#guard`s in the gated modules): the right tag is ACCEPTED
> (`decide … `), a wrong tag is REJECTED (`decide … == false`), and a drift-genesis is REJECTED at the
> bound tag. **Design decision** the finding's "needs design resolution" called for: the literal anchor
> conjunct is NOT preservable by the EVERY-step `CellContract.step_ob` (a `makeSovereign` aimed at the
> mandate cell — the sole un-caveat-gated record rebind — can drop the field), so the genuine binding
> cannot ride the unconditional contract machinery. The faithful resolution carries the genuine binding
> via the per-app **anchor-safe induction** (`cwm_compartment_strong_forever` / `sgm_bucket_strong_forever`,
> the formerly-orphaned `*Strong` carriers) and threads it into the crown `agent_mandate_safety_forever`
> under the stated residual hypothesis `DualSchedAnchorSafe` (the precise behaviour the immutable-anchor
> caveat cannot reject inline). The contract now carries only the unconditional legs (step-legal
> program-live + pay + revoked); the genuine tag-pin is the fourth crown conjunct. Mutation-confirmed
> load-bearing: reverting either predicate to the tag-ignoring body breaks the carry lemma and trips the
> `#assert_axioms` hygiene check. `lake build Dregg2` green. The §3 `agent_mandate_safety_forever_strong`
> is now a strong-predicate-form restatement of the (already tag-pinning) main crown.
