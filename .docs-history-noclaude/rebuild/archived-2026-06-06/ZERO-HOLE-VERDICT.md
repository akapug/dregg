# ZERO-OPEN-HOLE VERDICT — dregg2 metatheory (adversarial, read-only)
...
**CONFIRMED.** The dregg2 metatheory (`metatheory/`) is literally free of open
holes, the full `lake build` is green with **zero** open-hole warnings, the
`Claims.lean` `#assert_axioms` ledger elaborates clean, and the CI guard has
**demonstrated teeth** (a planted open hole trips its regex). The three former
open-hole bodies are each replaced by an **honest** construct (typeclass field /
named hypothesis), not by vacuity-laundering — verified individually below.

## (1) ZERO open holes — evidence

- `lake build` → `BUILD=0`, `open_holes=0` (count of the `declaration uses` open-hole
  warning Lean emits for every open hole / `admit` / open-hole kernel axiom). No `error:` lines.
- `lake env lean Dregg2/Claims.lean` → exit `0`, no error/open-hole/warning output.
  The `#assert_axioms` ledger (which turns any *inherited* open-hole kernel axiom into a hard
...
- Textual scan of the WHOLE `metatheory` tree (excluding `.lake/` deps) for a bare
  open-hole token NOT inside backticks: every remaining hit is a COMMENT or
  doc-string (e.g. `Claims.lean` markdown headers describing the guard,
  `Dregg2.lean` import doc-strings, `Exec/Consensus.lean:133` "kernel-clean").
  ZERO term/tactic bodies use an open hole.
- The three named former open-hole bodies are gone:
  - `Core.lean:162` is now PROSE inside a doc-comment (narrating "...no open hole
    anywhere"), above `class ConservesStep` (the replacement).
  ...
  - `Spec/VatBoundary.lean` `phi_functorial` is now a PROVED theorem under
    `NonDegenerate` (no open hole in body).

## (2) HONEST conversions — per-obligation teeth audit
...
- Now a **typeclass field**: `Core.ConservesStep.step : count A + minted = count B
  ...
  `CryptoKernel` Prop-portal idiom, NOT an open hole.
...
- HONEST RESIDUAL (disclosed, not a defect): the *abstract measure*
  ...
  former open-hole primitive had — the improvement is that there is now a genuine
...
- Now `theorem phi_functorial (hnd : NonDegenerate stmtOf) : PhiFunctorial …` —
  genuinely PROVED (no open hole), honestly CONDITIONAL on `NonDegenerate`.
...
- The `ci.yml` axiom-hygiene-guard job ("Metatheory open-hole guard"), Lean pinned
  to `metatheory/lean-toolchain` (v4.30.0), runs `bash
  scripts/axiom-hygiene-guard.sh`.
- The script runs the real `lake build`, captures the exit code DIRECTLY (no
  head/tail masking), and fails if: build≠0, OR the `declaration uses` open-hole
  warning is grepped (count > 0), OR any open-hole-kernel-axiom mention.
- CRITICAL teeth detail verified empirically: Lean 4.30 wraps the word in
  BACKTICKS. The guard regex uses a wildcard quote char, so it MATCHES the real
  backtick form (and also the legacy single-quote form). A toothless regex pinned
  to one quote form would MISS the real warning — the script does NOT use that form.
- EMPIRICAL planted-hole test (read-only, throwaway `/tmp` file, corpus
  untouched): a file with `theorem h : 1 = 2` closed by an open hole, elaborated
  under the corpus toolchain, emits the `declaration uses` open-hole warning, and
  the guard regex matched it (`guard_regex_matches=1`). A planted open hole WOULD
  fail CI.
- Two complementary layers, honestly distinguished: (a) the textual build-warning
  grep = the whole-corpus net (catches any open hole anywhere, pinned or not); (b) the
  `Claims.lean` `#assert_axioms`/`#assert_namespace_axioms` ledger = the
  deep-but-targeted transitive-inheritance tripwire (catches an open hole hidden behind
  a renamed lemma, but only for pinned decls/namespaces). Neither alone is
  whole-corpus + transitive; together they cover both.

## FINAL STATEMENT

The metatheory is free of open holes and CI-guarded. The 3 assumptions now honestly are:
