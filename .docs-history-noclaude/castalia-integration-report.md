...
```
`dregg`'s `dregg-trace` Datalog evaluator can express these directly. Zenith defines the vocabulary; dregg proves predicates over it.

**Attenuated task capabilities for agents/work-orders**: When Hub dispatches a work order, the agent receives a scoped, time-limited, budget-bounded capability token rather than ambient credentials. `dregg`'s `AuthToken` (macaroon + Biscuit hybrid with caveats) and `tokenizer` (sealed secret proxy) map directly. The agent returns a proof-carrying receipt chain as its audit trail.

What Castalia does NOT need from dregg: federation consensus, cell execution, note trees, the intent marketplace, the browser extension, or the economic model. These are dregg-internal infrastructure. Castalia integrates at the capability/proof/verification surface, not the runtime surface.
...
**The primitive dregg should adopt**: The `objContract.counter` pattern -- a minimal on-chain anchor that provides ordering and non-repudiation without revealing who owns what. This maps to dregg's existing machinery in two ways:
...
The rule from the integration brief is explicit: **membership/standing credentials should remain non-transferable claims bound to a subject. `dregg` capabilities can be delegated FROM that standing for specific tasks, times, resources, and budgets.**

In implementation:

1. A `SoulBoundCredential` (Castalia's `soul_bound_credentials` table, status: active/revoked/expired/suspended) is checked as a **predicate fact** in dregg's Datalog, not stored as a token. It is a signed JSON credential (off-chain fallback) or a Midnight credential (target). `dregg`'s `prove_predicate_unlinkable()` can prove predicates over it without revealing it.
...
3. `dregg`'s non-revocation proofs (`prove_not_revoked`, `prove_not_revoked_accumulator`) map to Castalia's credential status transitions. When a credential is revoked, its revocation hash enters the revocation set. Future capability presentations fail the non-revocation check.

**What must NOT happen**: collapsing `member(S, C)` into a transferable `dregg` token. The moment standing becomes a bearer token, you get a secondary market in community standing. The integration brief explicitly warns against this.
...
**Concrete implementation using existing dregg surfaces**:
...
2. **Capability minting** (Hub creates a dregg `AuthToken`):
   - Root: Hub's signing key
   - Caveats: `action = "review"`, `resource = artifact_id`, `expiry = T`, `budget = 1`

3. **Presentation** (dregg SDK, running in the reviewer's client):
...
This demo requires:
- The `dregg-token` crate (works, tests pass)
- The `dregg-trace` crate (works, tests pass)
...
2. **Do not make `dregg` federation membership identical to Castalia state/community membership.** A Castalia state may use one or more `dregg` federations, and a `dregg` federation may host many Castalia states. These are not the same concept. Do not map `state_id` directly to a dregg federation ID.

3. **Do not assume Matrix disappears.** `dregg`'s intent/MCP/3PI surfaces do not replace community conversation and social trust. Matrix stays as the social coordination layer.

4. **Do not assume Base/EVM becomes canonical identity.** `dregg`'s EVM bridge is useful for public verification/settlement, not necessarily identity root. Treat Base as a proving/settlement demo path.

5. **Do not make Hub state proof-only yet.** Hub still needs readable database/case/review/artifact state. `dregg` proofs attach to Hub records before replacing any persistence model.
...
The integration brief provides a clean three-layer mapping to dregg's current architecture:

### secC = client/prover/cclerk side

**`dregg` surface**: Browser extension (`window.dregg`), SDK (`AgentCipherclerk`, `AgentRuntime`, `SiloClient`), HD cclerk, proof generation, capability holding.
...
**`dregg` surface**: Node verification layer (`dregg-node` HTTP API), wire verification (`dregg-wire` STARK verification on receive), MCP/HTTP authorization checks.
...
**`dregg` surface**: Datalog evaluator (`dregg-trace`), predicate proofs (`dregg-circuit`), Castalia-specific fact schemas (new, to be defined by Zenith).

**What it does for Castalia**: Names the governance semantics as policy facts. Defines role schemas (`CommunityRole`), community capabilities (`CommunityCapability`), RFC/bucket/repo action policies. This is where Castalia says "what reviewer means" and dregg proves "this subject satisfies the reviewer predicate."

**Relationship**: secZ is authored by Zenith/Castalia community governance. It uses dregg as the engine but is not defined by dregg. `dregg` is the substrate; secZ is the policy profile painted on top.


## 7. Gabriel's "On-Chain Nonce" Question: Should `dregg`'s Macaroon Nonces Be On-Chain?
...
**Should dregg's macaroon nonces be on-chain?**
...
**Recommended design**: `dregg` macaroon nonces remain off-chain by default. The `chain/` workspace (SP1/EVM settlement, `DreggVault`, `DreggCredentialGate`) provides an optional on-chain anchoring path for high-value or cross-trust-domain operations. The nonce goes on-chain only when the action crosses a trust boundary that neither party's local federation covers -- precisely Gabriel's original use case of "transfers between cipherclerks that don't share a trusted third party."

This maps to the Base integration scaffolding already in dregg:
- `DreggVault.withdraw()` uses an on-chain nullifier for double-spend prevention
- `DreggCredentialGate.verifyCredential()` uses an on-chain presentation nullifier for sybil resistance
