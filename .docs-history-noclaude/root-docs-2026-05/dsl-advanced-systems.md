# `dregg` DSL: Advanced Systems Design
...
```rust
// Auto-generated by #[dregg_circuit] for temporal_predicate:
...
The STARK proves arithmetic correctness over field elements. Bulletproofs provide range proofs over the committed values. The binding signature ties them together. The DSL's composition layer (`#[dregg_compose]`) orchestrates these heterogeneous proofs and defines what binds them.
...
4. **Private DeFi requires heterogeneous proof composition** (STARK + Bulletproofs + Schnorr). The DSL's `#[dregg_compose]` layer handles binding; individual proof systems remain specialized.
