# Evaluation: Should dregg replace net/ with iroh?
...
## 1. What dregg's net/ currently provides

| Capability | Implementation |
|---|---|
| QUIC transport | quinn 0.11 (direct dep, 1 layer) |
| Mutual TLS | rustls 0.23, self-signed certs, ALPN `dregg/p2p/1` |
...
## 2. What iroh provides that dregg does not have

| Capability | Value to dregg |
|---|---|
...
| DNS-based discovery (pkarr) | LOW -- dregg uses federation config for peer discovery |
...
## 3. What dregg has that iroh does NOT provide
...
| WASM story | WEAK | iroh's WASM support is browser-only with `wasm-bindgen-futures`, no UDP (no hole punching in browser), effectively relay-only. dregg's WASM crate currently does not include networking at all. |
...
| Authentication model mismatch | MEDIUM | iroh uses ed25519 `SecretKey`/`PublicKey` (ed25519-dalek) for node identity. dregg uses `blake3(cert_der)` for NodeId + separate Ed25519 signing keys from `dregg-types`. These are reconcilable but require adapter code. |
| Gossip semantics mismatch | MEDIUM | iroh-gossip has a 4KB default max message size (`DEFAULT_MAX_MESSAGE_SIZE`). `dregg` messages (turns with data, forest data for atomic turns) can be larger. Configurable but shows different design assumptions. |
...
| Forked quinn | HIGH | iroh uses `iroh-quinn` 0.13, a fork. If upstream quinn evolves (currently 0.11 in dregg), we would be locked into iroh's fork decisions. |
...
| Maintaining QUIC ourselves | LOW | Quinn is stable and well-maintained. dregg just uses it; we don't maintain QUIC itself. |
...
| Peer discovery | LOW | `dregg` uses federation config. We don't need DNS-based discovery. |
...
- iroh-blobs is entirely irrelevant to dregg's use case (we don't transfer large files via gossip).
- The HyParView membership protocol in iroh-gossip is more sophisticated than our eager degree cap, but our federation has a known, small participant set (not thousands of anonymous peers). HyParView matters when you have 1000+ nodes with churn. `dregg` federations are typically 3-21 nodes.

**What is genuinely valuable:**

- NAT traversal and relay fallback. This is the one thing that is genuinely hard to build yourself and that iroh does well. If dregg ever needs to work across residential NATs without a VPN or known public endpoints, iroh's magicsock is the right answer.
...
3. `dregg`'s deployment model (federated nodes with known addresses) sidesteps iroh's primary value proposition (NAT traversal for unknown peers).
4. The dependency cost is enormous (6 deps -> 47+ deps, forked quinn).
5. The WASM story doesn't help us (iroh WASM is relay-only; dregg WASM doesn't include networking).

**Consider iroh as transport-only (Strategy A) when:**
- `dregg` needs to support nodes behind residential NATs (not just data center or known-endpoint deployments).
