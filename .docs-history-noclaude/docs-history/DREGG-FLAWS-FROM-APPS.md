# DREGG-FLAWS-FROM-APPS.md

**Scope:** `dregg`-platform-level flaws surfaced by auditing all 10 apps in batches 1+2 (2026-05-24).
...
| lending | 634 | BROKEN | Zero `dregg` primitive usage; sentinel `0xAA/0xBB/0xCC` identities; STARK proofs decorative; no oracle. 4 P0s, 10 P1s. |
...
Synthesized from all 10 CLAUDIT.md `Dregg-level flaws surfaced` sections, deduplicated, organized by theme, ranked by load-bearingness. G1–G15 from batch 1 are kept stable; G16–G30 are batch-2 additions.

### Theme A — Framework / auth / identity binding (the highest-leverage tier)

#### G1. `dregg-app-framework`: `AuthenticatedRequest<C: Capability>` axum extractor.
...
#### G16. `FederationClock` primitive in `dregg-app-framework`.
...
`dregg` has `Effect::NoteSpend { nullifier, … }` for one-shot spends and `BlindedQueue` per-dequeue nullifiers, but no general `(domain, key) → spent` Merkle set primitive with a federation-published root. Subscription debits, bounty claims, voting nullifiers, and recurring-claim flows all want this. Generalizes `NoteSpend`.
...
#### G31. Promote `bridge::present` to a top-level "`dregg` credentials" module.

`bridge/src/present.rs::BridgePresentationBuilder` + `macaroon/` already implement what `apps/identity/` claims to do, *correctly*, with federation-bound issuer membership and real STARK presentation. The identity app reinvents a strict subset, badly. Promote this to documented `dregg-credentials` and deprecate the app's reinvention.

#### G39. Non-revocation circuit must bind credential hash, not just root.

`dregg_circuit::dsl::revocation` has `pi::REVOCATION_ROOT` as the only PI. A holder can substitute a different non-revoked hash and the verifier cannot tell (identity P1-6). **Fix:** add `pi::REVOCATION_HASH` as a second PI bound to the trace column carrying the queried hash.
...
#### G33. Canonical name-directory primitive (`dregg-directory` crate).

CapTP has `SwissTable` (swiss → live capability); no platform analog for *named* (human-readable → swiss/sturdy-ref, with rent / expiry / dispute baked in). A `dregg-directory` crate combining `SwissTable` + `Authorizer` + `EscrowManager`-backed rent + Merkle-rooted name index would replace ~80% of the nameservice and governed-namespace code.

#### G34. CapTP high-level `resolve(name) -> DreggUri` client API.
...
Nameservice's premium-tier scheme (1-3 chars = 1000/epoch, etc.) is a per-app convention. ENS uses Vickrey for short names; Handshake runs a real Vickrey. `dregg` has neither. Without a platform-level primitive, every name-shaped app re-rolls.
...
`dregg` has federation key material in `coord/` but no app-facing primitive for "encrypt to a coordinator who decrypts only inside a STARK." Privacy-voting F4. Required for coercion-resistance and anti-collusion.

#### G27. Bulletin-board primitive.

`dregg-storage` has `BlindedQueue`, `ProgrammableQueue`, `Inbox`, `DataflowQueue` — but nothing shaped like an append-only, federation-attested bulletin with per-leaf inclusion proofs. Voting needs it; whistleblower-style flows need it; any commit-reveal needs it.
...
- `dregg_storage::BlindedQueue` produces nullifiers but no `Effect` consumes them (orderbook bypasses executor).
...
Stablecoin, identity, gallery all surfaced "frontend calls endpoints that don't exist with wrong body shapes." A typed `dregg-sdk-ts` generated from `apps/<x>/src/server.rs` route definitions would prevent silent drift.
...
- `dregg-dsl` produces real circuits for real backends. The flaw is constraint vocabulary (G4) + the apps' tendency to write the same witness-vs-constraint bug it doesn't help them avoid.
- `subscription` is the **first audited app to use a `dregg` primitive non-decoratively** (`receive_signed_delegation` actually verifies). Existence proof that apps *can* use the primitives correctly when the developer commits to it.
...
6. **G31 (promote `bridge::present` to `dregg-credentials`).** Cheap; deprecates the broken identity-app reinvention.
7. **G32 + G33 (Effect::RegisterName + dregg-directory).** Unblocks nameservice and governed-namespace.
...
2. **Should `dregg-app-framework` be promoted to a "well-paved-path" layer?** Opinionated defaults, refuse-to-construct-without-verifier, axum extractors, schema-generated TS SDK?
...
- `apps/nameservice/src/registry.rs:18` — `pub type DreggUri = String;` (the real `captp::uri::DreggUri` never imported): **CONFIRMED.**
