...
2. **A missing SDK helper** — add to `AgentCipherclerk` (Tier 0/1 list in
   `SDK-REVIEW.md`).
...
- **`AppServer::with_cclerk(cclerk)`** — installs the cclerk as an
  axum `Extension<AppCipherclerk>`. Handlers extract it via
  `axum::Extension<AppCipherclerk>` and build signed actions through it.
...
- **`apps/nameservice` migrated** — `effects.rs` no longer constructs
  ...
  `cclerk.make_action(registry_cell, "register_name", effects)`. The
...
1. Add `let cclerk = AppCipherclerk::new(AgentCipherclerk::from_..., federation_id);`
...
pub fn build_order_escrow_action(
    ...
    cclerk.make_action(registry_cell, "create_order_escrow", vec![effect])
}
```
...
**Migration steps:**
1. Add `state.cclerk: AppCipherclerk` to gallery's app state.
...
3. For each `Turn { ... }` literal, replace with
   `state.cclerk.make_turn(state.cclerk.make_action(target, method, effects))`.
...
accept either an `Authorizer` (current) or an `AppCipherclerk` (new).
...
pub fn build_ballot_submit_action(
    ...
    cclerk.make_action(ballot_cell, "submit_ballot", effects)
}
```
...
### Gap 3 — Cipherclerk-bound effect targeting

**Symptom:** `cclerk.make_action(target, method, effects)` requires
the *caller* to specify a target `CellId`. For app-internal actions
(transferring between the app's own cells), the target is the
cipherclerk's own cell — but the caller has to compute that with
`cclerk.cell_id()`. Mechanically fine; ergonomically a small papercut.
...
because `cclerk.make_turn` only takes one action.
