...
//! presentations into Groth16 proofs that can be verified by the
...
//!        v
//! wrap circuit (verifies the STARK presentation; native gnark, pending)
//!        |
//!        v
...
/// The contract calls its configured verifier contract to check the Groth16 proof,
/// then checks that the committed public values match the expected federation root
/// and predicate hash.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct EvmCredentialProof {
    /// The Groth16 proof bytes (from the wrap prover).
    pub proof_bytes: Vec<u8>,

    /// The public values committed by the wrap circuit.
    /// Encodes: (valid: bool, federation_root: [u8;32], predicate_hash: [u8;32], nullifier: [u8;32])
    pub public_values: Vec<u8>,

    /// The wrap circuit's verifying-key identifier.
    pub vkey: String,

    /// The verifier contract address on Base.
...
/// Public values committed by the wrap circuit for credential verification.
///
...
///
/// # Mock Mode
/// With the `mock` feature, this produces a simulated proof suitable for testing
/// the integration flow end-to-end; otherwise it is fail-closed
/// ([`ChainError::WrapProverMissing`]) until the native gnark wrap is wired.
