...
impl VaultEventListener {
    ...
    pub async fn run(&self) -> Result<(), ChainError> {
        ...
        let deposit_topic = compute_event_topic("Deposit(address,uint256,bytes32,uint256)");

        let mut cursor = self.last_block;
        let mut consecutive_errors = 0u32;

        loop {
            match self.poll_events(vault_addr, &deposit_topic, cursor).await {
                Ok(events) => {
                    ...
                    for event in &events {
                        if let Err(e) = self.note_tx.send(event.clone()).await {
                            ...
                            return Err(ChainError::Other(anyhow::anyhow!("note channel closed")));
                        }
