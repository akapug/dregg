//! Withdrawal proof generation: produces wrapped Groth16 proofs for on-chain vault withdrawals.
//!
...
//!
//! This module generates the STARK proof of these properties, then wraps it
...
//!     ├─ 3. Wrap the STARK (produces Groth16; native gnark wrap, pending)
//!     │
//!     v
...
pub struct WithdrawalRequest {
    ...
    /// correct nullifier derivation, value/recipient binding). The real wrap
    /// prover re-verifies this in-circuit and emits a Groth16 proof; the `mock`
    /// path ignores it. Empty in mock requests.
    #[serde(default)]
...
pub struct WithdrawalProof {
    /// The EVM proof (Groth16) ready for vault.withdraw().
...
/// Public values committed by the wrap circuit for withdrawal verification.
///
...
/// Generate a wrapped Groth16 proof suitable for on-chain withdrawal from the vault.
///
/// This function:
/// 1. Validates the withdrawal request parameters
/// 2. Constructs the STARK proof of note membership + ownership
/// 3. Wraps the STARK to produce a Groth16 proof (native gnark wrap, pending)
...
///
/// # Mock Mode
/// With the `mock` feature, produces a simulated proof for testing the flow;
/// otherwise fail-closed ([`ChainError::WrapProverMissing`]).
pub async fn generate_withdrawal_proof(
    ...
    let expected_nullifier =
        derive_nullifier(&request.secrets.nullifier_key, &request.note_commitment);
...
///
/// This crate (`dregg-chain`) is the **EVM withdrawal** boundary. Its
/// nullifier matches what the wrap circuit commits as a public output for the
...
/// circuit and contract verify. The two are doubly separated: a different hash
/// family (BLAKE3 vs Poseidon2) AND a different domain string. Reconciling them
/// into a single derivation would require:
///
///   1. Changing the wrap circuit's PI layout and re-deploying the EVM verifier.
...
async fn mock_generate_withdrawal_proof(
    ...
    // Build the public values that the wrap circuit would commit.
...
/// (`sp1Proof` is the deployed ABI's parameter name; it carries whatever wrap
/// prover's Groth16 bytes — the name changes when the verifier contract does.)
///
...
/// Encode the wrapped proof as the vault contract expects: abi.encode(proofBytes, publicValues).
#[cfg_attr(not(feature = "mock"), allow(dead_code))]
...
pub fn verify_withdrawal_proof_locally(proof: &WithdrawalProof) -> Result<bool, ChainError> {
    ...
    let values: WithdrawalPublicValues = bincode::deserialize(&proof.evm_proof.public_values)
        .map_err(|e| ChainError::InvalidProof(format!("cannot decode public values: {e}")))?;

    // Check consistency.
    if values.nullifier != proof.nullifier {
        return Err(ChainError::InvalidProof("nullifier mismatch".to_string()));
    }
    ...
    if values.recipient != proof.recipient {
        return Err(ChainError::InvalidProof("recipient mismatch".to_string()));
    }
...
mod tests {
    ...
    fn test_withdrawal_nullifier_domain_separated_from_in_protocol() {
        ...
        let in_protocol_zero = blake3::derive_key("dregg-note nullifier v1", &input_zero_nonce);
        assert_ne!(
