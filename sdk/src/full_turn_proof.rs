...
fn prove_cohort_run_chain(
    ...
    for (k, run) in runs.iter().enumerate() {
        ...
        let (proof_bytes, rot_pi, vk_hash) = if let Some(cap) = cap_open_run {
            ...
            // The vk_hash MUST pin the EFFECTIVE descriptor (the write wrapper when the c-list was
            // threaded), not the authority-only `route.key` — the proof is descriptor-bound.
            let vk_hash = cap_open_vk_hash_by_key(cap_open_effective_key(&route, cap))?;
...
mod tests {
    ...
    /// binds the BEFORE cap-root (col 65) → AFTER cap-root (col 87) via a genuine sorted-Poseidon2
    /// cap-tree write. The IR-v2 prover realizes that `map_op` against a witness HEAP whose root
