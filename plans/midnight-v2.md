...
- `bridge/src/midnight.rs` — Level 1 attestation bridge (25 tests)
  ...
  - PyanaToMidnight / MidnightToPyana message types
...
- `pyana-dsl-tests/src/fri_verifier_dsl.rs` — FRI verifier prototype (29 cols, 7 tests)
...
**What it gives us:** Token bridging (lock on pyana → attest → mint on Midnight, and reverse).

### Level 2: Proof-Carrying Bridge (DESIGNED, PROTOTYPE EXISTS)
**Trust model:** Pyana sends a PROOF that Midnight verifies in-circuit.
**Security:** Computational soundness of the STARK (no federation trust for safety).

**Feasible path:** Wrap pyana STARK into BLS12-381 PLONK externally, then Midnight's
...
**What it gives us:** A pyana capability proof IS a valid Midnight contract call.

A `CircuitDescriptor` compiles to:
- BabyBear STARK (pyana-native, fast proving)
...
**Trust model:** A proof on pyana is a VALID INPUT to a Midnight computation.
**What it gives us:** "I proved on pyana I'm authorized" → feeds into a Midnight
...
- [ ] End-to-end test: lock on pyana → attest → unlock on Midnight mock
...
- [ ] Compile a simple pyana predicate (e.g., "balance >= threshold") via gen_midnight.rs → ZKIR
...
- [ ] Pyana capability proof → shared Poseidon commitment → Midnight contract input
- [ ] "Prove on pyana you're authorized to spend" → "Midnight moves shielded tokens"
- [ ] Compose: pyana Effect VM proof + Midnight ZSwap proof + shared commitment binding
- [ ] Full privacy: pyana hides WHO is authorized, Midnight hides HOW MUCH moves

## What Pyana Adds to Midnight's Ecosystem

Midnight has shielded VALUE transfer. Pyana has private AUTHORIZATION.

| Midnight alone | Pyana + Midnight |
|----------------|-----------------|
...
The deepest integration: Midnight's DeFi contracts gain pyana's authorization model.
A Midnight DEX could require a pyana capability proof to trade — proving you're an
authorized participant without revealing who you are or how you got the authorization.

## What Midnight Adds to Pyana's Ecosystem
...
Pyana is a COORDINATION LAYER. Sovereign state, consensus only when needed.
For 90%+ of pyana operations (single-owner, bilateral), a blockchain is overkill.
Use Midnight for SETTLEMENT and VALUE. Use pyana for AUTHORIZATION and EXECUTION.
