...
- **Our approach (recommended):** Use the existing gossip model (intents are already public with anonymous creators), but have matching happen at the *federation level* (not individual cipherclerks). The federation nodes collectively run the cycle-detection algorithm on the public intent pool. This works because:
