...
unsafe fn fw_cfg_dma(
    ...
    loop {
        ...
        if ctl & !FW_CFG_DMA_CTL_ERROR == 0 {
            if ctl & FW_CFG_DMA_CTL_ERROR != 0 {
                debug_println!("[deos-tutorial]   WARN fw_cfg DMA ERROR bit");
            }
            break;
        }
        core::hint::spin_loop();
...
pub fn configure_ramfb() -> bool {
    ...
    let select = match unsafe { find_ramfb_select(scratch_v, scratch_p) } {
        Some(s) => {
            debug_println!(
                "[deos-tutorial]   fw_cfg 'etc/ramfb' selector = {:#06x}",
                s
            );
            s
        }
        None => {
            debug_println!("[deos-tutorial]   ERROR 'etc/ramfb' not found (is -device ramfb present?)");
            return false;
        }
    };
    ...
    debug_println!(
        "[deos-tutorial]   ramfb CONFIGURED: addr={:#x} XRGB8888 {}x{} stride={}",
        fb_p,
        WIDTH,
        HEIGHT,
        STRIDE
