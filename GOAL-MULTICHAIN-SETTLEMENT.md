...
- ⚠ FLAGGED (pre-existing, off-wire, NOT green-hacked): ir2_umem_double_spend_refuses FAILS — the STAGED umem-cohort raw prove_vm_descriptor2_inner ACCEPTS an intra-proof double-spend (pre-flight wrapper refuses; deployed default never calls it). A staged-path soundness gap to close before that path deploys. + vk_epoch_misc_light_client_binding test doesn't compile (Faithful8 vs [u8;32] drift).
