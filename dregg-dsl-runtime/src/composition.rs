...
pub fn compose_and(
    ...
    let main_circuit = CircuitDescriptor {
        name: format!("dregg-and-{}-{}-v1", circuit_a.name, circuit_b.name),
...
pub fn compose_or(
        ...
        ConstraintExpr::Gated {
            ...
            inner: Box::new(ConstraintExpr::Polynomial {
                terms: vec![
                    ...
                    PolyTerm {
                        coeff: BabyBear::new(dregg_circuit::field::BABYBEAR_P - 1),
                        col_indices: vec![],
                    },
                ],
            }),
    ...
    let main_circuit = CircuitDescriptor {
        name: format!("dregg-or-{}-{}-v1", circuit_a.name, circuit_b.name),
        trace_width: main_width,
        max_degree: 2,
        columns,
        constraints,
...
pub fn compose_chain(circuits: &[&CircuitDescriptor]) -> ComposedCircuitDescriptor {
    ...
    let main_circuit = CircuitDescriptor {
        name: "dregg-chain-composition-v1".into(),
        trace_width: main_width,
...
pub fn compose_aggregate(circuits: &[&CircuitDescriptor]) -> ComposedCircuitDescriptor {
    ...
    let main_circuit = CircuitDescriptor {
        name: "dregg-aggregate-composition-v1".into(),
        trace_width: main_width,
        max_degree: 1,
        columns,
        constraints,
...
pub fn compute_descriptor_vk_elements(descriptor: &CircuitDescriptor) -> [BabyBear; VK_HASH_WIDTH] {
    ...
    for i in 0..VK_HASH_WIDTH {
        ...
        elements[i] = BabyBear::new(val % dregg_circuit::field::BABYBEAR_P);
    }
    elements
}
