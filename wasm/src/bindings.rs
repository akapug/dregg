...
// ---------------------------------------------------------------------------
// Cell program view types — shared with the node (the ONE total projection).
//
// The view enum + the total `to_view` conversions live in
// `dregg_cell::program` so the wasm runtime, the node's cell endpoint, and
// any future serving surface project the SAME self-describing shape, and so
// adding a `StateConstraint` variant without a view projection is a compile
// error there (the live-view seam cannot silently reopen). Re-exported here
// for existing `dregg_wasm::bindings::CellProgramView` consumers.
// ---------------------------------------------------------------------------

pub use dregg_cell::program::{
    CellProgramView, StateConstraintView, TransitionCaseView, TransitionGuardView,
};
