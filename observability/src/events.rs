...
fn constraint_dissect(
    ...
    match constraint {
        ...
        SC::FieldLteOther { index, other, .. } => ("field_lte_other", Some(*index), vec![*other]),
        ...
        SC::PrefixOf { seg_indices, .. } => (
            "prefix_of",
            seg_indices.first().copied(),
            seg_indices.iter().skip(1).copied().collect(),
        ),
        ...
        SC::ObservedFieldEquals { local_field, .. } => {
            ("observed_field_equals", Some(*local_field), vec![])
        }
