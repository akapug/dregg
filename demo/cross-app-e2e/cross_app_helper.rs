...
//! # Why this exists
//!
//! `demo/cross-app-e2e/REAL-VERSION.md` documents the gap: the existing
//! Python harness (`verify.py`) re-derives BLAKE3 commitments from JSON
//! artifacts but never calls `EmbeddedExecutor::submit_action`, never
//! verifies receipts, and never invokes `pyana-verifier`. Every per-app
//! `integration_*.rs` test now drives the executor in isolation (commit
//! `d235a86b`), but the cross-app composition is still in Python helper
//! land.
//!
//! This binary closes that loop: each of the seven story steps invokes
//! the relevant app's `build_*_action`, drives it through an
//! `EmbeddedExecutor`, captures the resulting `TurnReceipt`, and emits
//! a JSON artifact under `state/`. The downstream `verify_real.py`
//! consumes those artifacts, hands the chain to `pyana-verifier
//! replay-chain` (per-agent receipt-chain integrity), and asserts the
//! cross-app links (event-data references between apps) hold.
//!
//! # What this is NOT
//!
//! - **STARK proofs:** `EmbeddedExecutor` is the app-framework's
//!   embedded path; it does not run the prover. STARK proofs are
//!   generated by `pyana-node`'s MCP layer
//!   (`generate_effect_vm_proof`) which has no app-aware builders yet
//!   (no `pyana_register_name`, no `pyana_publish_subscription` MCP
//!   tools). Closing that gap is its own lane; this helper drives the
//!   executor + receipt-chain layer end-to-end without a node.
//!
