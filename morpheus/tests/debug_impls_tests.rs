...
fn test_format_functions() {
    ...
    pyana_morpheus::protocol_log!("Processing view change to {}", format_view_num(&view_num));

    // Block log
    pyana_morpheus::block_log!(&block);
    pyana_morpheus::block_log!(&block, true); // Verbose

    // Vote log
    pyana_morpheus::vote_log!(&vote_data);

    // QC log
    pyana_morpheus::qc_log!(&thresh_signed_vote);

    // Message log
    for msg in &messages {
        pyana_morpheus::message_log!(msg);
    }
    pyana_morpheus::message_log!(&messages[0], true); // Verbose
}
