...
## ⚑⚑ `gen_plonky3` DELETED (`43b930241` + the swept `67bd6af79`) — a backend that EMITTED UNCOMPILABLE CODE
ember: "we should be deleting some of this old crap." **Proven, not assumed** — and the proof exists ONLY
because of the `unexpected_cfgs` flip. The chain, worth remembering:
1. rustc's authoritative phantom list = 16 always-false `cfg(feature="plonky3")` gates in `dregg-dsl-tests`,
   GENERATED by `#[dregg_caveat]` (grep found nothing — the cfg is emitted by the proc-macro; rustc points at
   the invocation). **My "~11 across 4 crates" static estimate was noise; rustc's list was the truth.**
2. NO consumer ever declared `plonky3` — not even the DSL's own compile-test crate. So I declared it, to
   compile the codegen for the first time ever.
3. **It failed instantly**: the emitted `{Name}P3Air` targets an OBSOLETE Plonky3 API
   (`MainWindow::current_slice()`, `AB::Expr::ONE` sans trait). **Broken for who-knows-how-long because
   NOTHING EVER COMPILED IT.** Its 4 `falsifier_*` tests check emitted TOKEN STRUCTURE — which cannot detect
   API drift in code nobody builds (same species as a soundness suite over a mock: passes forever).
4. Its agreement vote was always a fiction — the old doc SAID SO honestly: the differential harness never
   instantiated the emitted P3Air; it hand-builds a descriptor and agrees with a RE-DERIVED MIRROR.
**I nearly deleted this for the WRONG reason ("dead codegen") and the doc stopped me — then declaring the
feature proved the STRONGER charge. Reading beat inferring; compiling beat reading.**
Agreement set 5 -> 4 (losing nothing real). Harness keeps its plonky3 lane (it drives PRODUCTION `dsl_p3_air`
— the right thing to validate). Verified green.
**A CLAIM DIED WITH IT, said out loud:** the "Range-check soundness" doc promised `<=`/`>=`/`in_range!`
bit-decomposition range checks — that code lived ONLY in gen_plonky3, so **the soundness was never real in
any built artifact**. Comparisons lower via `gen_air` auxiliary columns; whether THAT is field-wrap sound is
NAMED not claimed: `DslComparisonRangeSoundnessResidual`.

### ⚠ THE `git add -A` HAZARD — THIRD CASUALTY THIS SESSION
`67bd6af79` (an UNRELATED crypto commit) swept my staged `gen_plonky3.rs` deletion into itself, leaving HEAD
