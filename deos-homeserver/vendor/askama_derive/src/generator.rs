...
impl<'a, 'h> Generator<'a, 'h> {
    fn new(
        ...
        Self {
            ...
            // deos-homeserver patch (see vendor/askama_derive/PATCH.md): start
            // Invalid instead of Unresolved so `caller_dir()` always yields None
            // and `rel_path` returns the ABSOLUTE template / askama.toml path
            // unchanged. Upstream resolves a caller-relative path from
            // `Span::call_site().local_file()`, which on this toolchain reports
            // the macro *definition* site — continuwuity wraps
            // `#[derive(Template)]` inside a `template!` macro_rules — while
            // rustc resolves the emitted `include_bytes!` against the
            // *invocation* site, so the relative path is off by the invocation
            // dir depth (e.g. `pages/account/../askama.toml`, a missing file).
            // Absolute `include_bytes!` paths are change-tracked identically, so
            // this drops only upstream's path prettiness, never correctness.
            caller_dir: CallerDir::Invalid,
        }
    }
