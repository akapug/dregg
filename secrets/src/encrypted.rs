...
impl EncryptedFileStore {
  ...
  fn write_file(&self, path: &Path, data: &[u8]) -> Result<(), SecretStoreError> {
    ...
    tmp.persist(path).map_err(|e| SecretStoreError::Io(e.error))?;
    Ok(())
  }
}
...
impl SecretStore for EncryptedFileStore {
  fn put(&self, id: &SecretId, value: &[u8]) -> Result<(), SecretStoreError> {
    ...
    let meta = if existing_meta.exists() {
      ...
      let mut m: SecretMetadata =
        serde_json::from_str(&raw).map_err(|e| SecretStoreError::Serialization(e.to_string()))?;
      m.updated_at = now;
      m
    } else {
      SecretMetadata {
    ...
    let meta_json =
      serde_json::to_string(&meta).map_err(|e| SecretStoreError::Serialization(e.to_string()))?;
