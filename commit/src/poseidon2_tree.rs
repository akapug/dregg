...
mod tests {
    ...
    fn end_to_end_note_spending_stark_from_real_tree() {
        ...
        let result = verify_note_spend(nullifier, tree_root, &stark_proof);
        assert!(
            result.is_ok(),
        ...
        assert!(
            verify_note_spend(wrong_nullifier, tree_root, &stark_proof).is_err(),
            "Should reject wrong nullifier"
        );
        let wrong_root = BabyBear::new(0xBAD);
        assert!(
            verify_note_spend(nullifier, wrong_root, &stark_proof).is_err(),
