...
pub const RELAY_OPERATOR_FACTORY_VK: [u8; 32] = *b"dregg-storage-tpl-relayop-factor";
...
pub fn register(ctx: &StarbridgeAppContext) -> [u8; 32] {
    ...
    ctx.register_inspector(InspectorDescriptor {
        ...
        descriptor: serde_json::json!({
            "component": "dregg-relay-operator",
            "module": "/dregg-storage-templates/relay-operator.js",
...
mod tests {
    ...
    fn descriptor_validates_against_canonical_program() {
        ...
        dregg_app_framework::validate_child_vk_canonical(&d, &relay_operator_program())
    ...
    fn relay_case_carries_dfa_predicate() {
        ...
        let has_dfa = relay.constraints.iter().any(|c| {
            matches!(c, StateConstraint::Witnessed { wp } if matches!(
                wp.kind,
                dregg_cell::predicate::WitnessedPredicateKind::Dfa
            ))
        });
