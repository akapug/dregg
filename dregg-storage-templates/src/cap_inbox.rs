...
//! ## What this replaces
//!
//! - `dregg_storage::inbox::CapInbox` operator-process state machine.
...
pub const CAP_INBOX_FACTORY_VK: [u8; 32] = *b"dregg-storage-tpl-capinbox-fact!";
...
pub fn cap_inbox_program() -> CellProgram {
    CellProgram::Cases(vec![
        ...
        // exactly +1, and the dequeue path is gated at the cclerk
        // layer on tail < head).
        // ──────────────────────────────────────────────────────────
        TransitionCase {
            guard: TransitionGuard::MethodIs {
...
pub fn build_send_action(
    ...
    cclerk.make_action(inbox_cell, "send", effects)
}
...
pub fn build_dequeue_action(
    ...
    cclerk.make_action(inbox_cell, "dequeue", effects)
}
...
pub fn build_grant_sender_action(
    ...
    cclerk.make_action(inbox_cell, "grant_sender", effects)
}
...
pub fn register(ctx: &StarbridgeAppContext) -> [u8; 32] {
    ...
    ctx.register_inspector(InspectorDescriptor {
        ...
        descriptor: serde_json::json!({
            "component": "dregg-cap-inbox",
            "module": "/dregg-storage-templates/cap-inbox.js",
...
mod tests {
    ...
    fn descriptor_validates_against_canonical_program() {
        let d = cap_inbox_factory_descriptor();
        dregg_app_framework::validate_child_vk_canonical(&d, &cap_inbox_program())
    ...
    fn send_action_shape() {
        ...
        let action = build_send_action(&cclerk, cell, new_head, new_total, new_root, commitment);
