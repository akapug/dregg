//! **The powerbox, welded onto the real `dreggnet-webauth` cap rail.**
//!
//! The prototype models a powerbox capability as an in-crate HMAC-sealed
//! [`crate::powerbox::SturdyRef`] (the design twin). The **deployed** path is the
//! real DreggNet cap rail: a capability is a real [`dreggnet_webauth::cred::Credential`]
//! — an ed25519 caveat-chain token (`dga1_…`, the same wire as `dregg-auth`),
//! rooted at the host's [`dreggnet_webauth::cred::RootKey`], that *only ever
//! attenuates* and is re-verified cryptographically on every present. This module
//! is the weld: it mints, attenuates, and reads a grain capability **on the real
//! rail**, so the powerbox grant a grain presents is a genuine DreggNet credential
//! the gateway's `webauth` forward-auth already understands.
//!
//! The mapping from Sandstorm's powerbox onto the rail:
//!
//! | powerbox / SturdyRef notion          | real `dreggnet-webauth` realization                 |
//! |--------------------------------------|-----------------------------------------------------|
//! | the host seal secret (`SealKey`)     | the host [`RootKey`] (the authority root keypair)   |
...
//!
//! Every red-team property the in-crate model proves holds here on the real rail,
//! and now *cryptographically*: a **forged** credential (not signed by the host
//! root) fails the ed25519 chain verify; a **leaked** credential presented by a
//! non-owner fails the `subject` caveat; a credential for **another grain** fails
//! the `grain` caveat; **amplification** is impossible because [`Credential::attenuate`]
//! only appends caveats (the facet set can only shrink, never grow).
...
impl HostAuthority {
    ...
    /// `Effect::GrantCapability` artifact on the DreggNet side (the witnessed kernel
