...
//!
//! Running [Sandstorm.io](https://sandstorm.org) apps as dregg cells on DreggNet.
//! The crate models the keystone mappings the integration plan
//! (`../docs/SANDSTORM-INTEGRATION-PLAN.md`) turns on, as plain Rust + tests, so the
//! design is *exercised* (with real `.spk` crypto) before the production weld onto
//! the real `dreggnet-exec` / `webauth` / breadstuffs `dregg-cell` crates:
//!
...
//! 6. [`powerbox`] — Sandstorm's **powerbox** = dregg's attenuable caps: a typed
//!    `PowerboxDescriptor` compiles to a [`powerbox::Pred`] (the picker filter); the
//!    grant is a strictly-attenuating, *witnessed* turn; a `SturdyRef` is a
//!    [`powerbox::SturdyRef`] (`dga1_…`) sealed to its owner (a leaked token is inert).
//!
...
pub use powerbox::{
    compile, present, Caveat, DreggCapRef, PowerboxDescriptor, PowerboxGrant, PowerboxRequest,
    Pred, RestoreError, SealKey, SturdyRef, Tag,
};
pub use spk::{Archive, File, FileContent, Spk, SpkBuilder, SpkError};
pub use tenant::{TenantError, TenantId, TenantRegistry};

// --- the production weld surfaces (the prototype→devnet welds) ---
pub use exec_workload::{cap_tier_for, ExecGrainWorkload, GrainRun};
pub use serving::{grain_name_for_host, publish_grain_snapshot, serve_grain, GrainSession};
...
mod integration_tests {
    ...
    use crate::powerbox::{DreggCapRef, SealKey};
    use crate::spk::{test_signing_key, File, Spk, SpkBuilder};

    /// The host's seal secret — caps presented to the bridge are host-sealed under it.
    fn host_seal() -> SealKey {
        SealKey::from_secret(b"sandstorm-host-secret")
    }
    ...
    fn install_spk_then_run_grain_and_persist() {
        ...
        //    over the grain, host-sealed to her; her facets become X-Sandstorm-Permissions.
        let seal = host_seal();
        let mut var = Umem::new();
        let alice = Session::sealed(
            "u:alice",
            "alice",
            "s:1",
            &DreggCapRef::new(grain_id, &["view", "edit"]),
            "u:alice",
            &seal,
        );
        ...
        grain.sleep(served.new_data_root.0.clone()).unwrap();
        ...
        let bob = Session::sealed(
            "u:bob",
            "bob",
            "s:2",
            &DreggCapRef::new(grain_id, &["view"]),
            "u:bob",
            &seal,
        );
    ...
    fn a_hostile_grain_is_refused_at_every_layer() {
        ...
        let _exposure = OverlayExposure::expose("mallory.example.com", grain_id);
        ...
        assert!(!HttpBridge::egress(&grain.network, "mallory.example.com", 443).is_allowed());
        ...
        //   Even a *genuinely host-sealed* cap over the victim grain (which mallory may
        //   legitimately hold) confers nothing here: the target does not match.
        let seal = host_seal();
        let cross = Session::sealed(
            "u:mallory",
            "mallory",
            "s:x",
            &DreggCapRef::new("cell:victim-secret", &["view", "edit"]),
            "u:mallory",
            &seal,
        );
        let mut var = Umem::new();
        ...
        //   L7 — and a *forged* cap (not sealed by this host) over the grain itself is
        //   refused outright: the bridge authenticates the seal, not the struct.
        let forged_over_self = {
            let attacker = SealKey::from_secret(b"mallory-no-host-secret");
            Session {
                user_id: "u:mallory".into(),
                username: "mallory".into(),
                session_id: "s:f".into(),
                sturdyref: DreggCapRef::new(grain_id, &["view", "edit"])
                    .seal_for("u:mallory", &attacker),
