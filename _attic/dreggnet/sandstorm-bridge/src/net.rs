...
pub struct OverlayExposure {
    ...
    /// Sandstorm's per-session wildcard host, e.g. `<name>.example.com`).
...
mod tests {
    ...
    fn exposed_grain_still_cannot_reach_out() {
        ...
        let net = NetworkPolicy::confined();
        let exposure = OverlayExposure::expose("notes.example.com", "cell:grain1");

        // The grain's bridge is reachable on the overlay...
        assert_eq!(exposure.overlay_name, "notes.example.com");
        ...
        assert!(!net.has_any_egress());
        assert_eq!(
            net.check_outbound("notes.example.com", 443),
            EgressDecision::DeniedNoCap
        );
        assert_eq!(
            net.check_outbound("another-grain.example.com", 443),
            EgressDecision::DeniedNoCap
        );
    }
