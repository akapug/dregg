//! `dreggnet-webauth` — dregg-cap-based forward-auth for the DreggNet web edge.
//!
//! Replaces the legacy HTTP basic-auth on the gated surfaces (ops `:8090`,
//! grafana `:3000`, the gateway `/admin`) with **dregg capability** auth: a user
//! holds an attenuable, offline-verifiable credential (a `dga1_…` token, minted
//! by the root authority into their cipherclerk wallet); Caddy's `forward_auth`
//! directive calls this service once per request; the service verifies the
//! presented credential against the *required capability* for that surface and
//! returns `2xx` (+ identity headers) or `401`. No password anywhere.
//!
//! ## The pieces
//! - [`cred`] — the faithful, wire-compatible port of the dregg-auth credential
//!   core (ed25519 caveat-chain, no-amplify attenuation, `dga1_` wire form).
//! - [`grant`] — the web-surface capability vocabulary (`ops-admin`,
//!   `grafana-view`, `gateway-admin`) over that core.
//! - [`config::WebAuthConfig`] — runtime config (root pubkey, host→cap map, the
//!   break-glass override).
//! - [`decide`] — the per-request authorization decision Caddy's `forward_auth`
//!   rides.
//! - [`server`] — the pure-std HTTP server exposing `/auth`, `/login`,
//!   `/logout`, `/healthz`.
//!
//! ## Honest scope
//! This is the **code-proven** half: the cap-verify + attenuation + the
//! forward-auth decision + the login flow, all unit/integration tested. The
//! **live-edge rewire** (pointing the production Caddy at this service, dropping
//! the basic-auth blocks) is a deploy step — see `deploy/staging/Caddyfile`'s
//! commented `forward_auth` blocks and keep the break-glass override set until
//! the cap flow is exercised in production.
...
use cred::{Credential, PublicKey};

/// The outcome of a forward-auth decision.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum Verdict {
    /// Admit. Carries the identity headers to echo back to Caddy (so the
...
pub fn decide(cfg: &WebAuthConfig, input: &AuthInput) -> Verdict {
    ...
    // 5a. Revocation deny-set (Tier 0 compromise response) — the cloud-side
    //     `Effect::RevokeCapability`. A revoked credential is refused even though
    //     its signature + caps would otherwise admit it: a leaked token can be
    //     proactively killed (by tail) and a compromised account can be killed
    //     wholesale (by subject) without waiting for the expiry caveat. Revoked =
    //     UNAUTHENTICATED (401): the session is dead, present a live one.
...
/// header echoed to the upstream surface — and the key every consumer
/// (`org`/`dregg-secrets`/`console`/`guard`/`billing`) scopes a resource to.
///
...
/// (`dregg_types::CellId::derive_raw` over the *inception* key), so a DreggNet
/// account and its rotatable substrate identity cell are the same principal.
///
...
pub fn subject_of_credential(cred: &Credential) -> Option<String> {
    ...
    Some(format!("dregg:{}", &cred::hex(&cred.tail())[..16]))
}
...
mod tests {
    ...
    /// deny-set: it was admitting, and now it is refused — the cloud-side
    /// `Effect::RevokeCapability`. A DIFFERENT token for the same account still
    /// admits (tail-keying kills exactly one session).
    #[test]
    ...
    fn reanchored_subject_is_stable_across_reissue() {
        ...
        assert_ne!(
            cred::Credential::decode(&s1).unwrap().tail_hex(),
            cred::Credential::decode(&s2).unwrap().tail_hex()
        );
    }
    ...
    fn legacy_credential_keeps_tail_subject() {
        ...
        assert_eq!(subj, format!("dregg:{}", &cred::hex(&legacy.tail())[..16]));
