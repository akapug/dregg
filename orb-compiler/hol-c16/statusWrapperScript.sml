...
open statusCoreTheory;      (* statusCore, statusClass, evaluate_statusCore_framed, statusCore_noFFI *)
open statusLinkBInstTheory; (* statusClassProg, statusClassProg_def *)

val _ = new_theory "statusWrapper";

(* the staged control block the load_vec oracle establishes:
   [0)=code, [8)=result slot.  ONE input word, NO arena (no loop => no memRel). *)
Definition statusCtrlStaged_def:
  statusCtrlStaged (code:num) (ba:word64) (s:(64,'ffi) panSem$state) <=>
...
(* THE SINGLE NAMED TRUSTED ASSUMPTION — the FFI-oracle contract (same shape as
   step/boundScan): (L) @load_vec stages the control block per statusCtrlStaged;
   (R) @report_vec emits the result word onto the observable FFI trace. *)
Definition statusFFI_def:
  statusFFI (code:num) (s0:(64,'ffi) panSem$state) <=>
    (!(s:(64,'ffi) panSem$state). s.base_addr = s0.base_addr /\
       ...
       ?s1 loadEv.
         ...
         statusCtrlStaged code s.base_addr s1 /\
...
(* statusMainBody = the VERBATIM body of «main» in statusClassProg (the parser
   output), with the C15 `statusCore` constant folded in for its decision `If`.
   BUILT BY ML SURGERY from `functions statusClassProg` (no hand transcription):
   extract the «main» body, then substitute the emitted If-node (= rhs of
   statusCore_def) by the constant `statusCore`.  So statusMainBody IS the
   parser output modulo the statusCore abbreviation. *)
val funcs_body = (REWRITE_CONV [statusClassProg_def] THENC EVAL)
                   “functions statusClassProg” |> concl |> rhs;
...
val coreTm      = rhs (concl statusCore_def);
val body_core   = Term.subst [coreTm |-> “statusCore”] body64;
val statusMainBody_def =
  new_definition("statusMainBody_def", “statusMainBody = ^body_core”);

(* sanity: the substitution actually fired (statusCore occurs in the folded body) *)
val _ = if Term.free_in “statusCore” body_core then ()
        else raise Fail "statusCore substitution did not fire";
