(* ===========================================================================
   ...
   admit.pnk back into HOL as a concrete literal, and scope the Link-B
   antecedent `compile_prog_max c mc admitProg = SOME(bytes,...)` on
   ...
   ONE real stage (admit) and reports precisely where it stands.

   Theory contents:
     admitProg_def / admitProg_is_parser_output
         admitProg = OUTL(parse_topdecs_to_ast <admit.pnk>) — the
         CakeML-verified Pancake parser's output (leanc's text->AST out of TCB),
         identical to the C10/C11 pinning.

     admitBytes_def / admitBitmaps_def
         The native run's `.byte`/`.quad` output, read at THEORY-BUILD TIME by
         invoking the bootstrapped `cake --pancake` on admit.pnk and parsing
         the emitted assembly.  admitBytes : word8 list is the concrete HOL
         term for the machine code; admitBitmaps : word64 list the data.
     ...
     admitBytes_length / admitBitmaps_length  (kernel-checked, EVAL)
     ...
     LAYER 1 (kernel-checked, NO new oracle — an instance of the proven Link-B
              theorem):
     admit_pan_to_target_specialised
         pan_to_targetProof$pan_to_target_compile_semantics specialised to
         `:64`, admitProg, and the concrete native admitBytes/
         admitBitmaps in the `bytes`/`bitmaps` slots.  This is the REAL
     ...
     admit_compile_prog_native
         ∃c'. pan_to_target$compile_prog x64_config x64_backend_config
                  admitProg = SOME(admitBytes, admitBitmaps, c')
   ...
   packagings on admitProg, plus the standard runtime install package.
   =========================================================================== *)
...
val _ = new_theory "admitBytesBridge";

(* ---------------------------------------------------------------------------
   (R1) admitProg — BY CONSTRUCTION the verified parser's output on the
   leanc-emitted admit.pnk (identical to C10 §R1).
   --------------------------------------------------------------------------- *)
val src_string =
  let val is = TextIO.openIn "admit.pnk"
...
val admitProg_def =
  new_definition("admitProg_def", “admitProg = ^prog_tm”);

Theorem admitProg_is_parser_output:
  parse_topdecs_to_ast ^srcTm = INL admitProg
Proof
  REWRITE_TAC[admitProg_def] \\ MATCH_ACCEPT_TAC parse_thm
QED

(* ---------------------------------------------------------------------------
   The native run: invoke the bootstrapped `cake` on admit.pnk (if the
   emitted assembly is not already present) and read the code/data back into HOL.
   --------------------------------------------------------------------------- *)
val _ =
  if OS.FileSys.access ("admit.S", []) then ()
  else ignore (OS.Process.system
    "/home/hbox/r05/cake-x64-64/cake --pancake < admit.pnk > admit.S");
...
val admitBytes_def =
  new_definition("admitBytes_def", “admitBytes = ^bytesListTm”);
val admitBitmaps_def =
  new_definition("admitBitmaps_def", “admitBitmaps = ^bmListTm”);

Theorem admitBytes_length:
  LENGTH admitBytes = ^(numSyntax.term_of_int (length byteInts))
Proof
  REWRITE_TAC[admitBytes_def] \\ EVAL_TAC
QED

Theorem admitBitmaps_length:
  LENGTH admitBitmaps = ^(numSyntax.term_of_int (length bmInts))
Proof
  REWRITE_TAC[admitBitmaps_def] \\ EVAL_TAC
QED

(* ---------------------------------------------------------------------------
   LAYER 1 — the reflected bytes occupy the REAL Link-B `bytes` slot.
   Specialise pan_to_target_compile_semantics to :64, admitProg, and the
   concrete native literals.  A genuine instance of the proven theorem (NOT
   vacuous): its conclusion is the machine_sem refinement for admitProg,
   conditional on the named antecedents (which now mention the concrete
   admitBytes, not a bound variable).
   --------------------------------------------------------------------------- *)
val admitProg64 =
  Term.inst [Type.alpha |-> “:64”]
            (prim_mk_const {Name = "admitProg", Thy = "admitBytesBridge"});

val admit_pan_to_target_specialised =
  save_thm("admit_pan_to_target_specialised",
    pan_to_target_compile_semantics
      |> INST_TYPE [Type.alpha |-> “:64”]
      |> INST [ mk_var ("pan_code", “:64 panLang$decl list”) |-> admitProg64,
                mk_var ("bytes",    “:word8 list”)           |-> “admitBytes”,
                mk_var ("bitmaps",  “:word64 list”)          |-> “admitBitmaps” ]);

(* ---------------------------------------------------------------------------
   LAYER 2 — the native-bootstrap reflection (oracle `cake_native_bootstrap`).
   The exact function the `cake` binary runs under --pancake, on admitProg,
...
val native_eq_tm =
  “∃c'. pan_to_target$compile_prog x64_target$x64_config
           x64_config$x64_backend_config admitProg =
             SOME (admitBytes, admitBitmaps, c')”;

val admit_compile_prog_native =
  save_thm("admit_compile_prog_native",
...
val _ =
  ...
  in
    ...
    p ("--- admitBytes_length ---\n" ^ thm_to_string admitBytes_length ^ "\n");
    p ("--- admitBitmaps_length ---\n" ^ thm_to_string admitBitmaps_length ^ "\n");
    p ("--- admit_compile_prog_native (LAYER 2, oracle) ---\n"
       ^ thm_to_string admit_compile_prog_native ^ "\n");
    let val (orc1, ax1) = Tag.dest_tag (Thm.tag admit_compile_prog_native)
    in p ("LAYER2 oracles = [" ^ String.concatWith "," orc1 ^ "]  axioms = ["
          ^ String.concatWith "," ax1 ^ "]\n") end;
    let val (orc2, ax2) = Tag.dest_tag (Thm.tag admit_pan_to_target_specialised)
        val c = concl admit_pan_to_target_specialised
        val ante1 = c |> dest_imp |> #1 |> strip_conj |> hd
    in p ("--- admit_pan_to_target_specialised (LAYER 1) ---\n");
    ...
    p ("theory axioms (admitBytesBridge) = "
       ^ Int.toString (length (axioms "admitBytesBridge")) ^ "\n");
    TextIO.closeOut os
  end;
