(* ===========================================================================
   C30 — THE FINAL COMPOSITION: spec -> machine code for the constant-secheaders
   TRANSFORM stage's response-BYTE effect.  Composes the whole-program Link-A
   wrapper (transformInstall) with the CakeML backend Link-B
   (transformProg_linkB): every observable behaviour of the installed x64 machine
   code emitted for copy.pnk is the single terminating trace whose reported FFI
   payload is EXACTLY `MAP n2w secHeadersBytes` — the deployed 159-byte serialized
   security-header block, byte for byte.  leanc is OUT of the TCB.
   =========================================================================== *)
open HolKernel boolLib bossLib Parse;
open arithmeticTheory listTheory wordsTheory finite_mapTheory optionTheory;
open panLangTheory panSemTheory;
open transformLinkBInstTheory transformSecHeadersTheory transformWrapperTheory
     transformInstallTheory;

val _ = new_theory "transformEndToEnd";

val linkB_ant    = transformProg_linkB |> concl |> dest_imp |> #1;
val linkB_concl  = transformProg_linkB |> concl |> dest_imp |> #2;
...
val e2e_goal =
  mk_imp (list_mk_conj [pkg_tm,
                        “transformFFI secHeadersBytes (s:(64,'ffi) panSem$state)”,
...
Theorem secheaders_bytes_machine_code = prove (e2e_goal,
  strip_tac >>
  mp_tac (SPEC_ALL transformProg_semantics_decls) >>
  ...
  `^subset_sd`
     by (match_mp_tac transformProg_linkB >> rpt conj_tac >>
