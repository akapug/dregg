...
(* C30: the SAME instance on the GENUINE PARSED loop body `copyLoopA` — the
   emitted loop writes EXACTLY the deployed security-header bytes into the output
   buffer.  leanc OUT of the TCB (copyLoopA is the verified-parser while body). *)
Theorem secheaders_copyLoopA_writes:
  copyInv secHeadersBytes src out 0 s /\ LENGTH secHeadersBytes <= s.clock ==>
    ?s'. evaluate (copyLoopA, s) = (NONE, s') /\
         ...
         FLOOKUP s'.locals «out» = SOME (ValWord out)
Proof
  strip_tac >> metis_tac [copyLoopA_writes]
QED
