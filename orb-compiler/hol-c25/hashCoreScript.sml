(* ===========================================================================
   C21 (carried from C20) — the DEPLOYED cache-key-hash LOOP CORE, on the GENUINE
   parsed while body.  `hashBodyA` is the verbatim Annot-wrapped `while` body the
   CakeML-verified parser emits for hashbytes.pnk (leanc out of TCB); `hashLoopCore`
   is the emitted `While foldGuard hashBodyA`.  The C16 fold-loop schema + the C19
   Nat->word homomorphism close it to `n2w (hashBytesN input)` (the deployed hash
   mod 2^64) with an ~8-line per-step fill-in, and `While_frame` gives the `ctrl`
   locals-frame the whole-program wrapper needs after the loop.

   This is the FOLD-SPECIFIC CORE that plugs into `mk_foldWrapper`: it exposes the
   framed core theorem `evaluate_hashLoopCore_framed` + `hashLoopCore_noFFI` that
