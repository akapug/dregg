(* ===========================================================================
   C14 probe, PART C — the semantics CLOCK-LIFT for the step «main».
   Bridges `evaluate (stepMainBody,...)` through the whole-program
   `Call NONE «main» []` and the all-clocks `panSem$semantics` lift.
   NOTE: because the branch-only core consumes NO clock, ANY nonzero clock
   (here K=1) suffices — there is no loop-budget hypothesis.
   =========================================================================== *)
...
open stepWrapperTheory;            (* stepFFI, stepMainBody *)
open stepMainRefineTheory;         (* stepMainBody_refines *)

val _ = new_theory "stepSem";

Theorem call_main_run:
  FLOOKUP (s'':(64,'ffi)panSem$state).code «main» = SOME ([], stepMainBody) /\
  s''.clock <> 0 /\
  stepFFI c b ((dec_clock s'') with locals := FEMPTY) ==>
  ?t loadEv rb.
...
Proof
  ...
  `s0.locals = FEMPTY /\ stepFFI c b s0 /\ s0.ffi = s''.ffi`
     by (fs [Abbr `s0`, dec_clock_def]) >>
  drule_all stepMainBody_refines >> strip_tac >>
  qmatch_asmsub_rename_tac
     `evaluate (stepMainBody, s0) = (SOME (Return (ValWord 0w)), sF)` >>
  `sF.clock <= s0.clock` by (imp_res_tac evaluate_clock >> fs []) >>
  `evaluate (stepMainBody, dec_clock s'' with locals := FEMPTY)
...
Proof
  ...
  `sc.code = s'.code /\ sc.ffi = s'.ffi /\ sc.clock = 1`
     by simp [Abbr `sc`] >>
  `FLOOKUP sc.code «main» = SOME ([], stepMainBody)` by fs [] >>
  `sc.clock <> 0` by fs [] >>
  `stepFFI c b ((dec_clock sc) with locals := FEMPTY)`
