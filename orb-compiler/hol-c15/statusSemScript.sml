(* ===========================================================================
   ...
   Bridges `evaluate (statusMainBody,...)` through the whole-program
...
open statusWrapperTheory;          (* statusFFI, statusMainBody *)
open statusMainRefineTheory;       (* statusMainBody_refines *)

val _ = new_theory "statusSem";

Theorem call_main_run:
  FLOOKUP (s'':(64,'ffi)panSem$state).code «main» = SOME ([], statusMainBody) /\
  s''.clock <> 0 /\
  statusFFI code ((dec_clock s'') with locals := FEMPTY) ==>
  ?t loadEv rb.
...
Proof
  ...
  `s0.locals = FEMPTY /\ statusFFI code s0 /\ s0.ffi = s''.ffi`
     by (fs [Abbr `s0`, dec_clock_def]) >>
  drule_all statusMainBody_refines >> strip_tac >>
  qmatch_asmsub_rename_tac
     `evaluate (statusMainBody, s0) = (SOME (Return (ValWord 0w)), sF)` >>
  `sF.clock <= s0.clock` by (imp_res_tac evaluate_clock >> fs []) >>
  `evaluate (statusMainBody, dec_clock s'' with locals := FEMPTY)
...
Proof
  ...
  `sc.code = s'.code /\ sc.ffi = s'.ffi /\ sc.clock = 1`
     by simp [Abbr `sc`] >>
  `FLOOKUP sc.code «main» = SOME ([], statusMainBody)` by fs [] >>
  `sc.clock <> 0` by fs [] >>
  `statusFFI code ((dec_clock sc) with locals := FEMPTY)`
