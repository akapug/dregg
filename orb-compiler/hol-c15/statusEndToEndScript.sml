(* ===========================================================================
   ...
   statusClassProg_semantics_decls (whole-program Link A) with
   statusClassProg_linkB (Link B backend) into ONE kernel-checked theorem: every
   ...
   Lean spec `C15.statusClass` value `n2w (statusClass code)`.  The install-
   package hypotheses are taken VERBATIM from statusClassProg_linkB; the only
   extra hypothesis is the single FFI-oracle contract `statusFFI`.
   =========================================================================== *)
open HolKernel boolLib bossLib Parse;
open arithmeticTheory listTheory wordsTheory finite_mapTheory optionTheory;
open panLangTheory panSemTheory;
open statusLinkBInstTheory;    (* statusClassProg, statusClassProg_linkB *)
open statusCoreTheory;         (* statusClass *)
open statusWrapperTheory;      (* statusFFI *)
open statusInstallTheory;      (* statusClassProg_semantics_decls *)

val _ = new_theory "statusEndToEnd";

val linkB_ant    = statusClassProg_linkB |> concl |> dest_imp |> #1;
val linkB_concl  = statusClassProg_linkB |> concl |> dest_imp |> #2;
