(* ===========================================================================
   ...
   cachefresh.pnk back into HOL as a concrete literal, and scope the Link-B
   antecedent `compile_prog_max c mc cachefreshProg = SOME(bytes,...)` on
   ...
   ONE real stage (cachefresh) and reports precisely where it stands.

   Theory contents:
     cachefreshProg_def / cachefreshProg_is_parser_output
         cachefreshProg = OUTL(parse_topdecs_to_ast <cachefresh.pnk>) — the
         CakeML-verified Pancake parser's output (leanc's text->AST out of TCB),
         identical to the C10/C11 pinning.

     cachefreshBytes_def / cachefreshBitmaps_def
         The native run's `.byte`/`.quad` output, read at THEORY-BUILD TIME by
         invoking the bootstrapped `cake --pancake` on cachefresh.pnk and parsing
         the emitted assembly.  cachefreshBytes : word8 list is the concrete HOL
         term for the machine code; cachefreshBitmaps : word64 list the data.
     ...
     cachefreshBytes_length / cachefreshBitmaps_length  (kernel-checked, EVAL)
     ...
     LAYER 1 (kernel-checked, NO new oracle — an instance of the proven Link-B
              theorem):
     cachefresh_pan_to_target_specialised
         pan_to_targetProof$pan_to_target_compile_semantics specialised to
         `:64`, cachefreshProg, and the concrete native cachefreshBytes/
         cachefreshBitmaps in the `bytes`/`bitmaps` slots.  This is the REAL
     ...
     cachefresh_compile_prog_native
         ∃c'. pan_to_target$compile_prog x64_config x64_backend_config
                  cachefreshProg = SOME(cachefreshBytes, cachefreshBitmaps, c')
   ...
   packagings on cachefreshProg, plus the standard runtime install package.
   =========================================================================== *)
...
val _ = new_theory "cachefreshBytesBridge";

(* ---------------------------------------------------------------------------
   (R1) cachefreshProg — BY CONSTRUCTION the verified parser's output on the
   leanc-emitted cachefresh.pnk (identical to C10 §R1).
   --------------------------------------------------------------------------- *)
val src_string =
  let val is = TextIO.openIn "cachefresh.pnk"
...
val cachefreshProg_def =
  new_definition("cachefreshProg_def", “cachefreshProg = ^prog_tm”);

Theorem cachefreshProg_is_parser_output:
  parse_topdecs_to_ast ^srcTm = INL cachefreshProg
Proof
  REWRITE_TAC[cachefreshProg_def] \\ MATCH_ACCEPT_TAC parse_thm
QED

(* ---------------------------------------------------------------------------
   The native run: invoke the bootstrapped `cake` on cachefresh.pnk (if the
   emitted assembly is not already present) and read the code/data back into HOL.
   --------------------------------------------------------------------------- *)
val _ =
  if OS.FileSys.access ("cachefresh.S", []) then ()
  else ignore (OS.Process.system
    "/home/hbox/r05/cake-x64-64/cake --pancake < cachefresh.pnk > cachefresh.S");
...
val slines = readLines "cachefresh.S";
...
val cachefreshBytes_def =
  new_definition("cachefreshBytes_def", “cachefreshBytes = ^bytesListTm”);
val cachefreshBitmaps_def =
  new_definition("cachefreshBitmaps_def", “cachefreshBitmaps = ^bmListTm”);

Theorem cachefreshBytes_length:
  LENGTH cachefreshBytes = ^(numSyntax.term_of_int (length byteInts))
Proof
  REWRITE_TAC[cachefreshBytes_def] \\ EVAL_TAC
QED

Theorem cachefreshBitmaps_length:
  LENGTH cachefreshBitmaps = ^(numSyntax.term_of_int (length bmInts))
Proof
  REWRITE_TAC[cachefreshBitmaps_def] \\ EVAL_TAC
QED

(* ---------------------------------------------------------------------------
   LAYER 1 — the reflected bytes occupy the REAL Link-B `bytes` slot.
   Specialise pan_to_target_compile_semantics to :64, cachefreshProg, and the
   concrete native literals.  A genuine instance of the proven theorem (NOT
   vacuous): its conclusion is the machine_sem refinement for cachefreshProg,
   conditional on the named antecedents (which now mention the concrete
   cachefreshBytes, not a bound variable).
   --------------------------------------------------------------------------- *)
val cachefreshProg64 =
  Term.inst [Type.alpha |-> “:64”]
            (prim_mk_const {Name = "cachefreshProg", Thy = "cachefreshBytesBridge"});

val cachefresh_pan_to_target_specialised =
  save_thm("cachefresh_pan_to_target_specialised",
    pan_to_target_compile_semantics
      |> INST_TYPE [Type.alpha |-> “:64”]
      |> INST [ mk_var ("pan_code", “:64 panLang$decl list”) |-> cachefreshProg64,
                mk_var ("bytes",    “:word8 list”)           |-> “cachefreshBytes”,
                mk_var ("bitmaps",  “:word64 list”)          |-> “cachefreshBitmaps” ]);

(* ---------------------------------------------------------------------------
   LAYER 2 — the native-bootstrap reflection (oracle `cake_native_bootstrap`).
   The exact function the `cake` binary runs under --pancake, on cachefreshProg,
...
val native_eq_tm =
  “∃c'. pan_to_target$compile_prog x64_target$x64_config
           x64_config$x64_backend_config cachefreshProg =
             SOME (cachefreshBytes, cachefreshBitmaps, c')”;

val cachefresh_compile_prog_native =
  save_thm("cachefresh_compile_prog_native",
...
val _ =
  ...
  in
    ...
    p ("--- cachefreshBytes_length ---\n" ^ thm_to_string cachefreshBytes_length ^ "\n");
    p ("--- cachefreshBitmaps_length ---\n" ^ thm_to_string cachefreshBitmaps_length ^ "\n");
    p ("--- cachefresh_compile_prog_native (LAYER 2, oracle) ---\n"
       ^ thm_to_string cachefresh_compile_prog_native ^ "\n");
    let val (orc1, ax1) = Tag.dest_tag (Thm.tag cachefresh_compile_prog_native)
    in p ("LAYER2 oracles = [" ^ String.concatWith "," orc1 ^ "]  axioms = ["
          ^ String.concatWith "," ax1 ^ "]\n") end;
    let val (orc2, ax2) = Tag.dest_tag (Thm.tag cachefresh_pan_to_target_specialised)
        val c = concl cachefresh_pan_to_target_specialised
        val ante1 = c |> dest_imp |> #1 |> strip_conj |> hd
    in p ("--- cachefresh_pan_to_target_specialised (LAYER 1) ---\n");
    ...
    p ("theory axioms (cachefreshBytesBridge) = "
       ^ Int.toString (length (axioms "cachefreshBytesBridge")) ^ "\n");
    TextIO.closeOut os
  end;
