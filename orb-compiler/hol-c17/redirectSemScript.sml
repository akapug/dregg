(* ===========================================================================
   ...
   Bridges `evaluate (redirectMainBody,...)` through the whole-program
...
open redirectWrapperTheory;          (* redirectFFI, redirectMainBody *)
open redirectMainRefineTheory;       (* redirectMainBody_refines *)

val _ = new_theory "redirectSem";

Theorem call_main_run:
  FLOOKUP (s'':(64,'ffi)panSem$state).code «main» = SOME ([], redirectMainBody) /\
  s''.clock <> 0 /\
  redirectFFI code ((dec_clock s'') with locals := FEMPTY) ==>
  ?t loadEv rb.
...
Proof
  ...
  `s0.locals = FEMPTY /\ redirectFFI code s0 /\ s0.ffi = s''.ffi`
     by (fs [Abbr `s0`, dec_clock_def]) >>
  drule_all redirectMainBody_refines >> strip_tac >>
  qmatch_asmsub_rename_tac
     `evaluate (redirectMainBody, s0) = (SOME (Return (ValWord 0w)), sF)` >>
  `sF.clock <= s0.clock` by (imp_res_tac evaluate_clock >> fs []) >>
  `evaluate (redirectMainBody, dec_clock s'' with locals := FEMPTY)
...
Proof
  ...
  `sc.code = s'.code /\ sc.ffi = s'.ffi /\ sc.clock = 1`
     by simp [Abbr `sc`] >>
  `FLOOKUP sc.code «main» = SOME ([], redirectMainBody)` by fs [] >>
  `sc.clock <> 0` by fs [] >>
  `redirectFFI code ((dec_clock sc) with locals := FEMPTY)`
