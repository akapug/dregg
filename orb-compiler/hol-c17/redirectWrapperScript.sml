...
open redirectCoreTheory;      (* redirectCore, redirectStatus, evaluate_redirectCore_framed, redirectCore_noFFI *)
open redirectLinkBInstTheory; (* redirectStatusProg, redirectStatusProg_def *)

val _ = new_theory "redirectWrapper";

(* the staged control block the load_vec oracle establishes:
   [0)=code, [8)=result slot.  ONE input word, NO arena (no loop => no memRel). *)
Definition redirectCtrlStaged_def:
  redirectCtrlStaged (code:num) (ba:word64) (s:(64,'ffi) panSem$state) <=>
...
(* THE SINGLE NAMED TRUSTED ASSUMPTION — the FFI-oracle contract (same shape as
   step/boundScan): (L) @load_vec stages the control block per redirectCtrlStaged;
   (R) @report_vec emits the result word onto the observable FFI trace. *)
Definition redirectFFI_def:
  redirectFFI (code:num) (s0:(64,'ffi) panSem$state) <=>
    (!(s:(64,'ffi) panSem$state). s.base_addr = s0.base_addr /\
       ...
       ?s1 loadEv.
         ...
         redirectCtrlStaged code s.base_addr s1 /\
...
(* redirectMainBody = the VERBATIM body of «main» in redirectStatusProg (the parser
   output), with the C15 `redirectCore` constant folded in for its decision `If`.
   BUILT BY ML SURGERY from `functions redirectStatusProg` (no hand transcription):
   extract the «main» body, then substitute the emitted If-node (= rhs of
   redirectCore_def) by the constant `redirectCore`.  So redirectMainBody IS the
   parser output modulo the redirectCore abbreviation. *)
val funcs_body = (REWRITE_CONV [redirectStatusProg_def] THENC EVAL)
                   “functions redirectStatusProg” |> concl |> rhs;
...
val coreTm      = rhs (concl redirectCore_def);
val body_core   = Term.subst [coreTm |-> “redirectCore”] body64;
val redirectMainBody_def =
  new_definition("redirectMainBody_def", “redirectMainBody = ^body_core”);

(* sanity: the substitution actually fired (redirectCore occurs in the folded body) *)
val _ = if Term.free_in “redirectCore” body_core then ()
        else raise Fail "redirectCore substitution did not fire";
