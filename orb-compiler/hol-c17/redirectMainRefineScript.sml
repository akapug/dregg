(* ===========================================================================
   ...
   `n2w (redirectStatus code)`.  NO clock precondition (branch-only, no While).
   The forward wrap extracts the emitted Dec/Annot/Seq spine in ML by rand-
   walking `redirectMainBody` and discharges each node with the uniform
...
open redirectCoreTheory;      (* redirectCore, redirectStatus, redirectRel, evaluate_redirectCore_framed, redirectCore_noFFI *)
open redirectWrapperTheory;   (* redirectCtrlStaged, redirectFFI, redirectMainBody *)
open proofManagerLib;
val _ = new_theory "redirectMainRefine";
...
val mbT   = Term.inst [Type.alpha |-> ty64] (rhs (concl redirectMainBody_def));
...
g `redirectFFI code s0 /\ s0.locals = FEMPTY ==>
   ?sF loadEv rb.
     evaluate (redirectMainBody, s0) = (SOME (Return (ValWord 0w)), sF) /\
     sF.ffi.io_events =
       s0.ffi.io_events ++ loadEv ++
       [IO_event (ffi$ExtCall «report_vec»)
          (word_to_bytes (n2w (redirectStatus code) : word64) F) rb]`;
e (strip_tac);
e (qpat_x_assum `redirectFFI _ _` (strip_assume_tac o SIMP_RULE std_ss [redirectFFI_def]));
...
e (`redirectRel code 0w sRz` by (simp [redirectRel_def] >> fs []));
ck "redirectRel-done";
(* ---- redirectCore (NO clock precondition) ---- *)
e (drule evaluate_redirectCore_framed >> strip_tac);
e (qmatch_asmsub_rename_tac `evaluate (redirectCore, sRz) = (NONE, sCore)`);
e (qabbrev_tac `wstar = n2w (redirectStatus code) : word64`);
...
e (`sCore.ffi.io_events = s0.ffi.io_events ++ loadEv`
     by (`sCore.ffi.io_events = sRz.ffi.io_events`
            by (`evaluate (redirectCore, sRz) = (NONE, sCore)` by fs [] >>
                drule noFFI_io_events >> simp [redirectCore_noFFI]) >> fs []));
ck "redirectCore-done";
...
(* ==================== wrap Decs/Annots up to redirectMainBody ==================== *)
...
e (`?sF. evaluate (redirectMainBody, s0) = (SOME (Return (ValWord 0w)), sF) /\ sF.ffi.io_events = tr`
     by (simp [Once redirectMainBody_def] >> annotw));
val _ = print ("\n@@@ redirectMainBody fact built. nGoals=" ^ Int.toString (length (top_goals ()) handle _ => 0) ^ "\n");
...
val redirectMainBody_refines = top_thm ();
val _ = save_thm ("redirectMainBody_refines", redirectMainBody_refines);
val _ = print "\n@@@ redirectMainBody_refines SAVED\n";
val _ = print (thm_to_string redirectMainBody_refines);
val _ = print "\n@@@ TAILDONE\n";
val _ = export_theory ();
