(* ===========================================================================
   ...
   gzipupper.pnk back into HOL as a concrete literal, and scope the Link-B
   antecedent `compile_prog_max c mc gzipupperProg = SOME(bytes,...)` on
   ...
   ONE real stage (gzipupper) and reports precisely where it stands.

   Theory contents:
     gzipupperProg_def / gzipupperProg_is_parser_output
         gzipupperProg = OUTL(parse_topdecs_to_ast <gzipupper.pnk>) — the
         CakeML-verified Pancake parser's output (leanc's text->AST out of TCB),
         identical to the C10/C11 pinning.

     gzipupperBytes_def / gzipupperBitmaps_def
         The native run's `.byte`/`.quad` output, read at THEORY-BUILD TIME by
         invoking the bootstrapped `cake --pancake` on gzipupper.pnk and parsing
         the emitted assembly.  gzipupperBytes : word8 list is the concrete HOL
         term for the machine code; gzipupperBitmaps : word64 list the data.
     ...
     gzipupperBytes_length / gzipupperBitmaps_length  (kernel-checked, EVAL)
     ...
     LAYER 1 (kernel-checked, NO new oracle — an instance of the proven Link-B
              theorem):
     gzipupper_pan_to_target_specialised
         pan_to_targetProof$pan_to_target_compile_semantics specialised to
         `:64`, gzipupperProg, and the concrete native gzipupperBytes/
         gzipupperBitmaps in the `bytes`/`bitmaps` slots.  This is the REAL
     ...
     gzipupper_compile_prog_native
         ∃c'. pan_to_target$compile_prog x64_config x64_backend_config
                  gzipupperProg = SOME(gzipupperBytes, gzipupperBitmaps, c')
   ...
   packagings on gzipupperProg, plus the standard runtime install package.
   =========================================================================== *)
...
val _ = new_theory "gzipupperBytesBridge";

(* ---------------------------------------------------------------------------
   (R1) gzipupperProg — BY CONSTRUCTION the verified parser's output on the
   leanc-emitted gzipupper.pnk (identical to C10 §R1).
   --------------------------------------------------------------------------- *)
val src_string =
  let val is = TextIO.openIn "gzipupper.pnk"
...
val gzipupperProg_def =
  new_definition("gzipupperProg_def", “gzipupperProg = ^prog_tm”);

Theorem gzipupperProg_is_parser_output:
  parse_topdecs_to_ast ^srcTm = INL gzipupperProg
Proof
  REWRITE_TAC[gzipupperProg_def] \\ MATCH_ACCEPT_TAC parse_thm
QED

(* ---------------------------------------------------------------------------
   The native run: invoke the bootstrapped `cake` on gzipupper.pnk (if the
   emitted assembly is not already present) and read the code/data back into HOL.
   --------------------------------------------------------------------------- *)
val _ =
  if OS.FileSys.access ("gzipupper.S", []) then ()
  else ignore (OS.Process.system
    "/home/hbox/r05/cake-x64-64/cake --pancake < gzipupper.pnk > gzipupper.S");
...
val slines = readLines "gzipupper.S";
...
val gzipupperBytes_def =
  new_definition("gzipupperBytes_def", “gzipupperBytes = ^bytesListTm”);
val gzipupperBitmaps_def =
  new_definition("gzipupperBitmaps_def", “gzipupperBitmaps = ^bmListTm”);

Theorem gzipupperBytes_length:
  LENGTH gzipupperBytes = ^(numSyntax.term_of_int (length byteInts))
Proof
  REWRITE_TAC[gzipupperBytes_def] \\ EVAL_TAC
QED

Theorem gzipupperBitmaps_length:
  LENGTH gzipupperBitmaps = ^(numSyntax.term_of_int (length bmInts))
Proof
  REWRITE_TAC[gzipupperBitmaps_def] \\ EVAL_TAC
QED

(* ---------------------------------------------------------------------------
   LAYER 1 — the reflected bytes occupy the REAL Link-B `bytes` slot.
   Specialise pan_to_target_compile_semantics to :64, gzipupperProg, and the
   concrete native literals.  A genuine instance of the proven theorem (NOT
   vacuous): its conclusion is the machine_sem refinement for gzipupperProg,
   conditional on the named antecedents (which now mention the concrete
   gzipupperBytes, not a bound variable).
   --------------------------------------------------------------------------- *)
val gzipupperProg64 =
  Term.inst [Type.alpha |-> “:64”]
            (prim_mk_const {Name = "gzipupperProg", Thy = "gzipupperBytesBridge"});

val gzipupper_pan_to_target_specialised =
  save_thm("gzipupper_pan_to_target_specialised",
    pan_to_target_compile_semantics
      |> INST_TYPE [Type.alpha |-> “:64”]
      |> INST [ mk_var ("pan_code", “:64 panLang$decl list”) |-> gzipupperProg64,
                mk_var ("bytes",    “:word8 list”)           |-> “gzipupperBytes”,
                mk_var ("bitmaps",  “:word64 list”)          |-> “gzipupperBitmaps” ]);

(* ---------------------------------------------------------------------------
   LAYER 2 — the native-bootstrap reflection (oracle `cake_native_bootstrap`).
   The exact function the `cake` binary runs under --pancake, on gzipupperProg,
...
val native_eq_tm =
  “∃c'. pan_to_target$compile_prog x64_target$x64_config
           x64_config$x64_backend_config gzipupperProg =
             SOME (gzipupperBytes, gzipupperBitmaps, c')”;

val gzipupper_compile_prog_native =
  save_thm("gzipupper_compile_prog_native",
...
val _ =
  ...
  in
    ...
    p ("--- gzipupperBytes_length ---\n" ^ thm_to_string gzipupperBytes_length ^ "\n");
    p ("--- gzipupperBitmaps_length ---\n" ^ thm_to_string gzipupperBitmaps_length ^ "\n");
    p ("--- gzipupper_compile_prog_native (LAYER 2, oracle) ---\n"
       ^ thm_to_string gzipupper_compile_prog_native ^ "\n");
    let val (orc1, ax1) = Tag.dest_tag (Thm.tag gzipupper_compile_prog_native)
    in p ("LAYER2 oracles = [" ^ String.concatWith "," orc1 ^ "]  axioms = ["
          ^ String.concatWith "," ax1 ^ "]\n") end;
    let val (orc2, ax2) = Tag.dest_tag (Thm.tag gzipupper_pan_to_target_specialised)
        val c = concl gzipupper_pan_to_target_specialised
        val ante1 = c |> dest_imp |> #1 |> strip_conj |> hd
    in p ("--- gzipupper_pan_to_target_specialised (LAYER 1) ---\n");
    ...
    p ("theory axioms (gzipupperBytesBridge) = "
       ^ Int.toString (length (axioms "gzipupperBytesBridge")) ^ "\n");
    TextIO.closeOut os
  end;
