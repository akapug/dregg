...
open foldWrapCommonTheory foldLoopSchemaTheory;   (* C21: fold-wrapper machinery *)
...
(* =========================================================================
   C21 — THE FOLD-WRAPPER GENERATOR.  `mk_foldWrapper` turns the C20 hand stack
   (the FUEL-BUDGETED whole-program fold wrapper: MainRefine + Sem + Install +
   EndToEnd) into a mechanical GENERATOR.  NO new metatheory: every step is the
   same fixed tactic the C20 hand proof used; the generator only ASSEMBLES the
   proof for the parameters ⟨core-framed thm, ctrlStaged/FFI defs, mainBody
   surgery, spec word, arena/result offsets, prog + Link B⟩.

   The emitted fold spine is fixed (the deployed control-block convention):
     ctrl := @base ;  base := ctrl+arenaOff ;  @load_vec(ctrl,16,base,4096) ;
     len := lds ctrl ;  acc := 0 ;  i := 0 ;  b := 0 ;
     while i < len { <fold body> } ;  st ctrl+koff, acc ;
     @report_vec(ctrl+koff, 8, ctrl, 8) ;  return 0 ;
   so the ML spine peel + the tactic sequence are the C20 proof with the varying
   tokens parameterized.  The fold list is named `input` (the schema convention).
   ========================================================================= *)

(* -- mk_foldMainRefine : the whole-`main` FFI-trace refinement (the hard part).
      Verbatim the C20 `hashMainRefine` tactic script, parameterized. -- *)
fun mk_foldMainRefine
      { prefix       : string,
        ffiName      : string,   ffiDef      : thm,
        ctrlStagedDef : thm,
        arenaOff     : string,   koff        : string,
        specWord     : string,
        coreName     : string,   coreFramed  : thm,   coreNoFFI : thm,
        mainBodyName : string,   mainBodyDef : thm } : thm =
  let
    val ffiApp = ffiName ^ " input"
    val ffiPat = ffiName ^ " _ _"
    ...
    (* ---- the goal ---- *)
    val _ = g (QS (
      ffiApp ^ " s0 /\\ s0.locals = FEMPTY /\\ LENGTH input <= s0.clock ==>\n" ^
      "   ?sF loadEv rb.\n" ^
      "     evaluate (" ^ mainBodyName ^ ", s0) = (SOME (Return (ValWord 0w)), sF) /\\\n" ^
      "     sF.ffi.io_events =\n" ^
      ...
      "          (word_to_bytes (" ^ specWord ^ " : word64) F) rb]"))
    val _ = e strip_tac
    val _ = e (qpat_x_assum (QS ffiPat) (strip_assume_tac o SIMP_RULE std_ss [ffiDef]))
    val _ = e (qabbrev_tac (QS "ba = s0.base_addr"))
    val _ = EB (QS (
       "«ctrl»<>«base» /\\ «ctrl»<>«len» /\\ «ctrl»<>«acc» /\\ «ctrl»<>«i» /\\ «ctrl»<>«b» /\\ " ^
       "«base»<>«len» /\\ «base»<>«acc» /\\ «base»<>«i» /\\ «base»<>«b» /\\ " ^
       "«len»<>«acc» /\\ «len»<>«i» /\\ «len»<>«b» /\\ «acc»<>«i» /\\ «acc»<>«b» /\\ «i»<>«b»")) EVAL_TAC
    val _ = e (qabbrev_tac (QS "sCtrl = s0 with locals := s0.locals |+ («ctrl», ValWord ba)"))
    val _ = e (qabbrev_tac (QS ("sBase = sCtrl with locals := sCtrl.locals |+ («base», ValWord (ba + " ^ arenaOff ^ "))")))
    val _ = EB (QS (
    ...
    val _ = EB (QS (
       "FLOOKUP sBase.locals «ctrl» = SOME (ValWord ba) /\\ " ^
       "FLOOKUP sBase.locals «base» = SOME (ValWord (ba + " ^ arenaOff ^ "))"))
    ...
    val _ = EB (QS (
       "ba IN s1.memaddrs /\\ s1.memory ba = Word (n2w (LENGTH input)) /\\ " ^
       "(ba+" ^ koff ^ ") IN s1.memaddrs /\\ memRel input (ba+" ^ arenaOff ^ ") s1 /\\ " ^
       "LENGTH input < 2n**63 /\\ EVERY (\\x. x<256) input"))
       (fs [ctrlStagedDef])
    val _ = EB (QS (
       "FLOOKUP s1.locals «ctrl» = SOME (ValWord ba) /\\ " ^
       "FLOOKUP s1.locals «base» = SOME (ValWord (ba+" ^ arenaOff ^ "))"))
       ((QS "s1.locals = sBase.locals") by (fs []) >> fs [])
    (* read len; declare acc, i, b *)
    val _ = EB (QS "eval s1 (Load One (Var Local «ctrl»)) = SOME (ValWord (n2w (LENGTH input)))")
       (irule eval_load_ctrlc >> qexists_tac (QS "ba") >> fs [])
    val _ = e (qabbrev_tac (QS "sLen = s1 with locals := s1.locals |+ («len», ValWord (n2w (LENGTH input)))"))
    ...
    val _ = EB (QS (
       "sB0.memory = s1.memory /\\ sB0.memaddrs = s1.memaddrs /\\ sB0.be = s1.be /\\ " ^
       "sB0.structs = s1.structs /\\ sB0.clock = s1.clock /\\ sB0.ffi = s1.ffi /\\ sB0.base_addr = ba"))
       (simp [ABB "sB0", ABB "sI", ABB "sAcc", ABB "sLen"] >> fs [])
    val _ = EB (QS (
       "FLOOKUP sB0.locals «ctrl» = SOME (ValWord ba) /\\ " ^
       "FLOOKUP sB0.locals «base» = SOME (ValWord (ba+" ^ arenaOff ^ ")) /\\ " ^
    ...
    (* foldInv at sB0 *)
    val _ = EB (QS ("memRel input (ba+" ^ arenaOff ^ ") sB0"))
       (gvs [memRel_def, ABB "sB0", ABB "sI", ABB "sAcc", ABB "sLen"] >> fs [memRel_def])
    val _ = EB (QS ("foldInv input (ba+" ^ arenaOff ^ ") 0 0w sB0"))
       (simp [foldInv_def] >> fs [] >> metis_tac [])
    val _ = EB (QS "LENGTH input <= sB0.clock")
       ((QS "sB0.clock = s0.clock") by (fs [] >> (QS "s1.clock = sBase.clock") by fs [] >> fs []) >> fs [])
    (* run the loop core *)
    val _ = e (drule coreFramed >> disch_then drule >> strip_tac)
    val _ = e (qmatch_asmsub_rename_tac (QS ("evaluate (" ^ coreName ^ ", sB0) = (NONE, sCore)")))
    val _ = e (qabbrev_tac (QS ("wstar = " ^ specWord ^ " : word64")))
    val _ = EB (QS "FLOOKUP sCore.locals «acc» = SOME (ValWord wstar)") (fs [ABB "wstar"])
    val _ = EB (QS "FLOOKUP sCore.locals «ctrl» = SOME (ValWord ba)") (fs [])
    val _ = EB (QS "sCore.clock <= sB0.clock") (imp_res_tac evaluate_clock >> fs [])
    val _ = EB (QS "sCore.memaddrs = s1.memaddrs /\\ sCore.base_addr = ba")
       (imp_res_tac evaluate_invariants >> gvs [])
    val _ = EB (QS "sCore.ffi.io_events = s0.ffi.io_events ++ loadEv")
       ((QS "sCore.ffi.io_events = sB0.ffi.io_events")
          by ((QS ("evaluate (" ^ coreName ^ ", sB0) = (NONE, sCore)")) by (fs []) >>
              drule noFFI_io_events >> simp [coreNoFFI]) >>
        (QS "sB0.ffi.io_events = s0.ffi.io_events ++ loadEv") by (fs []) >> fs [])
    (* Store / report / Return *)
    val _ = EB (QS ("(ba + " ^ koff ^ ") IN sCore.memaddrs")) (gvs [])
    val _ = e (qabbrev_tac (QS ("sS = sCore with memory := ((ba + " ^ koff ^ ") =+ Word wstar) sCore.memory")))
    val _ = EB (QS ("evaluate (Store (Op Add [Var Local «ctrl»; Const " ^ koff ^ "]) (Var Local «acc»), sCore) = (NONE, sS)"))
       (simp [ABB "sS"] >> irule evaluate_store_ctrl_acc >> fs [])
    val _ = EB (QS (
       "sS.base_addr = ba /\\ sS.memaddrs = sCore.memaddrs /\\ sS.locals = sCore.locals /\\ " ^
       "sS.clock = sCore.clock /\\ sS.ffi = sCore.ffi")) (simp [ABB "sS"])
    ...
    val _ = e strip_tac
    val _ = e (qmatch_asmsub_rename_tac
                 (QS "evaluate (ExtCall «report_vec» _ _ _ _, sS) = (NONE, sRep)"))
    val _ = EB (QS "sS.ffi.io_events = s0.ffi.io_events ++ loadEv") (fs [])
    ...
    val _ = EB ([QUOTE "evaluate (", ANTIQUOTE REST1,
                 QUOTE ", sCore) = (SOME (Return (ValWord 0w)), empty_locals sRep)"])
                (irule Seq_thread >> qexists_tac (QS "sS") >> fs [])
    val _ = EB ([QUOTE "evaluate (", ANTIQUOTE CORE, QUOTE ", sB0) = (NONE, sCore)"])
                (irule Annot_Seq >> fs [])
    val _ = EB ([QUOTE "evaluate (", ANTIQUOTE RBODY,
                 QUOTE ", sB0) = (SOME (Return (ValWord 0w)), empty_locals sRep)"])
                (irule Seq_thread >> qexists_tac (QS "sCore") >> fs [])
    val _ = EB ([QUOTE "?st. evaluate (", ANTIQUOTE RBODY,
                 QUOTE ", sB0) = (SOME (Return (ValWord 0w)), st) /\\ st.ffi.io_events = tr"])
                (qexists_tac (QS "empty_locals sRep") >> fs [])
    (* wrap Decs/Annots up to mainBody *)
    val _ = EB (QS "s0 with locals := s0.locals |+ («ctrl», ValWord ba) = sCtrl") (simp [ABB "sCtrl"])
    val _ = EB (QS ("sCtrl with locals := sCtrl.locals |+ («base», ValWord (ba + " ^ arenaOff ^ ")) = sBase")) (simp [ABB "sBase"])
    val _ = EB (QS "s1 with locals := s1.locals |+ («len», ValWord (n2w (LENGTH input))) = sLen") (simp [ABB "sLen"])
    ...
    val _ = EB (QS ("eval sCtrl (Op Add [Var Local «ctrl»; Const " ^ arenaOff ^ "]) = SOME (ValWord (ba + " ^ arenaOff ^ "))"))
       (irule eval_ctrl_add >> fs [])
    val _ = EB ([QUOTE "evaluate (", ANTIQUOTE loadSeq, QUOTE ", sBase) = (NONE, s1)"])
                (irule Annot_Seq >> first_assum ACCEPT_TAC)
    (* forward wrap *)
    val decw = fn (vq, steq) =>
      irule Dec_trace >> qexists_tac (QS vq) >> rpt conj_tac >>
    ...
    val seqldw = irule Seq_trace >> qexists_tac (QS "s1") >> rpt conj_tac >>
                 FIRST [ first_assum ACCEPT_TAC, (fs [] >> NO_TAC), (metis_tac []) ]
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE Db,
                 QUOTE ", sI) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"])
                (decw ("ValWord 0w", "sI with locals := _ = sB0"))
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE Ai,
                 QUOTE ", sI) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"]) annotw
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE Di,
                 QUOTE ", sAcc) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"])
                (decw ("ValWord 0w", "sAcc with locals := _ = sI"))
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE Aacc,
                 QUOTE ", sAcc) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"]) annotw
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE Dacc,
                 QUOTE ", sLen) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"])
                (decw ("ValWord 0w", "sLen with locals := _ = sAcc"))
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE Alen,
                 QUOTE ", sLen) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"]) annotw
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE Dlen,
                 QUOTE ", s1) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"])
                (decw ("ValWord (n2w (LENGTH input))", "s1 with locals := _ = sLen"))
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE AL,
                 QUOTE ", s1) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"]) annotw
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE SEQld,
                 QUOTE ", sBase) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"]) seqldw
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE Dbase,
                 QUOTE ", sCtrl) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"])
                (decw ("ValWord (ba + " ^ arenaOff ^ ")", "sCtrl with locals := _ = sBase"))
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE body_c,
                 QUOTE ", sCtrl) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"]) annotw
    val _ = EB ([QUOTE "?st'. evaluate (", ANTIQUOTE Dctrl,
                 QUOTE ", s0) = (SOME (Return (ValWord 0w)), st') /\\ st'.ffi.io_events = tr"])
...
(* -- mk_foldWrapper : MainRefine + Sem + Install + EndToEnd, all generated. -- *)
fun mk_foldWrapper
  ...
  let
    ...
    val ffiApp = ffiName ^ " input"
    (* -- 1. MainRefine (the hard part) -- *)
    val mainRefine = mk_foldMainRefine
      { prefix=prefix, ffiName=ffiName, ffiDef=ffiDef, ctrlStagedDef=ctrlStagedDef,
        arenaOff=arenaOff, koff=koff, specWord=specWord, coreName=coreName,
        coreFramed=coreFramed, coreNoFFI=coreNoFFI,
        mainBodyName=mainBodyName, mainBodyDef=mainBodyDef }
    (* -- 2. Sem: call_main_run + main_semantics (fuel-budgeted) -- *)
    val callMainRun = prove (
      TM (
      ...
      strip_tac >>
      qabbrev_tac (QS "s0 = (dec_clock s'') with locals := FEMPTY") >>
      (QS ("s0.locals = FEMPTY /\\ LENGTH input <= s0.clock /\\ " ^ ffiApp ^ " s0 /\\ s0.ffi = s''.ffi")) by
         (fs [ABB "s0", dec_clock_def]) >>
      drule_all mainRefine >> strip_tac >>
      qmatch_asmsub_rename_tac
         (QS ("evaluate (" ^ mainBodyName ^ ", s0) = (SOME (Return (ValWord 0w)), sF)")) >>
    ...
    val mainSemantics = prove (
      ...
      (QS "LENGTH input <= (dec_clock sc).clock") by (simp [dec_clock_def] >> fs []) >>
      drule_all callMainRun >> strip_tac >>
      qexists_tac (QS "loadEv") >> qexists_tac (QS "rb") >>
    ...
    val semanticsDecls = prove (
      ...
      (QS ("FLOOKUP s'.code «main» = SOME ([], " ^ mainBodyName ^ ")"))
         by (simp [ABB "s'"] >> REWRITE_TAC [progDef] >> EVAL_TAC >>
             REWRITE_TAC (mainBodyDef :: unfoldCore)) >>
      (QS "s'.base_addr = s.base_addr /\\ s'.ffi = s.ffi")
         by (simp [ABB "s'"] >> REWRITE_TAC [progDef] >> EVAL_TAC) >>
      (QS ("!Kc. " ^ ffiApp ^ " ((dec_clock (s' with clock := Kc)) with locals := FEMPTY)"))
         by (gen_tac >> qpat_x_assum (QS (ffiApp ^ " s")) mp_tac >>
    ...
    (* -- 4. EndToEnd: compose with Link B (clean-context side-condition via exists_big) -- *)
    val linkB_ant   = linkB |> concl |> dest_imp |> #1
    val linkB_concl = linkB |> concl |> dest_imp |> #2
    val sd_tm = find_term (fn t => same_const (fst (strip_comb t)) “semantics_decls”
                           handle _ => false) linkB_concl
    ...
    val e2e_goal =
      mk_imp (list_mk_conj [pkg_tm, TM (ffiApp ^ " (s:(64,'ffi) panSem$state)"),
                            TM "?K. 0 < K /\\ LENGTH input < K"],
              mk_exists (TM "loadEv:io_event list",
                mk_exists (TM "rb:(word8 # word8) list", subset_spec)))
    val exists_big = prove (TM "!n:num. ?K. 0 < K /\\ n < K",
                            gen_tac >> qexists_tac (QS "SUC n") >> DECIDE_TAC)
    val machineCode = prove (e2e_goal,
      strip_tac >>
      ...
      (([ANTIQUOTE notFail_tm]) by
         (qpat_x_assum ([QUOTE "", ANTIQUOTE sd_tm, QUOTE " = _"]) (fn th => simp [th]))) >>
      (([ANTIQUOTE subset_sd]) by
         (match_mp_tac linkB >> rpt conj_tac >> first_assum ACCEPT_TAC)) >>
      qpat_x_assum ([QUOTE "", ANTIQUOTE sd_tm, QUOTE " = _"]) (fn th => gvs [th]))
    val _ = save_thm (prefix ^ "_machine_code", machineCode)
  in
    { mainRefine = mainRefine, callMainRun = callMainRun, mainSemantics = mainSemantics,
