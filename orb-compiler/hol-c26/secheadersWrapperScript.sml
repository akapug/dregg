...
open secheadersCoreTheory;      (* secheadersCore, hstsEffective, evaluate_secheadersCore_framed, secheadersCore_noFFI *)
open secheadersLinkBInstTheory; (* secHeadersProg, secHeadersProg_def *)

val _ = new_theory "secheadersWrapper";

(* the staged control block the load_vec oracle establishes:
   [0)=code, [8)=result slot.  ONE input word, NO arena (no loop => no memRel). *)
Definition secheadersCtrlStaged_def:
  secheadersCtrlStaged (code:num) (ba:word64) (s:(64,'ffi) panSem$state) <=>
...
(* THE SINGLE NAMED TRUSTED ASSUMPTION — the FFI-oracle contract (same shape as
   step/boundScan): (L) @load_vec stages the control block per secheadersCtrlStaged;
   (R) @report_vec emits the result word onto the observable FFI trace. *)
Definition secheadersFFI_def:
  secheadersFFI (code:num) (s0:(64,'ffi) panSem$state) <=>
    (!(s:(64,'ffi) panSem$state). s.base_addr = s0.base_addr /\
       ...
       ?s1 loadEv.
         ...
         secheadersCtrlStaged code s.base_addr s1 /\
...
(* secheadersMainBody = the VERBATIM body of «main» in secHeadersProg (the parser
   output), with the C15 `secheadersCore` constant folded in for its decision `If`.
   BUILT BY ML SURGERY from `functions secHeadersProg` (no hand transcription):
   extract the «main» body, then substitute the emitted If-node (= rhs of
   secheadersCore_def) by the constant `secheadersCore`.  So secheadersMainBody IS the
   parser output modulo the secheadersCore abbreviation. *)
val funcs_body = (REWRITE_CONV [secHeadersProg_def] THENC EVAL)
                   “functions secHeadersProg” |> concl |> rhs;
...
val coreTm      = rhs (concl secheadersCore_def);
val body_core   = Term.subst [coreTm |-> “secheadersCore”] body64;
val secheadersMainBody_def =
  new_definition("secheadersMainBody_def", “secheadersMainBody = ^body_core”);

(* sanity: the substitution actually fired (secheadersCore occurs in the folded body) *)
val _ = if Term.free_in “secheadersCore” body_core then ()
        else raise Fail "secheadersCore substitution did not fire";
