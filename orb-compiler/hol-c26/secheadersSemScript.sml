(* ===========================================================================
   ...
   Bridges `evaluate (secheadersMainBody,...)` through the whole-program
...
open secheadersWrapperTheory;          (* secheadersFFI, secheadersMainBody *)
open secheadersMainRefineTheory;       (* secheadersMainBody_refines *)

val _ = new_theory "secheadersSem";

Theorem call_main_run:
  FLOOKUP (s'':(64,'ffi)panSem$state).code «main» = SOME ([], secheadersMainBody) /\
  s''.clock <> 0 /\
  secheadersFFI code ((dec_clock s'') with locals := FEMPTY) ==>
  ?t loadEv rb.
...
Proof
  ...
  `s0.locals = FEMPTY /\ secheadersFFI code s0 /\ s0.ffi = s''.ffi`
     by (fs [Abbr `s0`, dec_clock_def]) >>
  drule_all secheadersMainBody_refines >> strip_tac >>
  qmatch_asmsub_rename_tac
     `evaluate (secheadersMainBody, s0) = (SOME (Return (ValWord 0w)), sF)` >>
  `sF.clock <= s0.clock` by (imp_res_tac evaluate_clock >> fs []) >>
  `evaluate (secheadersMainBody, dec_clock s'' with locals := FEMPTY)
...
Proof
  ...
  `sc.code = s'.code /\ sc.ffi = s'.ffi /\ sc.clock = 1`
     by simp [Abbr `sc`] >>
  `FLOOKUP sc.code «main» = SOME ([], secheadersMainBody)` by fs [] >>
  `sc.clock <> 0` by fs [] >>
  `secheadersFFI code ((dec_clock sc) with locals := FEMPTY)`
