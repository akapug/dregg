(* ===========================================================================
   ...
   `n2w (hstsEffective code)`.  NO clock precondition (branch-only, no While).
   The forward wrap extracts the emitted Dec/Annot/Seq spine in ML by rand-
   walking `secheadersMainBody` and discharges each node with the uniform
...
open secheadersCoreTheory;      (* secheadersCore, hstsEffective, secheadersRel, evaluate_secheadersCore_framed, secheadersCore_noFFI *)
open secheadersWrapperTheory;   (* secheadersCtrlStaged, secheadersFFI, secheadersMainBody *)
open proofManagerLib;
val _ = new_theory "secheadersMainRefine";
...
val mbT   = Term.inst [Type.alpha |-> ty64] (rhs (concl secheadersMainBody_def));
...
g `secheadersFFI code s0 /\ s0.locals = FEMPTY ==>
   ?sF loadEv rb.
     evaluate (secheadersMainBody, s0) = (SOME (Return (ValWord 0w)), sF) /\
     sF.ffi.io_events =
       s0.ffi.io_events ++ loadEv ++
       [IO_event (ffi$ExtCall «report_vec»)
          (word_to_bytes (n2w (hstsEffective code) : word64) F) rb]`;
e (strip_tac);
e (qpat_x_assum `secheadersFFI _ _` (strip_assume_tac o SIMP_RULE std_ss [secheadersFFI_def]));
e (qabbrev_tac `ba = s0.base_addr`);
e (`«base»<>«buf» /\ «base»<>«maxage» /\ «base»<>«result» /\
    «buf»<>«maxage» /\ «buf»<>«result» /\ «maxage»<>«result»` by EVAL_TAC);
...
e (`ba IN s1.memaddrs /\ s1.memory ba = Word (n2w code) /\
    (ba+8w) IN s1.memaddrs /\ code < 9223372036854775808`
     by (fs [secheadersCtrlStaged_def]));
...
e (qabbrev_tac `sVcode = s1 with locals := s1.locals |+ («maxage», ValWord (n2w code))`);
...
e (`FLOOKUP sRz.locals «maxage» = SOME (ValWord (n2w code)) /\
...
e (`secheadersRel code 0w sRz` by (simp [secheadersRel_def] >> fs []));
ck "secheadersRel-done";
(* ---- secheadersCore (NO clock precondition) ---- *)
e (drule evaluate_secheadersCore_framed >> strip_tac);
e (qmatch_asmsub_rename_tac `evaluate (secheadersCore, sRz) = (NONE, sCore)`);
e (qabbrev_tac `wstar = n2w (hstsEffective code) : word64`);
...
e (`sCore.ffi.io_events = s0.ffi.io_events ++ loadEv`
     by (`sCore.ffi.io_events = sRz.ffi.io_events`
            by (`evaluate (secheadersCore, sRz) = (NONE, sCore)` by fs [] >>
                drule noFFI_io_events >> simp [secheadersCore_noFFI]) >> fs []));
ck "secheadersCore-done";
...
(* ==================== wrap Decs/Annots up to secheadersMainBody ==================== *)
...
e (`s1 with locals := s1.locals |+ («maxage», ValWord (n2w code)) = sVcode` by simp [Abbr `sVcode`]);
...
e (`?sF. evaluate (secheadersMainBody, s0) = (SOME (Return (ValWord 0w)), sF) /\ sF.ffi.io_events = tr`
     by (simp [Once secheadersMainBody_def] >> annotw));
val _ = print ("\n@@@ secheadersMainBody fact built. nGoals=" ^ Int.toString (length (top_goals ()) handle _ => 0) ^ "\n");
...
val secheadersMainBody_refines = top_thm ();
val _ = save_thm ("secheadersMainBody_refines", secheadersMainBody_refines);
val _ = print "\n@@@ secheadersMainBody_refines SAVED\n";
val _ = print (thm_to_string secheadersMainBody_refines);
val _ = print "\n@@@ TAILDONE\n";
val _ = export_theory ();
