(* ===========================================================================
   ...
   secHeadersProg_semantics_decls (whole-program Link A) with
   secHeadersProg_linkB (Link B backend) into ONE kernel-checked theorem: every
   ...
   Lean spec `C15.hstsEffective` value `n2w (hstsEffective code)`.  The install-
   package hypotheses are taken VERBATIM from secHeadersProg_linkB; the only
   extra hypothesis is the single FFI-oracle contract `secheadersFFI`.
   =========================================================================== *)
open HolKernel boolLib bossLib Parse;
open arithmeticTheory listTheory wordsTheory finite_mapTheory optionTheory;
open panLangTheory panSemTheory;
open secheadersLinkBInstTheory;    (* secHeadersProg, secHeadersProg_linkB *)
open secheadersCoreTheory;         (* hstsEffective *)
open secheadersWrapperTheory;      (* secheadersFFI *)
open secheadersInstallTheory;      (* secHeadersProg_semantics_decls *)

val _ = new_theory "secheadersEndToEnd";

val linkB_ant    = secHeadersProg_linkB |> concl |> dest_imp |> #1;
val linkB_concl  = secHeadersProg_linkB |> concl |> dest_imp |> #2;
