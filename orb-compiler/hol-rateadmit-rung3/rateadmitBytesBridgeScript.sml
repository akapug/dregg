(* ===========================================================================
   ...
   rateadmit.pnk back into HOL as a concrete literal, and scope the Link-B
   antecedent `compile_prog_max c mc rateadmitProg = SOME(bytes,...)` on
   ...
   ONE real stage (rateadmit) and reports precisely where it stands.

   Theory contents:
     rateadmitProg_def / rateadmitProg_is_parser_output
         rateadmitProg = OUTL(parse_topdecs_to_ast <rateadmit.pnk>) — the
         CakeML-verified Pancake parser's output (leanc's text->AST out of TCB),
         identical to the C10/C11 pinning.

     rateadmitBytes_def / rateadmitBitmaps_def
         The native run's `.byte`/`.quad` output, read at THEORY-BUILD TIME by
         invoking the bootstrapped `cake --pancake` on rateadmit.pnk and parsing
         the emitted assembly.  rateadmitBytes : word8 list is the concrete HOL
         term for the machine code; rateadmitBitmaps : word64 list the data.
     ...
     rateadmitBytes_length / rateadmitBitmaps_length  (kernel-checked, EVAL)
     ...
     LAYER 1 (kernel-checked, NO new oracle — an instance of the proven Link-B
              theorem):
     rateadmit_pan_to_target_specialised
         pan_to_targetProof$pan_to_target_compile_semantics specialised to
         `:64`, rateadmitProg, and the concrete native rateadmitBytes/
         rateadmitBitmaps in the `bytes`/`bitmaps` slots.  This is the REAL
     ...
     rateadmit_compile_prog_native
         ∃c'. pan_to_target$compile_prog x64_config x64_backend_config
                  rateadmitProg = SOME(rateadmitBytes, rateadmitBitmaps, c')
   ...
   packagings on rateadmitProg, plus the standard runtime install package.
   =========================================================================== *)
...
val _ = new_theory "rateadmitBytesBridge";

(* ---------------------------------------------------------------------------
   (R1) rateadmitProg — BY CONSTRUCTION the verified parser's output on the
   leanc-emitted rateadmit.pnk (identical to C10 §R1).
   --------------------------------------------------------------------------- *)
val src_string =
  let val is = TextIO.openIn "rateadmit.pnk"
...
val rateadmitProg_def =
  new_definition("rateadmitProg_def", “rateadmitProg = ^prog_tm”);

Theorem rateadmitProg_is_parser_output:
  parse_topdecs_to_ast ^srcTm = INL rateadmitProg
Proof
  REWRITE_TAC[rateadmitProg_def] \\ MATCH_ACCEPT_TAC parse_thm
QED

(* ---------------------------------------------------------------------------
   The native run: invoke the bootstrapped `cake` on rateadmit.pnk (if the
   emitted assembly is not already present) and read the code/data back into HOL.
   --------------------------------------------------------------------------- *)
val _ =
  if OS.FileSys.access ("rateadmit.S", []) then ()
  else ignore (OS.Process.system
    "/home/hbox/r05/cake-x64-64/cake --pancake < rateadmit.pnk > rateadmit.S");
...
val slines = readLines "rateadmit.S";
...
val rateadmitBytes_def =
  new_definition("rateadmitBytes_def", “rateadmitBytes = ^bytesListTm”);
val rateadmitBitmaps_def =
  new_definition("rateadmitBitmaps_def", “rateadmitBitmaps = ^bmListTm”);

Theorem rateadmitBytes_length:
  LENGTH rateadmitBytes = ^(numSyntax.term_of_int (length byteInts))
Proof
  REWRITE_TAC[rateadmitBytes_def] \\ EVAL_TAC
QED

Theorem rateadmitBitmaps_length:
  LENGTH rateadmitBitmaps = ^(numSyntax.term_of_int (length bmInts))
Proof
  REWRITE_TAC[rateadmitBitmaps_def] \\ EVAL_TAC
QED

(* ---------------------------------------------------------------------------
   LAYER 1 — the reflected bytes occupy the REAL Link-B `bytes` slot.
   Specialise pan_to_target_compile_semantics to :64, rateadmitProg, and the
   concrete native literals.  A genuine instance of the proven theorem (NOT
   vacuous): its conclusion is the machine_sem refinement for rateadmitProg,
   conditional on the named antecedents (which now mention the concrete
   rateadmitBytes, not a bound variable).
   --------------------------------------------------------------------------- *)
val rateadmitProg64 =
  Term.inst [Type.alpha |-> “:64”]
            (prim_mk_const {Name = "rateadmitProg", Thy = "rateadmitBytesBridge"});

val rateadmit_pan_to_target_specialised =
  save_thm("rateadmit_pan_to_target_specialised",
    pan_to_target_compile_semantics
      |> INST_TYPE [Type.alpha |-> “:64”]
      |> INST [ mk_var ("pan_code", “:64 panLang$decl list”) |-> rateadmitProg64,
                mk_var ("bytes",    “:word8 list”)           |-> “rateadmitBytes”,
                mk_var ("bitmaps",  “:word64 list”)          |-> “rateadmitBitmaps” ]);

(* ---------------------------------------------------------------------------
   LAYER 2 — the native-bootstrap reflection (oracle `cake_native_bootstrap`).
   The exact function the `cake` binary runs under --pancake, on rateadmitProg,
...
val native_eq_tm =
  “∃c'. pan_to_target$compile_prog x64_target$x64_config
           x64_config$x64_backend_config rateadmitProg =
             SOME (rateadmitBytes, rateadmitBitmaps, c')”;

val rateadmit_compile_prog_native =
  save_thm("rateadmit_compile_prog_native",
...
val _ =
  ...
  in
    ...
    p ("--- rateadmitBytes_length ---\n" ^ thm_to_string rateadmitBytes_length ^ "\n");
    p ("--- rateadmitBitmaps_length ---\n" ^ thm_to_string rateadmitBitmaps_length ^ "\n");
    p ("--- rateadmit_compile_prog_native (LAYER 2, oracle) ---\n"
       ^ thm_to_string rateadmit_compile_prog_native ^ "\n");
    let val (orc1, ax1) = Tag.dest_tag (Thm.tag rateadmit_compile_prog_native)
    in p ("LAYER2 oracles = [" ^ String.concatWith "," orc1 ^ "]  axioms = ["
          ^ String.concatWith "," ax1 ^ "]\n") end;
    let val (orc2, ax2) = Tag.dest_tag (Thm.tag rateadmit_pan_to_target_specialised)
        val c = concl rateadmit_pan_to_target_specialised
        val ante1 = c |> dest_imp |> #1 |> strip_conj |> hd
    in p ("--- rateadmit_pan_to_target_specialised (LAYER 1) ---\n");
    ...
    p ("theory axioms (rateadmitBytesBridge) = "
       ^ Int.toString (length (axioms "rateadmitBytesBridge")) ^ "\n");
    TextIO.closeOut os
  end;
