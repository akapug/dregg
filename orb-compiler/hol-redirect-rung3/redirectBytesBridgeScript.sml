(* ===========================================================================
   ...
   redirectstatus.pnk back into HOL as a concrete literal, and scope the Link-B
   antecedent `compile_prog_max c mc redirectProg = SOME(bytes,...)` on
   ...
   ONE real stage (redirectstatus) and reports precisely where it stands.

   Theory contents:
     redirectProg_def / redirectProg_is_parser_output
         redirectProg = OUTL(parse_topdecs_to_ast <redirectstatus.pnk>) — the
         CakeML-verified Pancake parser's output (leanc's text->AST out of TCB),
         identical to the C10/C11 pinning.

     redirectBytes_def / redirectBitmaps_def
         The native run's `.byte`/`.quad` output, read at THEORY-BUILD TIME by
         invoking the bootstrapped `cake --pancake` on redirectstatus.pnk and parsing
         the emitted assembly.  redirectBytes : word8 list is the concrete HOL
         term for the machine code; redirectBitmaps : word64 list the data.
     ...
     redirectBytes_length / redirectBitmaps_length  (kernel-checked, EVAL)
     ...
     LAYER 1 (kernel-checked, NO new oracle — an instance of the proven Link-B
              theorem):
     redirect_pan_to_target_specialised
         pan_to_targetProof$pan_to_target_compile_semantics specialised to
         `:64`, redirectProg, and the concrete native redirectBytes/
         redirectBitmaps in the `bytes`/`bitmaps` slots.  This is the REAL
     ...
     redirect_compile_prog_native
         ∃c'. pan_to_target$compile_prog x64_config x64_backend_config
                  redirectProg = SOME(redirectBytes, redirectBitmaps, c')
   ...
   packagings on redirectProg, plus the standard runtime install package.
   =========================================================================== *)
...
val _ = new_theory "redirectBytesBridge";

(* ---------------------------------------------------------------------------
   (R1) redirectProg — BY CONSTRUCTION the verified parser's output on the
   leanc-emitted redirectstatus.pnk (identical to C10 §R1).
   --------------------------------------------------------------------------- *)
val src_string =
  let val is = TextIO.openIn "redirectstatus.pnk"
...
val redirectProg_def =
  new_definition("redirectProg_def", “redirectProg = ^prog_tm”);

Theorem redirectProg_is_parser_output:
  parse_topdecs_to_ast ^srcTm = INL redirectProg
Proof
  REWRITE_TAC[redirectProg_def] \\ MATCH_ACCEPT_TAC parse_thm
QED

(* ---------------------------------------------------------------------------
   The native run: invoke the bootstrapped `cake` on redirectstatus.pnk (if the
   emitted assembly is not already present) and read the code/data back into HOL.
   --------------------------------------------------------------------------- *)
val _ =
  if OS.FileSys.access ("redirectstatus.S", []) then ()
  else ignore (OS.Process.system
    "/home/hbox/r05/cake-x64-64/cake --pancake < redirectstatus.pnk > redirectstatus.S");
...
val slines = readLines "redirectstatus.S";
...
val redirectBytes_def =
  new_definition("redirectBytes_def", “redirectBytes = ^bytesListTm”);
val redirectBitmaps_def =
  new_definition("redirectBitmaps_def", “redirectBitmaps = ^bmListTm”);

Theorem redirectBytes_length:
  LENGTH redirectBytes = ^(numSyntax.term_of_int (length byteInts))
Proof
  REWRITE_TAC[redirectBytes_def] \\ EVAL_TAC
QED

Theorem redirectBitmaps_length:
  LENGTH redirectBitmaps = ^(numSyntax.term_of_int (length bmInts))
Proof
  REWRITE_TAC[redirectBitmaps_def] \\ EVAL_TAC
QED

(* ---------------------------------------------------------------------------
   LAYER 1 — the reflected bytes occupy the REAL Link-B `bytes` slot.
   Specialise pan_to_target_compile_semantics to :64, redirectProg, and the
   concrete native literals.  A genuine instance of the proven theorem (NOT
   vacuous): its conclusion is the machine_sem refinement for redirectProg,
   conditional on the named antecedents (which now mention the concrete
   redirectBytes, not a bound variable).
   --------------------------------------------------------------------------- *)
val redirectProg64 =
  Term.inst [Type.alpha |-> “:64”]
            (prim_mk_const {Name = "redirectProg", Thy = "redirectBytesBridge"});

val redirect_pan_to_target_specialised =
  save_thm("redirect_pan_to_target_specialised",
    pan_to_target_compile_semantics
      |> INST_TYPE [Type.alpha |-> “:64”]
      |> INST [ mk_var ("pan_code", “:64 panLang$decl list”) |-> redirectProg64,
                mk_var ("bytes",    “:word8 list”)           |-> “redirectBytes”,
                mk_var ("bitmaps",  “:word64 list”)          |-> “redirectBitmaps” ]);

(* ---------------------------------------------------------------------------
   LAYER 2 — the native-bootstrap reflection (oracle `cake_native_bootstrap`).
   The exact function the `cake` binary runs under --pancake, on redirectProg,
...
val native_eq_tm =
  “∃c'. pan_to_target$compile_prog x64_target$x64_config
           x64_config$x64_backend_config redirectProg =
             SOME (redirectBytes, redirectBitmaps, c')”;

val redirect_compile_prog_native =
  save_thm("redirect_compile_prog_native",
...
val _ =
  ...
  in
    ...
    p ("--- redirectBytes_length ---\n" ^ thm_to_string redirectBytes_length ^ "\n");
    p ("--- redirectBitmaps_length ---\n" ^ thm_to_string redirectBitmaps_length ^ "\n");
    p ("--- redirect_compile_prog_native (LAYER 2, oracle) ---\n"
       ^ thm_to_string redirect_compile_prog_native ^ "\n");
    let val (orc1, ax1) = Tag.dest_tag (Thm.tag redirect_compile_prog_native)
    in p ("LAYER2 oracles = [" ^ String.concatWith "," orc1 ^ "]  axioms = ["
          ^ String.concatWith "," ax1 ^ "]\n") end;
    let val (orc2, ax2) = Tag.dest_tag (Thm.tag redirect_pan_to_target_specialised)
        val c = concl redirect_pan_to_target_specialised
        val ante1 = c |> dest_imp |> #1 |> strip_conj |> hd
    in p ("--- redirect_pan_to_target_specialised (LAYER 1) ---\n");
    ...
    p ("theory axioms (redirectBytesBridge) = "
       ^ Int.toString (length (axioms "redirectBytesBridge")) ^ "\n");
    TextIO.closeOut os
  end;
