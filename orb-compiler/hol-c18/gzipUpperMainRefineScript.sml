(* ===========================================================================
   ...
   `n2w (gzipUpper code)`.  NO clock precondition (branch-only, no While).
   The forward wrap extracts the emitted Dec/Annot/Seq spine in ML by rand-
   walking `gzipUpperMainBody` and discharges each node with the uniform
...
open gzipUpperCoreTheory;      (* gzipUpperCore, gzipUpper, gzipUpperRel, evaluate_gzipUpperCore_framed, gzipUpperCore_noFFI *)
open gzipUpperWrapperTheory;   (* gzipUpperCtrlStaged, gzipUpperFFI, gzipUpperMainBody *)
open proofManagerLib;
val _ = new_theory "gzipUpperMainRefine";
...
val mbT   = Term.inst [Type.alpha |-> ty64] (rhs (concl gzipUpperMainBody_def));
...
g `gzipUpperFFI code s0 /\ s0.locals = FEMPTY ==>
   ?sF loadEv rb.
     evaluate (gzipUpperMainBody, s0) = (SOME (Return (ValWord 0w)), sF) /\
     sF.ffi.io_events =
       s0.ffi.io_events ++ loadEv ++
       [IO_event (ffi$ExtCall «report_vec»)
          (word_to_bytes (n2w (gzipUpper code) : word64) F) rb]`;
e (strip_tac);
e (qpat_x_assum `gzipUpperFFI _ _` (strip_assume_tac o SIMP_RULE std_ss [gzipUpperFFI_def]));
e (qabbrev_tac `ba = s0.base_addr`);
e (`«base»<>«buf» /\ «base»<>«b» /\ «base»<>«result» /\
    «buf»<>«b» /\ «buf»<>«result» /\ «b»<>«result»` by EVAL_TAC);
...
e (qabbrev_tac `sVcode = s1 with locals := s1.locals |+ («b», ValWord (n2w code))`);
...
e (`FLOOKUP sRz.locals «b» = SOME (ValWord (n2w code)) /\
...
e (`gzipUpperRel code 0w sRz` by (simp [gzipUpperRel_def] >> fs []));
ck "gzipUpperRel-done";
(* ---- gzipUpperCore (NO clock precondition) ---- *)
e (drule evaluate_gzipUpperCore_framed >> strip_tac);
e (qmatch_asmsub_rename_tac `evaluate (gzipUpperCore, sRz) = (NONE, sCore)`);
e (qabbrev_tac `wstar = n2w (gzipUpper code) : word64`);
...
e (`sCore.ffi.io_events = s0.ffi.io_events ++ loadEv`
     by (`sCore.ffi.io_events = sRz.ffi.io_events`
            by (`evaluate (gzipUpperCore, sRz) = (NONE, sCore)` by fs [] >>
                drule noFFI_io_events >> simp [gzipUpperCore_noFFI]) >> fs []));
ck "gzipUpperCore-done";
...
(* ==================== wrap Decs/Annots up to gzipUpperMainBody ==================== *)
...
e (`s1 with locals := s1.locals |+ («b», ValWord (n2w code)) = sVcode` by simp [Abbr `sVcode`]);
...
e (`?sF. evaluate (gzipUpperMainBody, s0) = (SOME (Return (ValWord 0w)), sF) /\ sF.ffi.io_events = tr`
     by (simp [Once gzipUpperMainBody_def] >> annotw));
val _ = print ("\n@@@ gzipUpperMainBody fact built. nGoals=" ^ Int.toString (length (top_goals ()) handle _ => 0) ^ "\n");
...
val gzipUpperMainBody_refines = top_thm ();
val _ = save_thm ("gzipUpperMainBody_refines", gzipUpperMainBody_refines);
val _ = print "\n@@@ gzipUpperMainBody_refines SAVED\n";
val _ = print (thm_to_string gzipUpperMainBody_refines);
val _ = print "\n@@@ TAILDONE\n";
val _ = export_theory ();
