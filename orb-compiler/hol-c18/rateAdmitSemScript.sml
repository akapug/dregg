(* ===========================================================================
   ...
   Bridges `evaluate (rateAdmitMainBody,...)` through the whole-program
...
open rateAdmitWrapperTheory;          (* rateAdmitFFI, rateAdmitMainBody *)
open rateAdmitMainRefineTheory;       (* rateAdmitMainBody_refines *)

val _ = new_theory "rateAdmitSem";

Theorem call_main_run:
  FLOOKUP (s'':(64,'ffi)panSem$state).code «main» = SOME ([], rateAdmitMainBody) /\
  s''.clock <> 0 /\
  rateAdmitFFI code ((dec_clock s'') with locals := FEMPTY) ==>
  ?t loadEv rb.
...
Proof
  ...
  `s0.locals = FEMPTY /\ rateAdmitFFI code s0 /\ s0.ffi = s''.ffi`
     by (fs [Abbr `s0`, dec_clock_def]) >>
  drule_all rateAdmitMainBody_refines >> strip_tac >>
  qmatch_asmsub_rename_tac
     `evaluate (rateAdmitMainBody, s0) = (SOME (Return (ValWord 0w)), sF)` >>
  `sF.clock <= s0.clock` by (imp_res_tac evaluate_clock >> fs []) >>
  `evaluate (rateAdmitMainBody, dec_clock s'' with locals := FEMPTY)
...
Proof
  ...
  `sc.code = s'.code /\ sc.ffi = s'.ffi /\ sc.clock = 1`
     by simp [Abbr `sc`] >>
  `FLOOKUP sc.code «main» = SOME ([], rateAdmitMainBody)` by fs [] >>
  `sc.clock <> 0` by fs [] >>
  `rateAdmitFFI code ((dec_clock sc) with locals := FEMPTY)`
