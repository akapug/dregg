(* ===========================================================================
   ...
   Bridges `evaluate (gzipUpperMainBody,...)` through the whole-program
...
open gzipUpperWrapperTheory;          (* gzipUpperFFI, gzipUpperMainBody *)
open gzipUpperMainRefineTheory;       (* gzipUpperMainBody_refines *)

val _ = new_theory "gzipUpperSem";

Theorem call_main_run:
  FLOOKUP (s'':(64,'ffi)panSem$state).code «main» = SOME ([], gzipUpperMainBody) /\
  s''.clock <> 0 /\
  gzipUpperFFI code ((dec_clock s'') with locals := FEMPTY) ==>
  ?t loadEv rb.
...
Proof
  ...
  `s0.locals = FEMPTY /\ gzipUpperFFI code s0 /\ s0.ffi = s''.ffi`
     by (fs [Abbr `s0`, dec_clock_def]) >>
  drule_all gzipUpperMainBody_refines >> strip_tac >>
  qmatch_asmsub_rename_tac
     `evaluate (gzipUpperMainBody, s0) = (SOME (Return (ValWord 0w)), sF)` >>
  `sF.clock <= s0.clock` by (imp_res_tac evaluate_clock >> fs []) >>
  `evaluate (gzipUpperMainBody, dec_clock s'' with locals := FEMPTY)
...
Proof
  ...
  `sc.code = s'.code /\ sc.ffi = s'.ffi /\ sc.clock = 1`
     by simp [Abbr `sc`] >>
  `FLOOKUP sc.code «main» = SOME ([], gzipUpperMainBody)` by fs [] >>
  `sc.clock <> 0` by fs [] >>
  `gzipUpperFFI code ((dec_clock sc) with locals := FEMPTY)`
