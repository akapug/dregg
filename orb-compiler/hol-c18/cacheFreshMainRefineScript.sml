(* ===========================================================================
   ...
   `n2w (cacheFresh code)`.  NO clock precondition (branch-only, no While).
   The forward wrap extracts the emitted Dec/Annot/Seq spine in ML by rand-
   walking `cacheFreshMainBody` and discharges each node with the uniform
...
open cacheFreshCoreTheory;      (* cacheFreshCore, cacheFresh, cacheFreshRel, evaluate_cacheFreshCore_framed, cacheFreshCore_noFFI *)
open cacheFreshWrapperTheory;   (* cacheFreshCtrlStaged, cacheFreshFFI, cacheFreshMainBody *)
open proofManagerLib;
val _ = new_theory "cacheFreshMainRefine";
...
val mbT   = Term.inst [Type.alpha |-> ty64] (rhs (concl cacheFreshMainBody_def));
...
g `cacheFreshFFI code s0 /\ s0.locals = FEMPTY ==>
   ?sF loadEv rb.
...
e (strip_tac);
e (qpat_x_assum `cacheFreshFFI _ _` (strip_assume_tac o SIMP_RULE std_ss [cacheFreshFFI_def]));
e (qabbrev_tac `ba = s0.base_addr`);
e (`«base»<>«buf» /\ «base»<>«age» /\ «base»<>«result» /\
    «buf»<>«age» /\ «buf»<>«result» /\ «age»<>«result»` by EVAL_TAC);
...
e (`ba IN s1.memaddrs /\ s1.memory ba = Word (n2w code) /\
    (ba+8w) IN s1.memaddrs /\ code < 4294967296`
     by (fs [cacheFreshCtrlStaged_def]));
...
e (qabbrev_tac `sVcode = s1 with locals := s1.locals |+ («age», ValWord (n2w code))`);
...
e (`FLOOKUP sRz.locals «age» = SOME (ValWord (n2w code)) /\
...
e (`cacheFreshRel code 0w sRz` by (simp [cacheFreshRel_def] >> fs []));
ck "cacheFreshRel-done";
(* ---- cacheFreshCore (NO clock precondition) ---- *)
e (drule evaluate_cacheFreshCore_framed >> strip_tac);
e (qmatch_asmsub_rename_tac `evaluate (cacheFreshCore, sRz) = (NONE, sCore)`);
e (qabbrev_tac `wstar = n2w (cacheFresh code) : word64`);
...
e (`sCore.ffi.io_events = s0.ffi.io_events ++ loadEv`
     by (`sCore.ffi.io_events = sRz.ffi.io_events`
            by (`evaluate (cacheFreshCore, sRz) = (NONE, sCore)` by fs [] >>
                drule noFFI_io_events >> simp [cacheFreshCore_noFFI]) >> fs []));
ck "cacheFreshCore-done";
...
(* ==================== wrap Decs/Annots up to cacheFreshMainBody ==================== *)
...
e (`s1 with locals := s1.locals |+ («age», ValWord (n2w code)) = sVcode` by simp [Abbr `sVcode`]);
...
e (`?sF. evaluate (cacheFreshMainBody, s0) = (SOME (Return (ValWord 0w)), sF) /\ sF.ffi.io_events = tr`
     by (simp [Once cacheFreshMainBody_def] >> annotw));
val _ = print ("\n@@@ cacheFreshMainBody fact built. nGoals=" ^ Int.toString (length (top_goals ()) handle _ => 0) ^ "\n");
...
val cacheFreshMainBody_refines = top_thm ();
val _ = save_thm ("cacheFreshMainBody_refines", cacheFreshMainBody_refines);
val _ = print "\n@@@ cacheFreshMainBody_refines SAVED\n";
val _ = print (thm_to_string cacheFreshMainBody_refines);
val _ = print "\n@@@ TAILDONE\n";
val _ = export_theory ();
