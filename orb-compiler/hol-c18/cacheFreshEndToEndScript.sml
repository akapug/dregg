(* ===========================================================================
   ...
   cacheFreshProg_semantics_decls (whole-program Link A) with
   cacheFreshProg_linkB (Link B backend) into ONE kernel-checked theorem: every
   ...
   Lean spec `C15.cacheFresh` value `n2w (cacheFresh code)`.  The install-
   package hypotheses are taken VERBATIM from cacheFreshProg_linkB; the only
   extra hypothesis is the single FFI-oracle contract `cacheFreshFFI`.
   =========================================================================== *)
open HolKernel boolLib bossLib Parse;
open arithmeticTheory listTheory wordsTheory finite_mapTheory optionTheory;
open panLangTheory panSemTheory;
open cacheFreshLinkBInstTheory;    (* cacheFreshProg, cacheFreshProg_linkB *)
open cacheFreshCoreTheory;         (* cacheFresh *)
open cacheFreshWrapperTheory;      (* cacheFreshFFI *)
open cacheFreshInstallTheory;      (* cacheFreshProg_semantics_decls *)

val _ = new_theory "cacheFreshEndToEnd";

val linkB_ant    = cacheFreshProg_linkB |> concl |> dest_imp |> #1;
val linkB_concl  = cacheFreshProg_linkB |> concl |> dest_imp |> #2;
