...
open gzipUpperCoreTheory;      (* gzipUpperCore, gzipUpper, evaluate_gzipUpperCore_framed, gzipUpperCore_noFFI *)
open gzipUpperLinkBInstTheory; (* gzipUpperProg, gzipUpperProg_def *)

val _ = new_theory "gzipUpperWrapper";

(* the staged control block the load_vec oracle establishes:
   [0)=code, [8)=result slot.  ONE input word, NO arena (no loop => no memRel). *)
Definition gzipUpperCtrlStaged_def:
  gzipUpperCtrlStaged (code:num) (ba:word64) (s:(64,'ffi) panSem$state) <=>
...
(* THE SINGLE NAMED TRUSTED ASSUMPTION — the FFI-oracle contract (same shape as
   step/boundScan): (L) @load_vec stages the control block per gzipUpperCtrlStaged;
   (R) @report_vec emits the result word onto the observable FFI trace. *)
Definition gzipUpperFFI_def:
  gzipUpperFFI (code:num) (s0:(64,'ffi) panSem$state) <=>
    (!(s:(64,'ffi) panSem$state). s.base_addr = s0.base_addr /\
       ...
       ?s1 loadEv.
         ...
         gzipUpperCtrlStaged code s.base_addr s1 /\
...
(* gzipUpperMainBody = the VERBATIM body of «main» in gzipUpperProg (the parser
   output), with the C15 `gzipUpperCore` constant folded in for its decision `If`.
   BUILT BY ML SURGERY from `functions gzipUpperProg` (no hand transcription):
   extract the «main» body, then substitute the emitted If-node (= rhs of
   gzipUpperCore_def) by the constant `gzipUpperCore`.  So gzipUpperMainBody IS the
   parser output modulo the gzipUpperCore abbreviation. *)
val funcs_body = (REWRITE_CONV [gzipUpperProg_def] THENC EVAL)
                   “functions gzipUpperProg” |> concl |> rhs;
...
val coreTm      = rhs (concl gzipUpperCore_def);
val body_core   = Term.subst [coreTm |-> “gzipUpperCore”] body64;
val gzipUpperMainBody_def =
  new_definition("gzipUpperMainBody_def", “gzipUpperMainBody = ^body_core”);

(* sanity: the substitution actually fired (gzipUpperCore occurs in the folded body) *)
val _ = if Term.free_in “gzipUpperCore” body_core then ()
        else raise Fail "gzipUpperCore substitution did not fire";
