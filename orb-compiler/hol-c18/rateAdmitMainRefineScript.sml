(* ===========================================================================
   ...
   `n2w (rateAdmit code)`.  NO clock precondition (branch-only, no While).
   The forward wrap extracts the emitted Dec/Annot/Seq spine in ML by rand-
   walking `rateAdmitMainBody` and discharges each node with the uniform
...
open rateAdmitCoreTheory;      (* rateAdmitCore, rateAdmit, rateAdmitRel, evaluate_rateAdmitCore_framed, rateAdmitCore_noFFI *)
open rateAdmitWrapperTheory;   (* rateAdmitCtrlStaged, rateAdmitFFI, rateAdmitMainBody *)
open proofManagerLib;
val _ = new_theory "rateAdmitMainRefine";
...
val mbT   = Term.inst [Type.alpha |-> ty64] (rhs (concl rateAdmitMainBody_def));
...
g `rateAdmitFFI code s0 /\ s0.locals = FEMPTY ==>
   ?sF loadEv rb.
     evaluate (rateAdmitMainBody, s0) = (SOME (Return (ValWord 0w)), sF) /\
     sF.ffi.io_events =
       s0.ffi.io_events ++ loadEv ++
       [IO_event (ffi$ExtCall «report_vec»)
          (word_to_bytes (n2w (rateAdmit code) : word64) F) rb]`;
e (strip_tac);
e (qpat_x_assum `rateAdmitFFI _ _` (strip_assume_tac o SIMP_RULE std_ss [rateAdmitFFI_def]));
e (qabbrev_tac `ba = s0.base_addr`);
e (`«base»<>«buf» /\ «base»<>«tokens» /\ «base»<>«result» /\
    «buf»<>«tokens» /\ «buf»<>«result» /\ «tokens»<>«result»` by EVAL_TAC);
...
e (`ba IN s1.memaddrs /\ s1.memory ba = Word (n2w code) /\
    (ba+8w) IN s1.memaddrs /\ code < 4294967296`
     by (fs [rateAdmitCtrlStaged_def]));
...
e (qabbrev_tac `sVcode = s1 with locals := s1.locals |+ («tokens», ValWord (n2w code))`);
...
e (`FLOOKUP sRz.locals «tokens» = SOME (ValWord (n2w code)) /\
...
e (`rateAdmitRel code 0w sRz` by (simp [rateAdmitRel_def] >> fs []));
ck "rateAdmitRel-done";
(* ---- rateAdmitCore (NO clock precondition) ---- *)
e (drule evaluate_rateAdmitCore_framed >> strip_tac);
e (qmatch_asmsub_rename_tac `evaluate (rateAdmitCore, sRz) = (NONE, sCore)`);
e (qabbrev_tac `wstar = n2w (rateAdmit code) : word64`);
...
e (`sCore.ffi.io_events = s0.ffi.io_events ++ loadEv`
     by (`sCore.ffi.io_events = sRz.ffi.io_events`
            by (`evaluate (rateAdmitCore, sRz) = (NONE, sCore)` by fs [] >>
                drule noFFI_io_events >> simp [rateAdmitCore_noFFI]) >> fs []));
ck "rateAdmitCore-done";
...
(* ==================== wrap Decs/Annots up to rateAdmitMainBody ==================== *)
...
e (`s1 with locals := s1.locals |+ («tokens», ValWord (n2w code)) = sVcode` by simp [Abbr `sVcode`]);
...
e (`?sF. evaluate (rateAdmitMainBody, s0) = (SOME (Return (ValWord 0w)), sF) /\ sF.ffi.io_events = tr`
     by (simp [Once rateAdmitMainBody_def] >> annotw));
val _ = print ("\n@@@ rateAdmitMainBody fact built. nGoals=" ^ Int.toString (length (top_goals ()) handle _ => 0) ^ "\n");
...
val rateAdmitMainBody_refines = top_thm ();
val _ = save_thm ("rateAdmitMainBody_refines", rateAdmitMainBody_refines);
val _ = print "\n@@@ rateAdmitMainBody_refines SAVED\n";
val _ = print (thm_to_string rateAdmitMainBody_refines);
val _ = print "\n@@@ TAILDONE\n";
val _ = export_theory ();
