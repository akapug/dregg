(* ===========================================================================
   ...
   cors.pnk back into HOL as a concrete literal, and scope the Link-B
   antecedent `compile_prog_max c mc corsProg = SOME(bytes,...)` on
   ...
   ONE real stage (cors) and reports precisely where it stands.

   Theory contents:
     corsProg_def / corsProg_is_parser_output
         corsProg = OUTL(parse_topdecs_to_ast <cors.pnk>) — the
         CakeML-verified Pancake parser's output (leanc's text->AST out of TCB),
         identical to the C10/C11 pinning.

     corsBytes_def / corsBitmaps_def
         The native run's `.byte`/`.quad` output, read at THEORY-BUILD TIME by
         invoking the bootstrapped `cake --pancake` on cors.pnk and parsing
         the emitted assembly.  corsBytes : word8 list is the concrete HOL
         term for the machine code; corsBitmaps : word64 list the data.
     ...
     corsBytes_length / corsBitmaps_length  (kernel-checked, EVAL)
     ...
     LAYER 1 (kernel-checked, NO new oracle — an instance of the proven Link-B
              theorem):
     cors_pan_to_target_specialised
         pan_to_targetProof$pan_to_target_compile_semantics specialised to
         `:64`, corsProg, and the concrete native corsBytes/
         corsBitmaps in the `bytes`/`bitmaps` slots.  This is the REAL
     ...
     cors_compile_prog_native
         ∃c'. pan_to_target$compile_prog x64_config x64_backend_config
                  corsProg = SOME(corsBytes, corsBitmaps, c')
   ...
   packagings on corsProg, plus the standard runtime install package.
   =========================================================================== *)
...
val _ = new_theory "corsBytesBridge";

(* ---------------------------------------------------------------------------
   (R1) corsProg — BY CONSTRUCTION the verified parser's output on the
   leanc-emitted cors.pnk (identical to C10 §R1).
   --------------------------------------------------------------------------- *)
val src_string =
  let val is = TextIO.openIn "cors.pnk"
...
val corsProg_def =
  new_definition("corsProg_def", “corsProg = ^prog_tm”);

Theorem corsProg_is_parser_output:
  parse_topdecs_to_ast ^srcTm = INL corsProg
Proof
  REWRITE_TAC[corsProg_def] \\ MATCH_ACCEPT_TAC parse_thm
QED

(* ---------------------------------------------------------------------------
   The native run: invoke the bootstrapped `cake` on cors.pnk (if the
   emitted assembly is not already present) and read the code/data back into HOL.
   --------------------------------------------------------------------------- *)
val _ =
  if OS.FileSys.access ("cors.S", []) then ()
  else ignore (OS.Process.system
    "/home/hbox/r05/cake-x64-64/cake --pancake < cors.pnk > cors.S");
...
val corsBytes_def =
  new_definition("corsBytes_def", “corsBytes = ^bytesListTm”);
val corsBitmaps_def =
  new_definition("corsBitmaps_def", “corsBitmaps = ^bmListTm”);

Theorem corsBytes_length:
  LENGTH corsBytes = ^(numSyntax.term_of_int (length byteInts))
Proof
  REWRITE_TAC[corsBytes_def] \\ EVAL_TAC
QED

Theorem corsBitmaps_length:
  LENGTH corsBitmaps = ^(numSyntax.term_of_int (length bmInts))
Proof
  REWRITE_TAC[corsBitmaps_def] \\ EVAL_TAC
QED

(* ---------------------------------------------------------------------------
   LAYER 1 — the reflected bytes occupy the REAL Link-B `bytes` slot.
   Specialise pan_to_target_compile_semantics to :64, corsProg, and the
   concrete native literals.  A genuine instance of the proven theorem (NOT
   vacuous): its conclusion is the machine_sem refinement for corsProg,
   conditional on the named antecedents (which now mention the concrete
   corsBytes, not a bound variable).
   --------------------------------------------------------------------------- *)
val corsProg64 =
  Term.inst [Type.alpha |-> “:64”]
            (prim_mk_const {Name = "corsProg", Thy = "corsBytesBridge"});

val cors_pan_to_target_specialised =
  save_thm("cors_pan_to_target_specialised",
    pan_to_target_compile_semantics
      |> INST_TYPE [Type.alpha |-> “:64”]
      |> INST [ mk_var ("pan_code", “:64 panLang$decl list”) |-> corsProg64,
                mk_var ("bytes",    “:word8 list”)           |-> “corsBytes”,
                mk_var ("bitmaps",  “:word64 list”)          |-> “corsBitmaps” ]);

(* ---------------------------------------------------------------------------
   LAYER 2 — the native-bootstrap reflection (oracle `cake_native_bootstrap`).
   The exact function the `cake` binary runs under --pancake, on corsProg,
...
val native_eq_tm =
  “∃c'. pan_to_target$compile_prog x64_target$x64_config
           x64_config$x64_backend_config corsProg =
             SOME (corsBytes, corsBitmaps, c')”;

val cors_compile_prog_native =
  save_thm("cors_compile_prog_native",
...
val _ =
  ...
  in
    ...
    p ("--- corsBytes_length ---\n" ^ thm_to_string corsBytes_length ^ "\n");
    p ("--- corsBitmaps_length ---\n" ^ thm_to_string corsBitmaps_length ^ "\n");
    p ("--- cors_compile_prog_native (LAYER 2, oracle) ---\n"
       ^ thm_to_string cors_compile_prog_native ^ "\n");
    let val (orc1, ax1) = Tag.dest_tag (Thm.tag cors_compile_prog_native)
    in p ("LAYER2 oracles = [" ^ String.concatWith "," orc1 ^ "]  axioms = ["
          ^ String.concatWith "," ax1 ^ "]\n") end;
    let val (orc2, ax2) = Tag.dest_tag (Thm.tag cors_pan_to_target_specialised)
        val c = concl cors_pan_to_target_specialised
        val ante1 = c |> dest_imp |> #1 |> strip_conj |> hd
    in p ("--- cors_pan_to_target_specialised (LAYER 1) ---\n");
    ...
    p ("theory axioms (corsBytesBridge) = "
       ^ Int.toString (length (axioms "corsBytesBridge")) ^ "\n");
    TextIO.closeOut os
  end;
