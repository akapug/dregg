(* ===========================================================================
   ...
   ipf.pnk back into HOL as a concrete literal, and scope the Link-B
   antecedent `compile_prog_max c mc ipfProg = SOME(bytes,...)` on
   ...
   ONE real stage (ipf) and reports precisely where it stands.

   Theory contents:
     ipfProg_def / ipfProg_is_parser_output
         ipfProg = OUTL(parse_topdecs_to_ast <ipf.pnk>) — the
         CakeML-verified Pancake parser's output (leanc's text->AST out of TCB),
         identical to the C10/C11 pinning.

     ipfBytes_def / ipfBitmaps_def
         The native run's `.byte`/`.quad` output, read at THEORY-BUILD TIME by
         invoking the bootstrapped `cake --pancake` on ipf.pnk and parsing
         the emitted assembly.  ipfBytes : word8 list is the concrete HOL
         term for the machine code; ipfBitmaps : word64 list the data.
     ...
     ipfBytes_length / ipfBitmaps_length  (kernel-checked, EVAL)
     ...
     LAYER 1 (kernel-checked, NO new oracle — an instance of the proven Link-B
              theorem):
     ipf_pan_to_target_specialised
         pan_to_targetProof$pan_to_target_compile_semantics specialised to
         `:64`, ipfProg, and the concrete native ipfBytes/
         ipfBitmaps in the `bytes`/`bitmaps` slots.  This is the REAL
   ...
   packagings on ipfProg, plus the standard runtime install package.
   =========================================================================== *)
...
val _ = new_theory "ipfBytesBridge";

(* ---------------------------------------------------------------------------
   (R1) ipfProg — BY CONSTRUCTION the verified parser's output on the
   leanc-emitted ipf.pnk (identical to C10 §R1).
   --------------------------------------------------------------------------- *)
val src_string =
  let val is = TextIO.openIn "ipf.pnk"
...
val ipfProg_def =
  new_definition("ipfProg_def", “ipfProg = ^prog_tm”);

Theorem ipfProg_is_parser_output:
  parse_topdecs_to_ast ^srcTm = INL ipfProg
Proof
  REWRITE_TAC[ipfProg_def] \\ MATCH_ACCEPT_TAC parse_thm
QED

(* ---------------------------------------------------------------------------
   The native run: invoke the bootstrapped `cake` on ipf.pnk (if the
   emitted assembly is not already present) and read the code/data back into HOL.
   --------------------------------------------------------------------------- *)
val _ =
  if OS.FileSys.access ("ipf.S", []) then ()
  else ignore (OS.Process.system
    "/home/hbox/r05/cake-x64-64/cake --pancake < ipf.pnk > ipf.S");
...
val ipfBytes_def =
  new_definition("ipfBytes_def", “ipfBytes = ^bytesListTm”);
val ipfBitmaps_def =
  new_definition("ipfBitmaps_def", “ipfBitmaps = ^bmListTm”);

Theorem ipfBytes_length:
  LENGTH ipfBytes = ^(numSyntax.term_of_int (length byteInts))
Proof
  REWRITE_TAC[ipfBytes_def] \\ EVAL_TAC
QED

Theorem ipfBitmaps_length:
  LENGTH ipfBitmaps = ^(numSyntax.term_of_int (length bmInts))
Proof
  REWRITE_TAC[ipfBitmaps_def] \\ EVAL_TAC
QED

(* ---------------------------------------------------------------------------
   LAYER 1 — the reflected bytes occupy the REAL Link-B `bytes` slot.
   Specialise pan_to_target_compile_semantics to :64, ipfProg, and the
   concrete native literals.  A genuine instance of the proven theorem (NOT
   vacuous): its conclusion is the machine_sem refinement for ipfProg,
   conditional on the named antecedents (which now mention the concrete
   ipfBytes, not a bound variable).
   --------------------------------------------------------------------------- *)
val ipfProg64 =
  Term.inst [Type.alpha |-> “:64”]
            (prim_mk_const {Name = "ipfProg", Thy = "ipfBytesBridge"});

val ipf_pan_to_target_specialised =
  save_thm("ipf_pan_to_target_specialised",
    pan_to_target_compile_semantics
      |> INST_TYPE [Type.alpha |-> “:64”]
      |> INST [ mk_var ("pan_code", “:64 panLang$decl list”) |-> ipfProg64,
                mk_var ("bytes",    “:word8 list”)           |-> “ipfBytes”,
                mk_var ("bitmaps",  “:word64 list”)          |-> “ipfBitmaps” ]);

(* ---------------------------------------------------------------------------
   LAYER 2 — the native-bootstrap reflection (oracle `cake_native_bootstrap`).
   The exact function the `cake` binary runs under --pancake, on ipfProg,
...
val native_eq_tm =
  “∃c'. pan_to_target$compile_prog x64_target$x64_config
           x64_config$x64_backend_config ipfProg =
             SOME (ipfBytes, ipfBitmaps, c')”;

val ipf_compile_prog_native =
  save_thm("ipf_compile_prog_native",
...
val _ =
  ...
  in
    ...
    p ("--- ipfBytes_length ---\n" ^ thm_to_string ipfBytes_length ^ "\n");
    p ("--- ipfBitmaps_length ---\n" ^ thm_to_string ipfBitmaps_length ^ "\n");
    p ("--- ipf_compile_prog_native (LAYER 2, oracle) ---\n"
       ^ thm_to_string ipf_compile_prog_native ^ "\n");
    let val (orc1, ax1) = Tag.dest_tag (Thm.tag ipf_compile_prog_native)
    in p ("LAYER2 oracles = [" ^ String.concatWith "," orc1 ^ "]  axioms = ["
          ^ String.concatWith "," ax1 ^ "]\n") end;
    let val (orc2, ax2) = Tag.dest_tag (Thm.tag ipf_pan_to_target_specialised)
        val c = concl ipf_pan_to_target_specialised
        val ante1 = c |> dest_imp |> #1 |> strip_conj |> hd
    in p ("--- ipf_pan_to_target_specialised (LAYER 1) ---\n");
    ...
    p ("theory axioms (ipfBytesBridge) = "
       ^ Int.toString (length (axioms "ipfBytesBridge")) ^ "\n");
    TextIO.closeOut os
  end;
