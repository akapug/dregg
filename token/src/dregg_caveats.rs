//! Typed Dregg caveats for Macaroon tokens.
//!
//! Defines the platform caveat types used by the Dregg runtime, plus the
...
//! pre-dregg ancestry with no production consumers in the dregg runtime.
...
use dregg_macaroon::caveat::{CaveatSet, CaveatType, WireCaveat};
...
// 0 = Organization — retired (no dregg consumers).
...
// 11 = FromMachine — retired (CLI/OS vestige, no dregg consumers).
// 12 = Command — retired (CLI vestige, no dregg consumers).
...
/// A decoded Dregg caveat representing a single access grant/restriction.
#[derive(Debug, Clone)]
pub enum DreggGrant {
    App {
        id: String,
...
fn encode_budget(
    ...
    rmp_serde::to_vec(&(id, parent_id, class, limit, window))
        .expect("msgpack encode should not fail")
}

fn decode_budget(
    body: &[u8],
) -> Result<(String, Option<String>, String, u64, Option<String>), TokenError> {
    rmp_serde::from_slice::<(String, Option<String>, String, u64, Option<String>)>(body)
        .map_err(|e| TokenError::Malformed(format!("caveat body decode: {e}")))
}

// ============================================================================
// Decode WireCaveat → DreggGrant
// ============================================================================

/// Decode a [`WireCaveat`] into a typed [`DreggGrant`].
pub fn decode_grant(wc: &WireCaveat) -> Result<DreggGrant, TokenError> {
    match wc.caveat_type {
        ...
        CAV_CONFINE_USER => {
            ...
            Ok(DreggGrant::ConfineUser(uid))
        }
        CAV_OAUTH_PROVIDER => {
            let provider = decode_string(&wc.body)?;
            Ok(DreggGrant::OAuthProvider(provider))
        }
        CAV_OAUTH_SCOPE => {
            let scope = decode_string(&wc.body)?;
            Ok(DreggGrant::OAuthScope(scope))
        }
        CAV_FEATURE_GLOB => {
        ...
        other => Ok(DreggGrant::Unknown(other, wc.body.clone())),
    }
}
...
pub fn verify_caveats(
    ...
    for wc in caveat_set.first_party_caveats() {
        match decode_grant(wc)? {
            DreggGrant::App { id, actions } => apps.push((id, actions)),
            DreggGrant::Service { name, actions } => services.push((name, actions)),
            DreggGrant::Feature(name) => features.push(name),
            ...
            DreggGrant::ConfineUser(uid) => confined_users.push(uid),
            DreggGrant::OAuthProvider(p) => oauth_providers.push(p),
            DreggGrant::OAuthScope(s) => oauth_scopes.push(s),
            DreggGrant::FeatureGlob { include, exclude } => feature_globs.push((include, exclude)),
            DreggGrant::Budget { id, limit, .. } => budgets.push((id, limit)),
            DreggGrant::Unknown(type_id, _) => {
    ...
    let expires_at = validity_windows.iter().filter_map(|(_, na)| *na).min();
...
mod tests {
    ...
    fn test_encode_decode_feature() {
        ...
        let grant = decode_grant(&wc).unwrap();
        match grant {
            DreggGrant::Feature(name) => assert_eq!(name, "ai-engine"),
    ...
    fn test_encode_decode_confine_user() {
        ...
        let grant = decode_grant(&wc).unwrap();
        match grant {
            DreggGrant::ConfineUser(uid) => assert_eq!(uid, "alice"),
