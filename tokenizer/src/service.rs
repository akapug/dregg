...
use crate::protocol::{Request, Response, HEADER_SIZE, MAX_PAYLOAD_SIZE};
...
impl KeyRing {
  ...
  pub fn from_stored_keys(keys: Vec<[u8; 32]>) -> Self {
    let keypairs = keys
      .into_iter()
      .map(TokenizerKeypair::from_bytes)
      .collect();
    Self { keypairs }
  }
  ...
  pub fn seal(&self, plaintext: &[u8]) -> Result<Vec<u8>, TokenizerError> {
    let kp = self.keypairs.last().expect("keyring must have at least one key");
...
pub(crate) async fn write_frame<T: serde::Serialize>(
  ...
  let payload = postcard::to_allocvec(msg).map_err(|e| {
    TokenizerError::Encoding(format!("serialization failed: {}", e))
  })?;
