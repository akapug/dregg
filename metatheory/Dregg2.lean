...
import Dregg2.Intent.Match       -- the solver as mathlib's coend ∫^B (A⟶B)×(B⟶C) (Types.coend over the offer profunctor): oneHop/twoHop non-vacuity + multi-hop typechecks + match_of_converts; the bilateral LensFill (get/put optic, the auction path) embeds as a one-hop route; co-Yoneda collapse deferred
