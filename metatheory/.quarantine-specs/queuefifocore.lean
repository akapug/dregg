...
theorem queueAllocateChainA_iff_spec (st : RecChainedState) (id : Nat) (actor cell : CellId) (cap : Nat)
  ...
  unfold queueAllocateChainA QueueAllocateSpec allocateGuard
  ...
  · rw [if_pos hauth]
    ...
    | some q =>
        ...
        rw [queueAllocateK]; rw [hfresh]
        constructor
        · intro h; exact absurd h (by simp)
        · rintro ⟨⟨_, hf⟩, _⟩; rw [hfresh] at hf; exact absurd hf (by simp)
    | none =>
        rw [queueAllocateK]; rw [hfresh]
        ...
        · intro h
          ...
          subst h
          exact ⟨⟨hauth, hfresh⟩, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl,
                 rfl, rfl, rfl, rfl, rfl⟩
...
theorem queueResizeChainA_iff_spec (st : RecChainedState) (id newCap : Nat) (actor cell : CellId)
  ...
  unfold queueResizeChainA QueueResizeSpec resizeGuard
  ...
  · rw [if_pos hg]
    ...
    | none =>
        ...
        constructor
        · intro h; exact absurd h (by simp)
        · rintro ⟨⟨_, _, q, hq, _⟩, _⟩; rw [hf] at hq; exact absurd hq (by simp)
    | some q =>
        by_cases hcap : q.buffer.length ≤ newCap
        · rw [queueResizeK, hf, if_pos hcap]
          simp only [resizeReceipt]
          constructor
          · intro h
            simp only [Option.some.injEq] at h
            subst h
            refine ⟨⟨hauth, hacc, q, hf, hcap⟩, ?_, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl, rfl,
                    rfl, rfl, rfl, rfl, rfl, rfl⟩
            intro q' hq'; rw [hf] at hq'; simp only [Option.some.injEq] at hq'; subst hq'; rfl
          · rintro ⟨_, hq, hlog, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12, h13, h14, h15, h16⟩
            have hqeq := hq q hf
            obtain ⟨k', l'⟩ := st'
            ...
            rfl
        · rw [queueResizeK, hf, if_neg hcap]
          constructor
          · intro h; exact absurd h (by simp)
          · rintro ⟨⟨_, _, q', hq', hcap'⟩, _⟩
            rw [hf] at hq'; simp only [Option.some.injEq] at hq'; subst hq'; exact absurd hcap' hcap
  · rw [if_neg hg]
    constructor
    · intro h; exact absurd h (by simp)
...
theorem queueEnqueueChainA_iff_spec (st : RecChainedState) (id m : Nat) (actor cell : CellId)
  ...
  · rw [if_pos hg]
    ...
    | none =>
        ...
        constructor
        · intro h; exact absurd h (by simp)
        · rintro ⟨k₁, _, _, hk1, _⟩; rw [hk] at hk1; exact absurd hk1 (by simp)
    | some k₁ =>
        rw [hk]
        ...
        · rw [if_pos hd]
          ...
          · intro h
            simp only [Option.some.injEq] at h
            subst h
            exact ⟨k₁, hauth, hacc, hk, hd1, hd2, hd3, hd4, rfl, rfl⟩
          · rintro ⟨k₁', _, _, hk1', _, _, _, _, hker, hlog⟩
            rw [hk] at hk1'; simp only [Option.some.injEq] at hk1'; subst hk1'
            obtain ⟨k', l'⟩ := st'
...
theorem queueDequeueChainA_iff_spec (st : RecChainedState) (id : Nat) (actor cell : CellId)
  ...
  · rw [if_pos hg]
    ...
    | none =>
        ...
        constructor
        · intro h; exact absurd h (by simp)
        · rintro ⟨_, _, k', m, hk', _⟩; rw [hk] at hk'; exact absurd hk' (by simp)
    | some kr =>
        obtain ⟨k', m⟩ := kr
        rw [hk]
        constructor
        · intro h
          simp only [Option.some.injEq] at h
          subst h
          exact ⟨hauth, hacc, k', m, hk, rfl, rfl⟩
        · rintro ⟨_, _, k'', m', hk'', hker, hlog⟩
          rw [hk] at hk''; simp only [Option.some.injEq, Prod.mk.injEq] at hk''
