...
| 2 | **mint** (`EffectVmEmitMint`) | **A−** | `mintDescriptor_full_sound:225` (`CellMintSpec`: `bal_lo = pre+amt`, frame frozen) + commit-binds + `unify_mint_exec:291` (→ `recCMintAsset`) | The supply-credit IS a `bal_lo` move — an in-commitment state-block column. Genuinely bound + anti-ghosted + executor-unified. **A− residual:** the global *supply total* side-table (if any) is not a descriptor column; per-cell credit is the whole effect here, so this is the strongest non-transfer effect. |
| 3 | **burn** (`EffectVmEmitBurn`) | **A−** | `burnDescriptor_full_sound:329` + `unify_burn_exec:433` (→ `recCBurnAsset`) + `descriptor_agrees_with_executor:…` | `bal_lo` debit + frame freeze bound + anti-ghost + executor-unified on every conserved clause. Same A− residual as mint (supply total off-row) + the named nonce-tick gap (`exec_nonce_is_frozen_not_ticked`). |
...
| 39 | **createEscrow** (`EffectVmEmitCreateEscrow`) | **C (amplified-not-deployed)** | `createEscrowFull_sound:644` (`CellCreateSpec ∧ CreateEscrowRootIntent ∧ commit`) + `createEscrowFull_binds_escrow_root:631` | Balance/frame leg bound. **The escrow root IS proved bound** — but on the AMPLIFIED `createEscrowVmDescriptorFull` whose root carrier is `SYS_DIG_AFTER = aux 96 = SYSTEM_ROOTS_DIGEST`, **PAST `EFFECT_VM_WIDTH = 186`** (§E: the deployed hand-AIR still absorbs `BabyBear::ZERO` at commitment slot 4 — "the one Rust-side step that graduates the cutover"). Also the root advance is `SYS_DIG_AFTER = SYS_DIG_BEFORE + step_param` (additive opaque step, `gEscrowRootUpdate:544`), not a recomputed escrow-list digest. Plus a conserved-leg debit/nonce divergence on the runtime trace. ⇒ proved-in-Lean, not-deployed, opaque-step ⇒ C. |
| 40 | **createCommittedEscrow** (`EffectVmEmitCreateCommittedEscrow`) | **C (amplified-not-deployed)** | `escrowCreateDescriptor_full_sound:277` | As createEscrow + committed-escrow digest; same SYS_DIG-past-width + opaque-step gaps. |
| 41 | **releaseEscrow** (`EffectVmEmitReleaseEscrow`) | **C (amplified-not-deployed)** | `releaseEscrowDescriptor_full_sound:249` | Balance release leg bound; escrow-root amplification same SYS_DIG-past-width + opaque-step gaps. |
| 42 | **refundEscrow** (`EffectVmEmitRefundEscrow`) | **C (amplified-not-deployed)** | `refundEscrowDescriptor_full_sound:247` | Balance refund leg bound; same escrow-root gaps. |
| 43 | **bridgeLock** (`EffectVmEmitBridgeLockA`) | **C (amplified-not-deployed)** | `bridgeLockDescriptor_full_sound:300` + `escrow_root_not_in_descriptor_commit:525` | Balance lock leg bound; the bridge escrow side-table root amplification carried at `aux 96` **PROVEN absent from the deployed descriptor commitment** (`escrow_root_not_in_descriptor_commit`). |
| 44 | **bridgeMint** (`EffectVmEmitBridgeMint`) | **C** | `bridgeMintDescriptor_full_sound:242` + `unify_bridgeMint_exec:319` | Balance credit leg bound + executor-unified; the inbound-bridge side-table / proof gate out-of-row. |
| 45 | **bridgeFinalize** (`EffectVmEmitBridgeFinalize`) | **C (amplified-not-deployed)** | `bridgeFinalizeDescriptor_full_sound:274` + `escrow_root_not_in_descriptor_commit:476` | Balance leg bound; bridge escrow root amplification PROVEN absent from deployed descriptor commit. |
| 46 | **bridgeCancel** (`EffectVmEmitBridgeCancel`) | **C (amplified-not-deployed)** | `bridgeCancelDescriptor_full_sound:269` + `escrow_root_not_in_descriptor_commit:453` | Balance leg bound; bridge escrow root amplification PROVEN absent from deployed descriptor commit. |
