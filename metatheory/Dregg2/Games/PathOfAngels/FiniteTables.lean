...
in the module's closure initialises.  Measured on a signer built against an archive that
exports `dregg_poa_signal_judge` (task/2342): `install_verified_mldsa_sign_core_real` runs
before argv is read, `dregg_ffi_init` initialises `NetworkJudge` → `Emit` → this module, and
the six SALVAGE checks below (fifteen seeds × a 12-round closure to 260 states with quadratic
`eraseDups`, plus the 90-seed parametric refinement) had not finished after three minutes at
5 GB — the process never reached `main`.  They therefore carry a `(_ : Unit)` parameter, which
