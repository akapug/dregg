...
No `sorry`/`admit`/`axiom`/`native_decide`. The Poseidon-CR portals are carried HYPOTHESES on the abstract
keystones (exactly the template). `#assert_axioms` whitelists `{propext, Classical.choice, Quot.sound}`.
-/
