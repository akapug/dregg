...
(`capRootGroupCol (EFFECT_VM_WIDTH + 151)`, the whole ~124-bit cap-root) at the read leaf — the
...
theorem effCapInsertV3_forces_afterMembership (S8 : Cap8Scheme)
    ...
    (hsat : Satisfied2 hash (effCapInsertV3 base name n) minit mfin maddrs t)
    (i : Nat) (hi : i < t.rows.length) (hnotlast : i + 1 ≠ t.rows.length) :
    MembersAt8 S8 (fun k => (envAt t i).loc (capRootGroupCol (EFFECT_VM_WIDTH + 151) k))
  ...
  have hroot : groupVal e (capOpenCols w).capRoot
      = (fun k => e.loc (capRootGroupCol (EFFECT_VM_WIDTH + 151) k)) := by
    funext k
    have hin : VmConstraint2.base (.gate (eqGate ((capOpenCols w).capRoot k)
        (capRootGroupCol (EFFECT_VM_WIDTH + 151) k))) ∈ afterCapRootWelds w :=
...
theorem effCapInsertV3_forces_write8 (S8 : Cap8Scheme)
    ...
    (hafter : SpineCommits S8 (fun k => (envAt t i).loc (capRootGroupCol (EFFECT_VM_WIDTH + 151) k))
    ...
    capInserts8 S8
        ...
        (fun k => (envAt t i).loc (capRootGroupCol (EFFECT_VM_WIDTH + 151) k)) := by
