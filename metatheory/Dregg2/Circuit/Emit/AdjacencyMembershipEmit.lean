...
/-- The full constraint list: the transition-domain core ++ the last-row ordering fix. The ordering
is now enforced on every row, so the emitted descriptor faithfully matches the deployed every-row
`assert_zero` lowering (no under-constrained top Merkle level). -/
def adjacencyConstraints : List VmConstraint2 :=
  adjacencyConstraintsCore ++ adjLastOrderFix
...
#guard adjacencyDesc.constraints.length == 32
