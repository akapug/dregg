...
The per-row child-ordering gates (`dirBinaryBody` / `leftOrderBody` / `rightOrderBody`) are emitted
as `.base (.gate …)` — TRANSITION constraints, vacuous on the last row (`holdsVm` is `True` when
`isLast`). The deployed DSL lowers those same gates as every-row `assert_zero` (they fire on the last
row too, `dsl_plonky3.rs:225/240`, `is_transition = false`). To match the deployed semantics — and
close the top-Merkle-level ordering forge the refinement proof caught — `adjLastOrderFix` re-lowers
exactly those six bodies as `.base (.boundary VmRow.last …)`, so the ordering is enforced on EVERY
row (the transition `.gate`s cover rows `0..n-2`, these boundaries cover row `n-1`). -/

/-- **The last-row ordering fix** — the six child-ordering bodies (dir-binary / left-order /
right-order, per path) as `VmRow.last` boundary constraints, covering the last row the transition
`.gate`s make vacuous. This is what makes the top Merkle level's ordering non-forgeable. -/
...
/-- **The last-row INDEX-RECONSTRUCTION fix** — the two per-path index-accumulation bodies
(`idx_out - idx_in - dir*pow`) as `VmRow.last` boundary constraints, covering the last row the
transition `.gate`s (`idxStepBody` at `pathBlock`) make vacuous. This is the ISOMORPHIC twin of
`adjLastOrderFix` for conjunct C (consecutiveness bound to the committed witness): without it,
`L_IDX_OUT[last]`/`U_IDX_OUT[last]` are decoupled from the genuine path reconstruction
`idx_in[last] + dir[last]*pow[last]` on the last row (only pinned to the index PIs + the
consecutiveness tooth), so a prover can publish index values that DO NOT match the reconstructed
tree positions of the two leaves — the top bit's index contribution is free. The deployed DSL
lowers this same `Polynomial` step as an every-row `assert_zero` (`is_transition = false`,
`dsl_plonky3.rs`), firing on the last row too; re-lowering it here makes the emit at least as strong
as the deployed AIR, binding the published index to the in-circuit reconstruction on EVERY row. -/
def adjLastIdxFix : List VmConstraint2 :=
  [ .base (.boundary VmRow.last (idxStepBody L_DIR L_IDX_IN L_IDX_OUT))
  , .base (.boundary VmRow.last (idxStepBody U_DIR U_IDX_IN U_IDX_OUT)) ]

/-- The transition-domain constraints (per-row gates + cross-row copies + boundary/PI pins) — every
constraint whose ordering coverage stops before the last row, plus the boundary/PI anchors. -/
def adjacencyConstraintsCore : List VmConstraint2 :=
...
/-- The full constraint list: the transition-domain core ++ the last-row ordering fix ++ the last-row
index-reconstruction fix. The child-ordering AND the index accumulation are now both enforced on every
row, so the emitted descriptor faithfully matches the deployed every-row `assert_zero` lowering (no
under-constrained top Merkle level, and no free published index on the last row). -/
def adjacencyConstraints : List VmConstraint2 :=
  adjacencyConstraintsCore ++ adjLastOrderFix ++ adjLastIdxFix
...
continuity/carry `windowGate`s, the leaf/root/index `piBinding`s, the row-0 `Fixed` anchors, the
internalized consecutiveness catch tooth, and the last-row ordering fix (`adjLastOrderFix`) that makes
the top Merkle level's child-ordering non-forgeable. The chip table (`TID_P2`) is Presence-detected. -/
...
`idx_out` equals the genuine in-circuit reconstruction `idx_in + dir*pow`. This is what the last-row
`adjLastIdxFix` boundary binds; a forged published index (decoupled from the path) makes it nonzero. -/
theorem idx_step_body_zero_iff (a : Assignment) (dir idxIn idxOut : Nat) :
    (idxStepBody dir idxIn idxOut).eval a = 0 ↔ a idxOut = a idxIn + a dir * a POW := by
  simp only [idxStepBody, negE, EmittedExpr.eval]
...
#guard adjacencyDesc.constraints.length == 34
#guard adjLastOrderFix.length == 6
#guard adjLastIdxFix.length == 2
