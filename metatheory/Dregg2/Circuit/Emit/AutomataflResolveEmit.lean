...
`eqScalarConstraints`, `node8Lookup`, …) are `n`-agnostic and REUSED as-is from the frozen block.
...
The commitment leg (`NGen.bindBoardRootsConstraints`) is deliberately KEPT at the `k = n*n ≤ 8`
single padded-leaf form: correct at `n = 2`, NOT at `n = 11` (`k = 121`). Swapping it for the packed
commitment is the NEXT step; no `n = 11` commitment shape is pinned here. -/
namespace NGen
...
def MH8_ZERO (n : Nat) : Nat := (MH0 n)
def oldRootCols (n : Nat) : List Nat := (List.range 8).map ((MH0 n) + 1 + ·)
def midRootCols (n : Nat) : List Nat := (List.range 8).map ((MH0 n) + 9 + ·)
def R_WIDTH (n : Nat) : Nat := (MH0 n) + 17
...
def bindBoardRootsConstraints (n : Nat) : List VmConstraint2 :=
  let zeroLeaf := List.replicate 8 (MH8_ZERO n)
  -- UNCHANGED single padded leaf: correct only at `k = n*n ≤ 8` (n ≤ 2). At n = 11 (k = 121) this
  -- needs the packed commitment of the NEXT step; deliberately NOT n-generic here.
  let leaf (cells : List Nat) : List Nat := cells ++ List.replicate (8 - (KK n)) (MH8_ZERO n)
  [ cgH (Head.lin 1 (MH8_ZERO n)) ]
  ++ [ node8Lookup (leaf ((List.range (KK n)).map (old n))) zeroLeaf (oldRootCols n) ]
  ++ (List.range 8).map (fun i =>
       (.base (.piBinding VmRow.first ((oldRootCols n)[i]!) (16 + i)) : VmConstraint2))
  ++ [ node8Lookup (leaf ((List.range (KK n)).map (mid n))) zeroLeaf (midRootCols n) ]
  ++ (List.range 8).map (fun i =>
       (.base (.piBinding VmRow.first ((midRootCols n)[i]!) (24 + i)) : VmConstraint2))
...
def resolveConstraints (n : Nat) : List VmConstraint2 :=
  (onePin n) :: ((boardRangeConstraints n)
    ...
    ++ (bindBoardRootsConstraints n))                                -- leg 8
...
byte-golden `automataflResolveDesc`; the commitment leg stays the `k ≤ 8` padded leaf (n = 2 only). -/
...
`automataflResolveDescN 11` is a well-formed term. These pin the board-cell count and the
move/occlusion block widths at the deployed target `n = 11` (`k = 121`). The COMMITMENT is NOT pinned
here — the padded-leaf commitment leg is correct only at `k ≤ 8` and is left to the commitment-swap
step, so no `board_root8` / width / total-constraint shape at `n = 11` is asserted. -/
...
#guard (NGen.resolveConstraints 2).length == 379
