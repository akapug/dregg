/-
# Dregg2.Circuit.Emit.EffectVmEmitSwissDrop — the CapTP sturdy-ref HANDOFF `swissDropA`, EMITTED
...
moment a `SwissDrop` selector lands, but it CANNOT pass the cutover harness today — reported, not
papered.

## The CONNECTOR — `sturdyrefRootProj` to universe-A's `swissDropA_full_sound`
...
namespace selSD
/-- The `swissDropA` effect selector column (NAME-ONLY; not a live runtime selector — header). The
runtime `DROP_REF=16` is a DIFFERENT GC operation — `swissDropA` is the swiss-table refcount-decrement
...
/-- **`swissDropVmDescriptor`** — the `swissDropA` concrete circuit: sturdyref-root MOVE
...
/-- **`SwissDropRowIntent env`** — post `sturdyref_root` (`field[4]`) is the digest param, nonce
...
/-- **`swissDropRowGates_holds_iff`** — on a swiss-handoff row (`s_noop = 0`), the gates all hold IFF
...
/-- **`swissDropVm_faithful` — THE deliverable.** -/
...
theorem swissDropVm_rejects_wrong_swissRoot (env : VmRowEnv)
    (hwrong : env.loc (saCol sturdyrefRootOff) ≠ env.loc (prmCol paramSD.SWISS_DIGEST_NEW)) :
...
/-- **`swissDropDescriptor_full_sound` — the structured soundness.** -/
...
/-- **`swissDropDescriptor_binds_sturdyref_root` — the per-column anti-ghost.** Equal published
`state_commit`s force the moved dedicated `sturdyref_root` (`field[4]`, absorbed column #7) equal. -/
theorem swissDropDescriptor_binds_sturdyref_root (hash : List ℤ → ℤ) (hCR : Poseidon2SpongeCR hash)
    (e₁ e₂ : VmRowEnv)
    (hs₁ : siteHoldsAll hash e₁ swissDropHashSites)
    (hs₂ : siteHoldsAll hash e₂ swissDropHashSites)
    (hcommit : e₁.loc (saCol state.STATE_COMMIT) = e₂.loc (saCol state.STATE_COMMIT)) :
    e₁.loc (saCol sturdyrefRootOff) = e₂.loc (saCol sturdyrefRootOff) := by
  have h := swissDropDescriptor_commit_binds_state hash hCR e₁ e₂ hs₁ hs₂ hcommit
...
/-! ## §8 — THE CONNECTOR — `sturdyrefRootProj` to universe-A's `swissDropA_full_sound`. -/
...
/-- **`unify_swissDrop` — THE CONNECTOR.** When universe-A's `DropSpec` holds (so `swissDropK
s.kernel sw = some k'`, `s'.kernel = k'`), the projected post-`sturdyref_root` `D s'.kernel.swiss
= D k'.swiss` — and `k'.swiss` is the genuine decremented/GC'd post-list. -/
...
/-- **`unify_swissDrop_via_full_sound` — the runnable dedicated-root move inherits the VALIDATED
guarantee.** A satisfying universe-A `swissDropA_full_sound` witness ⟹ `DropSpec` ⟹ the projected
...
theorem col_facts :
    saCol sturdyrefRootOff = 83 ∧ prmCol paramSD.SWISS_DIGEST_NEW = 70
    ∧ sbCol state.NONCE = 56 ∧ saCol state.NONCE = 78 ∧ selSD.SWISS_DROP = 5 :=
  ⟨rfl, rfl, rfl, rfl, rfl⟩
...
theorem swissGoodRow_realizes_intent : SwissDropRowIntent swissGoodRow := by
  ...
  · intro i hi
    ...
    have lhs0 : swissGoodRow.loc (79 + i) = 0 := by
      show (if (79 + i) = 5 then (1:ℤ) else if (79+i) = 56 then 9 else if (79+i) = 78 then 10
    ...
    have rhs0 : swissGoodRow.loc (57 + i) = 0 := by
      show (if (57 + i) = 5 then (1:ℤ) else if (57+i) = 56 then 9 else if (57+i) = 78 then 10
