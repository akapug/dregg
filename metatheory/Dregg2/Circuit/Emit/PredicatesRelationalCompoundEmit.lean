...
def relationalConstraints : List VmConstraint2 :=
  ...
  -- ⚑ C2b: THE VERDICT WELD — diff == value_a − value_b. WITHOUT this the eq/neq/range flag-checks
  --   below operate on a PROVER-CHOSEN free `diff`, so `result_bit == 1` asserts a relation that was
  --   never actually checked against the committed values (the decoupling bug). Tying diff to the real
  --   value difference makes C7/C9/C10 genuinely enforce ≥ / == / ≠ between VALUE_A and VALUE_B.
  ++ [gate (.add (subV (.var DIFF) VALUE_A) (.var VALUE_B))]
...
#guard relationalPredicateDesc.constraints.length == 48
