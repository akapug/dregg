...
/-! ## §X — THE NO-DOUBLE-SPEND FRESHNESS GATE, NOW AN IN-CIRCUIT CONJUNCT OF THE RUNNABLE DESCRIPTOR.

§A–§W bound the nullifier-set INSERT (the digest advance) into `state_commit`, but the headline
anti-replay precondition the EXECUTOR enforces fail-closed — `nf ∉ st.nullifiers` (`noteSpendNullifier`,
`apply.rs:941`; the spec guard `noteSpendGuard`'s 2nd conjunct) — was carried OUT-OF-CIRCUIT, as the
"turn-level non-membership gadget" residual (`noteSpend_freshness_still_needs_nonmembership`,
`EffectVmEmitNoteSpendCompose.TurnLevelFreshness`). That is the precise COMPLETENESS hole: the RUNNABLE
descriptor `noteSpendVmDescriptorWide` had NO arithmetic constraint relating `nf` to the committed set,
so a DOUBLE-SPEND row (a `nf` already present) satisfied it — a light client (which sees ONLY the proof)
would accept "a proof over an already-spent nullifier".

This section CLOSES that hole at the descriptor level. The sorted-neighbor non-membership ARITHMETIC
(`lo < nf < hi`, realized as two non-negative-gap linear gates) is NOT crypto — it is the exact gate the
genuine non-membership reference `Argus/Effects/NoteSpend.lean` (`gapLoGate`/`gapHiGate`) already proved
sound+complete+anti-ghost. We wire those gates onto the RUNNABLE descriptor's param columns, so the
freshness arithmetic is a HARD CONJUNCT of `satisfiedVm noteSpendVmDescriptorWideFresh`. After this, the
only residuals are NAMED primitives, not laundered preconditions:

  * **non-negativity of the gap witnesses** (`dLo, dHi ≥ 0`) — the range-check LOOKUP leg, declared in
    `.ranges` EXACTLY as the balance limbs are (`FreshRowOk`, the same status as `bal_lo`/`bal_hi`'s
    range-checks; the LogUp argument is the named gate-kind);
  * **neighbor-genuineness** (`(lo, hi)` ARE the committed adjacent neighbors of the `nullifiers` root —
    `GapInterval lo hi nullifiers`) — a Poseidon2-CR SORTED-TREE adjacency opening, the SAME named
    `Poseidon2SpongeCR` boundary the §A root binding already lives behind.

Both are exactly the residuals the keystone permits (a range-check lookup + a Poseidon2-CR opening).
EVERYTHING ELSE — the `lo < nf < hi` strict-between, the rejection of an in-set `nf` — is now in the
runnable circuit. The freshness param columns reuse the spent-nullifier param0 (so the gate constrains
the GENUINE consumed `nf`, not a free column) plus four FREE param columns (3–6); no width change, no
aliasing with the state/aux/sysroots carriers. -/

/-- The spent-nullifier param column (`param0 = nf`, the trace generator's convention — the SAME column
the nullifier-root step is computed from, so the freshness gate constrains the genuine consumed `nf`). -/
def FRESH_NF_PARAM : Nat := 0
/-- The lower-neighbor read-out param column (`lo`, a committed adjacent neighbor — the sorted-tree
opening's left leaf). -/
def FRESH_LO_PARAM : Nat := 3
/-- The upper-neighbor read-out param column (`hi`, the right adjacent leaf). -/
def FRESH_HI_PARAM : Nat := 4
/-- The lower gap-witness param column (`dLo ≥ 0`, the strict-`<` certificate `nf = lo + 1 + dLo`). -/
def FRESH_DLO_PARAM : Nat := 5
/-- The upper gap-witness param column (`dHi ≥ 0`, `hi = nf + 1 + dHi`). -/
def FRESH_DHI_PARAM : Nat := 6

/-- Lower-gap freshness gate body `nf − lo − 1 − dLo` (asserts `nf = lo + 1 + dLo`, so `lo < nf` given
`dLo ≥ 0`). A single PLONK linear gate over the param columns; coefficient `+1` on `nf`, `−1` on
`lo`/`dLo`. The runnable-descriptor analog of `Argus.gapLoBody`. -/
def gFreshLo : EmittedExpr :=
  .add (.var (prmCol FRESH_NF_PARAM))
    (.add (.mul (.const (-1)) (.var (prmCol FRESH_LO_PARAM)))
      (.add (.const (-1)) (.mul (.const (-1)) (.var (prmCol FRESH_DLO_PARAM)))))

/-- Upper-gap freshness gate body `hi − nf − 1 − dHi` (asserts `hi = nf + 1 + dHi`, so `nf < hi` given
`dHi ≥ 0`). -/
def gFreshHi : EmittedExpr :=
  .add (.var (prmCol FRESH_HI_PARAM))
    (.add (.mul (.const (-1)) (.var (prmCol FRESH_NF_PARAM)))
      (.add (.const (-1)) (.mul (.const (-1)) (.var (prmCol FRESH_DHI_PARAM)))))

/-- **`FreshRowOk env`** — the row carries a WELL-FORMED non-membership witness: both gap witnesses are
non-negative. This is the range-check LOOKUP leg (`dLo, dHi ∈ [0, 2^k)`), declared in the descriptor's
`.ranges` below — the SAME enforcement status as the `bal_lo`/`bal_hi` range-checks. Load-bearing: the
linear gates alone do not pin the inequality DIRECTION (a negative `dLo` satisfies `nf = lo + 1 + dLo`
with `nf ≤ lo`), so this is the named lookup residual, not a free assumption. -/
def FreshRowOk (env : VmRowEnv) : Prop :=
  0 ≤ env.loc (prmCol FRESH_DLO_PARAM) ∧ 0 ≤ env.loc (prmCol FRESH_DHI_PARAM)

instance (env : VmRowEnv) : Decidable (FreshRowOk env) :=
  inferInstanceAs (Decidable (0 ≤ env.loc (prmCol FRESH_DLO_PARAM) ∧ 0 ≤ env.loc (prmCol FRESH_DHI_PARAM)))

/-- **`gFreshLo_holds_iff`.** The lower freshness gate holds IFF `nf = lo + 1 + dLo` — the faithful
arithmetic of the linear gate. -/
theorem gFreshLo_holds_iff (env : VmRowEnv) :
    (VmConstraint.gate gFreshLo).holdsVm env false false
      ↔ env.loc (prmCol FRESH_NF_PARAM)
          = env.loc (prmCol FRESH_LO_PARAM) + 1 + env.loc (prmCol FRESH_DLO_PARAM) := by
  simp only [VmConstraint.holdsVm, gFreshLo, EmittedExpr.eval]
  constructor
  · intro h; linarith
  · intro h; linarith

/-- **`gFreshHi_holds_iff`.** The upper freshness gate holds IFF `hi = nf + 1 + dHi`. -/
theorem gFreshHi_holds_iff (env : VmRowEnv) :
    (VmConstraint.gate gFreshHi).holdsVm env false false
      ↔ env.loc (prmCol FRESH_HI_PARAM)
          = env.loc (prmCol FRESH_NF_PARAM) + 1 + env.loc (prmCol FRESH_DHI_PARAM) := by
  simp only [VmConstraint.holdsVm, gFreshHi, EmittedExpr.eval]
  constructor
  · intro h; linarith
  · intro h; linarith

/-- **`noteSpendVmDescriptorWideFresh`** — the WIDE runnable descriptor PLUS the two in-circuit freshness
gates (`gFreshLo`, `gFreshHi`) AND the two gap-witness range-checks in `.ranges`. Strictly additive over
`noteSpendVmDescriptorWide` (two extra per-row `.gate`s + two `VmRange` teeth). A satisfying witness now
carries the strict-between `lo < nf < hi` as a HARD arithmetic conjunct — the missing no-double-spend
freshness, in the circuit the prover runs. -/
def noteSpendVmDescriptorWideFresh : EffectVmDescriptor :=
  { noteSpendVmDescriptorWide with
    name := noteSpendVmAirName ++ "-sysroots-fresh"
    constraints := noteSpendVmDescriptorWide.constraints ++ [.gate gFreshLo, .gate gFreshHi]
    ranges := noteSpendVmDescriptorWide.ranges
                ++ [ ⟨prmCol FRESH_DLO_PARAM, 30⟩, ⟨prmCol FRESH_DHI_PARAM, 30⟩ ] }

/-- The strengthened descriptor still uses the wide hash-sites (so it remains a `RunnableFullStateSpec`
descriptor — the full-state crown lifts to it unchanged). -/
theorem noteSpendFresh_usesWideSites : noteSpendVmDescriptorWideFresh.hashSites = wideHashSites := rfl

/-- The strengthened descriptor's constraints CONTAIN the original wide descriptor's (so every §W
property — the full-state clause, the anti-ghosts — lifts to it verbatim by membership). -/
theorem noteSpendFresh_constraints_superset :
    ∀ c ∈ noteSpendVmDescriptorWide.constraints, c ∈ noteSpendVmDescriptorWideFresh.constraints := by
  intro c hc
  unfold noteSpendVmDescriptorWideFresh
  simp only [List.mem_append]
  exact Or.inl hc

/-- **`noteSpendFresh_forces_between` — THE NEW IN-CIRCUIT TEETH.** A row satisfying the strengthened
descriptor, under the range-check leg `FreshRowOk` (the named lookup residual — gap witnesses
non-negative), forces the neighbors to STRICTLY BRACKET the spent nullifier: `lo < nf < hi`. This is the
freshness arithmetic, now a CONJUNCT of `satisfiedVm noteSpendVmDescriptorWideFresh` — a double-spend
row cannot escape it. -/
theorem noteSpendFresh_forces_between (hash : List ℤ → ℤ) (env : VmRowEnv) (b1 b2 : Bool)
    (hok : FreshRowOk env)
    (hgates : ∀ c ∈ noteSpendVmDescriptorWideFresh.constraints, c.holdsVm env b1 b2) :
    env.loc (prmCol FRESH_LO_PARAM) < env.loc (prmCol FRESH_NF_PARAM)
      ∧ env.loc (prmCol FRESH_NF_PARAM) < env.loc (prmCol FRESH_HI_PARAM) := by
  obtain ⟨hdLo, hdHi⟩ := hok
  have hmemLo : (VmConstraint.gate gFreshLo) ∈ noteSpendVmDescriptorWideFresh.constraints := by
    unfold noteSpendVmDescriptorWideFresh; simp only [List.mem_append]; exact Or.inr (by simp)
  have hmemHi : (VmConstraint.gate gFreshHi) ∈ noteSpendVmDescriptorWideFresh.constraints := by
    unfold noteSpendVmDescriptorWideFresh; simp only [List.mem_append]; exact Or.inr (by simp)
  -- the freshness gates are per-row `.gate`s, so `holdsVm` is flag-free.
  have hLo : env.loc (prmCol FRESH_NF_PARAM)
      = env.loc (prmCol FRESH_LO_PARAM) + 1 + env.loc (prmCol FRESH_DLO_PARAM) := by
    rw [← gFreshLo_holds_iff env]
    have := hgates _ hmemLo
    simpa only [VmConstraint.holdsVm] using this
  have hHi : env.loc (prmCol FRESH_HI_PARAM)
      = env.loc (prmCol FRESH_NF_PARAM) + 1 + env.loc (prmCol FRESH_DHI_PARAM) := by
    rw [← gFreshHi_holds_iff env]
    have := hgates _ hmemHi
    simpa only [VmConstraint.holdsVm] using this
  exact ⟨by linarith, by linarith⟩

/-! ### §X.1 — reconciliation pins + axiom-hygiene for the in-circuit freshness arithmetic.

The CIRCUIT ⟺ EXECUTOR-PRECONDITION bridge (the in-circuit `lo < nf < hi`, on the named sorted-tree
opening, PROVES the executor's `nf ∉ nullifiers`) + the double-spend anti-ghost + the full-state-AND-
freshness crown + the non-vacuity rows are proved in `Argus/Effects/NoteSpend.lean` (which imports this
module and carries the proven `GapInterval`/`between_gap_not_mem`/`mem_not_between_gap` math bridge — it
cannot be imported back here without a cycle). This module supplies the gates + descriptor + the pure
arithmetic teeth `noteSpendFresh_forces_between`. -/

-- The freshness param columns are the spent-nullifier param0 + four FREE param columns (3–6), all
-- in-range of the 8 param slots, and the gate columns are pairwise distinct.
#guard FRESH_NF_PARAM == 0
#guard [FRESH_NF_PARAM, FRESH_LO_PARAM, FRESH_HI_PARAM, FRESH_DLO_PARAM, FRESH_DHI_PARAM].dedup.length == 5
#guard decide (FRESH_DHI_PARAM < NUM_PARAMS)
-- The strengthened descriptor adds exactly the two freshness gates + two range teeth; same wide sites.
#guard noteSpendVmDescriptorWideFresh.constraints.length == noteSpendVmDescriptorWide.constraints.length + 2
#guard noteSpendVmDescriptorWideFresh.ranges.length == noteSpendVmDescriptorWide.ranges.length + 2
#guard noteSpendVmDescriptorWideFresh.hashSites.length == 4

#assert_axioms gFreshLo_holds_iff
#assert_axioms gFreshHi_holds_iff
#assert_axioms noteSpendFresh_forces_between
#assert_axioms noteSpendFresh_usesWideSites
#assert_axioms noteSpendFresh_constraints_superset
