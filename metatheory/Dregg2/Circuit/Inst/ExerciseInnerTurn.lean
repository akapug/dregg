...
per-step refinement `hstep` (which `TurnEmit.step_emitted_refines_fullActionStep`
supplies). -/
...
/-- **`exercise_r4_facet_mask` — the facet-bridge lemma.** `execFullA`'s `exerciseA` ENFORCES the facet
mask (`innerFacetsAdmittedA`) and
...
inner-turn-emission front, carried here as the explicit `hinner` hypothesis: once
