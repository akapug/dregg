...
def realRow : Assignment := fun v =>
  ...
  else if v = 340 then 90                        -- rotated AFTER r0 (welded to after bal_lo = 90)
  else if v = 341 then 1                         -- rotated AFTER r1 (welded to after nonce = 1)
...
theorem transferRowIntent_envReal : TransferRowIntent envReal := by
  ...
  · intro i hi
    ...
    have ha : realRow (saCol (state.FIELD_BASE + i)) = 0 := by
      ...
      have e8 : ¬ (76 + (3 + i) = 340) := by omega
      have e9 : ¬ (76 + (3 + i) = 341) := by omega
    ...
    have hb : realRow (sbCol (state.FIELD_BASE + i)) = 0 := by
      ...
      have e8 : ¬ (54 + (3 + i) = 340) := by omega
      have e9 : ¬ (54 + (3 + i) = 341) := by omega
      simp only [if_neg e1, if_neg e2, if_neg e3, if_neg e4, if_neg e5, if_neg e6, if_neg e7,
        if_neg e8, if_neg e9]
    rw [ha, hb]
...
/-- Off the nine named columns (`1, 54, 76, 78, 68, 69, 189, 340, 341`), `realRow` is `0`. -/
theorem realRow_zero_of {v : Nat} (h1 : v ≠ 1) (h54 : v ≠ 54) (h76 : v ≠ 76)
    (h78 : v ≠ 78) (h68 : v ≠ 68) (h69 : v ≠ 69) (h188 : v ≠ 189) (h239 : v ≠ 340)
    (h240 : v ≠ 341) : realRow v = 0 := by
  simp only [realRow, sel.TRANSFER, sbCol, saCol, prmCol, STATE_BEFORE_BASE, STATE_AFTER_BASE,
...
theorem base_constraints_hold :
  ...
  · simp only [rotateV3, show transferVmDescriptor.constraints
    ...
    · -- after-block welds: base 339. r0(340)=after bal_lo(76)=90; r1(341)=after nonce(78)=1; the
