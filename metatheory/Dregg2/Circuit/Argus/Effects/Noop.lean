...
kernel queue side-table too — F3 removed `swiss`; the populated side-table witness is a REVOKED
registry entry now, at the same non-vacuity strength.) -/
...
#guard ((interp noopStmt kN).map (fun k => k.revoked.length)) == some 1              -- revoked registry frozen
