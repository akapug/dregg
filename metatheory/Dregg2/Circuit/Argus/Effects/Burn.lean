...
anti-ghost on every column/root (`burn_rejects_state_tamper_or_collides` /
`burn_rejects_root_tamper_or_collides`). ⚑ Read those teeth at their real strength: each concludes a
