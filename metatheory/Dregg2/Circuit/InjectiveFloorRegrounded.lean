...
| `Compress8CR` (the `Cap8Scheme` FIELD is now DELETED) | `DeployedCapTree` §5b | it WAS a FIELD of the `Cap8Scheme` structure (`chip8CR`), so EVERY 8-felt cap-tree theorem carried it and a real deployed `Cap8Scheme` VALUE COULD NOT EXIST. The field is gone; the tree's binding is EXTRACTED AS DATA (`capOpen8_binds_leaf_or_collides`) and `DeployedCapTree.deployedCap8Scheme` is a real inhabitant. The probabilistic residual below is unchanged and still the right home for the advantage bound; the sibling `Heap8Scheme`/`Fields8Scheme` still carry the field. |
