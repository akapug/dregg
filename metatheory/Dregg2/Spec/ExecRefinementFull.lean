...
The WHOLE-HISTORY closure
`Spec.Authority.only_connectivity_begets_connectivity` — that across an ENTIRE run, no reachable edge
appears that some authorized op did not generate — is a property of the `AbsRun` CLOSURE, not of the
single-step relation. We name it as the predicate `OnlyConnectivityCloses` and PROVE it
(`onlyConnectivityCloses`) by structural induction on the run, lifting the per-step graph dynamics
each `AbsStep` constructor carries. It is no longer a hypothesis. -/

/-- **`OnlyConnectivityCloses`** — the whole-history connectivity-closure property (PROVED as
`onlyConnectivityCloses` below): along an `AbsRun a a'`, every edge present in `a'.authGraph` is
either already in `a.authGraph` OR was generated by some authorized `conserveAddEdge` step on the run
(no reachability appears ex nihilo). This is the run-level reading of `Spec.Authority`'s headline
`only_connectivity_begets_connectivity`, here discharged for the executor-level `AbsRun` LTS. It is a
property of the `AbsRun` closure, the multi-step lift of the per-step forward simulation of §3–§4. -/
...
/-- **`onlyConnectivityCloses` — the whole-history connectivity closure, PROVED (not a hypothesis).**

The run-level lift of the per-step non-amplification: along ANY `AbsRun a a'`, every edge `h ⟶ c`
present in the final graph `a'.authGraph` either (i) was ALREADY present in the initial graph
`a.authGraph`, or (ii) was added by some authorized `conserveAddEdge` step `AbsStep b b'` reached
from `a` (an `AbsRun a b`), where `b'.authGraph = addEdge b.authGraph h c` with the edge's holder
being EXACTLY the recipient of that delegation. No edge appears ex nihilo across the whole run: the
only graph-growing constructor is `conserveAddEdge`, and the proof walks the run to the LAST step that
introduced the edge.

The proof is a structural induction on `AbsRun` (the run is a finite reflexive-transitive chain, NOT
a coinductive stream), lifting the per-step shape carried by each `AbsStep` constructor:

  * `refl`: `a' = a`, so the edge is an initial edge (left disjunct).
  * `step (h1 : AbsStep a b) (h2 : AbsRun b a')`: apply the IH to the SUFFIX run `h2` at the edge.
    - IH right disjunct: the edge was added on the `b ⟶ a'` suffix; PREPEND `h1` to the witness run
      (`AbsRun.step h1`), shifting the witnessing `AbsRun a b₀` to `AbsRun a b₀`. The added-edge step
      is unchanged.
    - IH left disjunct (`b.authGraph h c`): case on the HEAD step `h1`:
        · `conserveIdentity` / `discloseSupply` — `b.authGraph = a.authGraph`, edge is initial (left).
        · `conserveRemoveEdge` — `b.authGraph = removeEdge a.authGraph ..`; `removeEdge` only
          subtracts, so the edge was already in `a.authGraph` (left).
        · `conserveAddEdge recipient cap0` — `b.authGraph = addEdge a.authGraph recipient cap0`;
          unfold: either the edge was in `a.authGraph` (left), or it IS the freshly-added edge
          (`h = recipient ∧ c = cap0`), in which case `h1` ITSELF is the witnessing step with the
          empty prefix run `AbsRun.refl a` (right disjunct).

The CREATED-THEN-REMOVED subtlety is handled by construction: a removed edge does not survive to
`a'`, and if a removed edge is re-added it is the LATER addition the induction lands on. The
cross-vat / transitive concerns do not arise at THIS layer: `ExecRights = Unit` so an `addEdge` is an
atomic connectivity grant with no rights-closure to chase; the rights-lattice non-amplification lives
at the `Spec.Authority` / forest layer (`full_delegation_rights_tooth`, `confers`). -/
theorem onlyConnectivityCloses : OnlyConnectivityCloses := by
  intro a a' run
  induction run with
  | refl a =>
      -- `a' = a`: every final edge is an initial edge.
      intro h c hedge
      exact Or.inl hedge
  | @step a b a' h1 h2 ih =>
      -- `h1 : AbsStep a b`, `h2 : AbsRun b a'`, `ih` is the closure for the SUFFIX run `b ⟶ a'`.
      intro h c hedge
      rcases ih h c hedge with hb | ⟨w, w', hrun_bw, hstep, recipient, hadd, hrec⟩
      · -- The edge is present already in `b`. Trace it back across the HEAD step `h1`.
        cases h1 with
        | conserveIdentity hbal hgraph =>
            -- graph unchanged by the head step: the edge was in `a`.
            rw [hgraph] at hb; exact Or.inl hb
        | discloseSupply delta hbal hgraph =>
            -- graph unchanged by the head step: the edge was in `a`.
            rw [hgraph] at hb; exact Or.inl hb
        | conserveRemoveEdge holder cap hbal hgraph =>
            -- removeEdge only subtracts; surviving edge was present in `a`.
            rw [hgraph, removeEdge] at hb; exact Or.inl hb.1
        | conserveAddEdge recipient cap hbal hgraph =>
            -- addEdge: either the edge was in `a`, or it IS the freshly added edge → `h1` witnesses.
            rw [hgraph, addEdge] at hb
            rcases hb with hold | ⟨heqh, heqc⟩
            · exact Or.inl hold
            · -- the freshly added edge `recipient ⟶ cap`; the head step `h1` is the witness, with
              -- the empty prefix run `AbsRun.refl a`.
              refine Or.inr ⟨a, b, AbsRun.refl a, ?_, recipient, ?_, heqh.symm⟩
              · -- `h1 : AbsStep a b` — re-presented as a `conserveAddEdge` step.
                exact AbsStep.conserveAddEdge recipient cap hbal hgraph
              · -- the post-graph shape, with `c = cap` substituted.
                rw [hgraph]; rw [heqc]
      · -- The edge was added on the SUFFIX run `b ⟶ a'`; PREPEND the head step `h1` to the witness.
        exact Or.inr ⟨w, w', AbsRun.step h1 hrun_bw, hstep, recipient, hadd, hrec⟩
...
/-! ### Mutation-confirmation: the closure BITES (not vacuously true).

We exhibit (1) the RIGHT disjunct firing on a run that GENUINELY grows the graph — the `addEdge`
witness is live, not dead code; and (2) the property's TEETH against ex-nihilo edges — a run built
ONLY of graph-preserving steps (`conserveIdentity`/`conserveRemoveEdge`/`discloseSupply`) can NEVER
exhibit an edge absent from the initial graph (the left disjunct is forced, so an "unauthorized" fresh
edge is refuted). Together these pin the theorem as substantive: it forbids reachability appearing
without an authorized `conserveAddEdge`. -/

section MutationConfirm

/-- A bare initial graph holding nothing. -/
private def G0 : Graph Label ExecRights := fun _ _ => False
/-- The state after one `conserveAddEdge` of `5 ⟶ ⟨3,()⟩`. -/
private def Gadd : Graph Label ExecRights := addEdge G0 5 ⟨3, ()⟩

private def aInit : AbstractState := ⟨0, G0⟩
private def aGrown : AbstractState := ⟨0, Gadd⟩

/-- The single authorized add-edge step (balance conserved, one fresh connectivity edge). -/
private theorem grow_step : AbsStep aInit aGrown :=
  AbsStep.conserveAddEdge 5 ⟨3, ()⟩ (by simp [conservedInDomain, aInit, aGrown]) rfl

/-- **(1) The RIGHT disjunct is LIVE.** The fresh edge `5 ⟶ ⟨3,()⟩` is NOT in the initial graph, so
the closure CANNOT discharge it via the left disjunct — it MUST produce the `conserveAddEdge` witness.
This run genuinely grows the graph; the closure traces the new edge to its authorizing step. -/
private theorem mutation_right_disjunct_fires :
    aInit.authGraph 5 ⟨3, ()⟩ = False ∧
    (∃ b b' : AbstractState, AbsRun aInit b ∧ AbsStep b b' ∧
       ∃ recipient, b'.authGraph = addEdge b.authGraph recipient ⟨3, ()⟩ ∧ recipient = 5) := by
  have hedge : aGrown.authGraph 5 ⟨3, ()⟩ := by
    show Gadd 5 ⟨3, ()⟩; simp [Gadd, addEdge]
  rcases onlyConnectivityCloses (AbsRun.step grow_step (AbsRun.refl _)) 5 ⟨3, ()⟩ hedge with
    hinit | hwit
  · -- the left disjunct is IMPOSSIBLE here: `aInit` holds no edges.
    exact absurd hinit (by simp [aInit, G0])
  · exact ⟨by simp [aInit, G0], hwit⟩

/-- **(2) The closure has TEETH against ex-nihilo edges.** A run of ONLY graph-preserving steps
(here a single `conserveIdentity`) cannot make any edge appear that was not already initial: asserting
a fresh edge at the end forces a contradiction. So an "unauthorized" connectivity grant — one with no
`conserveAddEdge` to back it — is REFUTED, not silently admitted. -/
private theorem mutation_no_exnihilo_edge
    (a' : AbstractState) (hpreserve : a'.authGraph = aInit.authGraph) :
    ¬ a'.authGraph 5 ⟨3, ()⟩ := by
  rw [hpreserve]; simp [aInit, G0]

/-- The preserving run that (2) ranges over is a genuine `AbsRun` (a `conserveIdentity` step
keeping the graph): the witness that the no-ex-nihilo arm is non-empty, not vacuous over runs. -/
private theorem mutation_preserving_run :
    AbsRun aInit ⟨0, aInit.authGraph⟩ :=
  AbsRun.step (AbsStep.conserveIdentity (by simp [conservedInDomain, aInit]) rfl) (AbsRun.refl _)

#assert_axioms mutation_right_disjunct_fires
#assert_axioms mutation_no_exnihilo_edge
#assert_axioms mutation_preserving_run
