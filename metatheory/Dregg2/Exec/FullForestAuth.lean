...
/-! ## §9 — Concrete carriers + non-vacuity teeth: `Exec/GatedForestCfg.lean`.

The starbridge production instantiation (`GatedForestCarriers` / `starbridgeCarriers`) and its `#guard`
witness forests live in `GatedForestCfg.lean` — first-class bundle, not a pinned namespace. -/
