/-
# Dregg2.Exec.FFI — the §WIDE COMPLETE-TURN wire codec (META-FILL I).

SPLIT (2026-07-13, file-split only — no proof or statement changed): the scalar exports,
the narrow `Value`/`RecordKernelState` codec, the caps-bearing codec, and the §FULL
full-turn codec (formerly the first ~1030 lines of this file) now live in
`Dregg2.Exec.FFI.Narrow`, imported here — so importing `Dregg2.Exec.FFI` still provides
the WHOLE FFI surface, unchanged. THIS file is §WIDE onward, verbatim.
-/
